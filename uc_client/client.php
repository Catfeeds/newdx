...
//获取用户是否设置安全提问
function uc_user_checksecques($uid) {
	return call_user_func(UC_API_FUNC, 'user', 'check_secques', array('uid'=>$uid));
}
...
//'短消息', 新增方法, 逐步删除ucenter自带的方法. replace of uc_pm_list
function uc_message_show($uid, $page = 1, $pagesize = 10, $msglen = 0) {
	if(! $uid) return;
	$return = call_user_func(UC_API_FUNC, 'pm', 'list', array('uid' => $uid, 'page' => $page, 'pagesize' => $pagesize, 'msglen' => $msglen));
	return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}
//type: new, total, both
function uc_message_getCount($uid, $type='new') {
	if(! $uid) return;
	$return = call_user_func(UC_API_FUNC, 'pm', 'count', array('uid' => $uid, 'type' => $type));
	return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}
//check if the session is 'new' or not
function uc_pm_is_new($uid, $fromuid, $pmid=0) {
	if(! $uid || (! $fromuid && ! $pmid)) return false;
	$return = call_user_func(UC_API_FUNC, 'pm', 'isNew', array('uid' => $uid, 'fromuid' => $fromuid, 'pmid' => $pmid));
	return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}

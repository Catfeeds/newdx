...
class usercontrol extends base {
	...
	function onlogin() {
		...
		if(empty($user)) {
			...
			$status = -2;
		} elseif($checkques && $user['secques'] != '' && $user['secques'] != $_ENV['user']->quescrypt($questionid, $answer) && $_GET['action'] != 'm_login') {
			$status = -3;
		} else {
			$status = $user['uid'];
		}
	...
	function oncheck_secques() {
		$this->init_input();
		$uid = $this->input('uid');
		if($userinfo = $_ENV['user']->get_user_by_uid($uid)) {
			return $userinfo['secques'] ? 1 : 0;
		}
		return 0;
	}

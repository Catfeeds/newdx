...
@include '/www/wwwroot/newdx/uc_client/lib/core.php';
...
//		}
	...
	function _init_setting() {
		...
		// check_setting lxp 20131017
		global $_G;
		if(count($_G['setting']) < 10){
			require DISCUZ_ROOT.'./source/plugin/logs/logs.class.php';
			$logsObj = new logs('check_setting');
			@$logsObj->log_str('get_setting_length: '.count($_G['setting']));
		}
	}
	...
	function _init_newtpl(){
		...
		global $_G;
		$_G['newtpl'] = 1;
		/*$groupidsArray = explode("\t", $_G['member']['extgroupids']);
		if($_G['uid'] && in_array('66', $groupidsArray)){
			// 用户组判定切换
			$_G['newtpl'] = 1;
		} else {
			// cookie切换
			$_G['newtpl'] = getcookie('2014tpl');
		}*/
 		isset($_GET['old']) && $_G['newtpl'] = 0;  // 参数切换老版
...
class DB
{
	...
	//sql写入redis
	function redis_record($sql){
		global $_G;
		// $open_mark=memory('get','record_mark');
		$open_mark=$_G['cache']['plugin']['record']['config_open_or_close'];
		if($open_mark==1){
			$mem_record_setting=memory('get','mem_record_setting');
			if($mem_record_setting){
				$rediscount=memory('get','rediscount');
				require_once libfile('class/myredis');
				$redis = & myredis::instance();
				
				if($redis->connected){//判断redis 是否连接成功
					$rediscount_now = $redis->obj->lsize('redis_record');
					if($rediscount_now<$rediscount){
						//从SQL中获取表名
						$sql =strtolower($sql);
						$list=explode(' ',$sql);
						$type=$list[0];
						...
						foreach($tablename as $v){
							if(in_array($v,$tablename_arr)){
								if(in_array($type,$mem_record_setting[$v])){
									$time=time();
									$record=$time.' '.$sql;
									$redis->obj->lpush('redis_record',$record);
									break;
								}
							}
						}

	...
	function onignore() {
 	...
 	function onls() {
		...
		if($folder != 'searchbox') {
 			...
 			$newCount = $_ENV['pm']->get_num($this->user['uid'], 'newbox', $filter);
 	...
 	function onlist() {
 		...
 		$this->user['uid'] = intval($this->input('uid'));
 		$start = ($page - 1) * $pagesize;
 		$datas = $_ENV['pm']->getDataList($this->user['uid'], $start, $pagesize);

 		if($datas) {
 			foreach($datas as $k => $msg) {
				if($msglen) {
					$datas[$k]['message'] = htmlspecialchars($_ENV['pm']->removecode($datas[$k]['message'], $msglen));
				} else {
					unset($datas[$k]['message']);
				}
				unset($datas[$k]['folder']);
			}
 		}
 		return $datas;
 	}
 	function oncount() {
 		...
 		switch ($type) {
 			case 'new':
 				$counts['new'] = $_ENV['pm']->getCount($uid, 'new');
 				break;
 			case 'total':
 				$counts['total'] = $_ENV['pm']->getCount($uid, 'total');
 				break;
 			default:
 				$counts['new'] = $_ENV['pm']->getCount($uid, 'new');
 				$counts['total'] = $_ENV['pm']->getCount($uid, 'total');
 	...
 	function onisNew() {
 		...
 		return $_ENV['pm']->isNew($uid, $fromuid, $pmid);
 	}
 	function onviewnode() {
  		$this->init_input();
  		$this->user['uid'] = intval($this->input('uid'));

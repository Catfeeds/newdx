...
class pmmodel {
	...
	//replace of get_pm_list
	function getDataList($uid, $start = 0, $count = 10) {
		if(! $uid) return;
		$sql = 'SELECT pm.pmid, pm.msgfromid, pm.msgtoid, pm.folder, pm.new, pm.subject, pm.dateline, pm.message, pm.delstatus, pm.related, pm.fromappid, m.username as msgfrom FROM ' . UC_DBTABLEPRE . 'pms pm LEFT JOIN ' . UC_DBTABLEPRE 
		. "members m ON pm.msgfromid = m.uid WHERE pm.msgtoid={$uid} AND pm.related=0 AND pm.folder='inbox' ORDER BY pm.new DESC, pm.dateline DESC LIMIT {$start}, {$count}";
		if($uid == 34404746) {
			require_once DISCUZ_ROOT.'./source/plugin/logs/logs.class.php';
			$logs = new logs('cron_delete_invite');
			$logs->log_str("{$uid} : {$sql}");
		}
		$query = $this->db->query($sql);
		$array = array();
		$today = $this->base->time - $this->base->time % 86400;
	...
	//replace of get_num. type: new, total
	function getCount($uid, $type) {
		if(! $uid) return;
		$sql =  'SELECT count(*) FROM ' . UC_DBTABLEPRE . "pms WHERE msgtoid={$uid} AND related=0 AND folder='inbox'";
		if($type == 'new') {
			$sql .= ' AND new=1';
		}
		$num = $this->db->result_first($sql);
	...
	function isNew($uid, $fromuid, $pmid=0) {
		if($fromuid) {
			$sql = 'SELECT new FROM ' . UC_DBTABLEPRE . "pms WHERE msgfromid={$fromuid} AND msgtoid={$uid} AND related=0 AND folder='inbox' LIMIT 1";
		}
		elseif ($pmid) {
			$sql = 'SELECT new FROM ' . UC_DBTABLEPRE . "pms WHERE pmid='{$pmid}' AND (msgtoid IN ('{$uid}','0') OR msgfromid='{$uid}') LIMIT 1";
		}
		else{
			return false;
		}
		$new = $this->db->result_first($sql);
		return $new;
	}

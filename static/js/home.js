...
function showFace(showid, target, dropstr) {
	if($(showid + '_menu') != null) {
		...
		for(i=1; i<31; i++) {
			...
			faceli.innerHTML = '<img src="http://static.8264.com/static/image/smiley/comcom/'+i+'.gif" onclick="insertFace(\''+showid+'\','+i+', \''+ target +'\', \''+dropstr+'\')" style="cursor:pointer; position:relative;" />';
...
function wall_add(id) {
	...
	if ($('comment_wall')) {
		x.get('home.php?mod=misc&ac=ajax&op=comment&inajax=1&cid='+id, function(s){
			var newdl = document.createElement("li");
			newdl.id  = 'comment_'+id+'_li';
			newdl.className = 'list-comment-item';
			newdl.innerHTML = s;
			obj = obj.firstChild;

			...
			for(var typeid in smilies_array) {
				for(var page in smilies_array[typeid]) {
					for(var i in smilies_array[typeid][page]) {
						...
						str = str.replace(re, '<img src="http://static.8264.com/static/image/smiley/' + smilies_type[typeid][1] + '/' + smilies_array[typeid][page][i][2] + '" border="0" smilieid="' + smilies_array[typeid][page][i][0] + '" alt="' + smilies_array[typeid][page][i][1] + '" />');
					}
				}
			}
		}
...
function imgtag(attributes) {
	...
	//为了兼容Google浏览器下图片居中后给加上的行高1.5，所以把height大于0改为了2

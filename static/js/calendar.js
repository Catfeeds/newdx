...
function loadcalendar() {
	...
	s += '<tr align="center" id="calendar_week"><td><a href="###" onclick="refreshcalendar(yy, mm-1);return false;" title="上一月">《</a></td><td colspan="5" style="text-align: center"><a href="###" onclick="showdiv(\'year\');doane(event);return false;" class="dropmenu" title="点击选择年份" id="year"></a>&nbsp; - &nbsp;<a id="month" class="dropmenu" title="点击选择月份" href="###" onclick="showdiv(\'month\');doane(event);return false;"></a></td><td><A href="###" onclick="refreshcalendar(yy, mm+1);return false;" title="下一月">》</A></td></tr>';
	...
	for(var k = 2020; k >= 1931; k--) {
		...
		s += '<a href="###" onclick="refreshcalendar(' + k + ', mm);$(\'calendar_year\').style.display=\'none\';return false;"><span' + (today.getFullYear() == k ? ' class="calendar_today"' : '') + ' id="calendar_year_' + k + '">' + k + '</span></a><br />';
	}
	s += '</div></div>';
	s += '<div id="calendar_month" onclick="doane(event)" style="display: none;z-index:100001;">';
	for(var k = 1; k <= 12; k++) {
		s += '<a href="###" onclick="refreshcalendar(yy, ' + (k - 1) + ');$(\'calendar_month\').style.display=\'none\';return false;"><span' + (today.getMonth()+1 == k ? ' class="calendar_today"' : '') + ' id="calendar_month_' + k + '">' + k + ( k < 10 ? '&nbsp;' : '') + ' 月</span></a><br />';
	}
	s += '</div>';
...
function settime(d) {
	...
	if(typeof(calendarCallback)=="function"){
		calendarCallback(controlid);
	}        
}
...
function showcalendar(event, controlid1, addtime1, startdate1, enddate1, newinputHeight) {
	newinputHeight = isUndefined(newinputHeight) ? 20 : newinputHeight;
	...
	$('calendar').style.top	= (p['top'] + newinputHeight)+'px';
	doane(event);

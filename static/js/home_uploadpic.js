...
function delAttach(id) {
	$('attachbody').removeChild($('attach_' + id).parentNode.parentNode.parentNode.parentNode);
...
function addAttach() {
	...
	nowid++;
	var first_child = $('attachbody').firstChild;

	if(first_child != undefined && first_child != null) {
		$('attachbody').insertBefore(newnode, first_child);
	}
	else{
...
function insertAttach(id) {
	...
	$('attach_' + id).parentNode.style.display = 'none';

	addAttach();
}
function insert_all_imagetag() {
	var max_id = nowid - 1;
	for(var i=0; i<max_id; i++) {
		if($("attach_"+i) != undefined && $("attach_"+i) != null) {
			insertAttachimgTag(i);
		}
	}
}
...
function upload() {
	...
	var backup_form = forms;
	if(nowUid == 0) {
		forms = [];
		for(var i=0; i<form_length; i++) {
			forms[i] = backup_form[form_length-1-i];
		}
	}

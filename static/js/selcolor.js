...
function intocolor(divid,  obj, oid) {
	...
	colorTable='<table cellpadding="0" cellspacing="1" border="0" style="border-collapse: collapse;width:100%; background:#000;"><tr><td align="right" style="padding: 3px 2px 0 0;"><span style="cursor:pointer; color: #FFF;" onClick="setdisplay(\''+mdivid+'\');">关闭</span></td></tr></table>'+'<table border="1" id="selcolortable" cellspacing="0" cellpadding="0" style="border-collapse: collapse" bordercolor="000000" onclick="doclick()" style="cursor:pointer;">'
...
function doclick() {
	var bgcolor;
	if (document.all){ bgcolor = getEvent().srcElement.style.backgroundColor;}
	else { bgcolor = getEvent().target.style.backgroundColor;}

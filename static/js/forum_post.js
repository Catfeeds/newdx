...
function validate(theform) {
	...
	if(typeof validateextra == 'function') {
		var v = validateextra(theform);
		if(!v) {
			return false;
		}
	}
	if(typeof validateextra_activity == 'function') {
		var v = validateextra_activity(theform);
		if(!v) {
			return false;
		}
	}
	if(typeof validateextra_reward == 'function') {
		var v = validateextra_reward(theform);
		if(!v) {
			return false;
		}
	}
...
function validatenosymbol(theform) {
	var message = wysiwyg ? html2bbcode(getEditorContents()) : (!theform.parseurloff.checked ? parseurl(theform.message.value) : theform.message.value);
	if(($('postsubmit').name != 'replysubmit' && !($('postsubmit').name == 'editsubmit' && !isfirstpost) && theform.subject.value == "") || !sortid && !special && trim(message) == "") {
		showDialog('请完成标题或内容栏');
		return false;
	...
	var valid_string = message;
	valid_string = valid_string.replace(/[,.!"';:()，。：“”‘’！（）]/g, '');

	if(!disablepostctrl && !sortid && !special && ((postminchars != 0 && mb_strlen(valid_string) < postminchars) || (postmaxchars != 0 && mb_strlen(valid_string) > postmaxchars))) {
...
function updateImageList(action) {
	ajaxget('forum.php?mod=ajax&action=imagelist&new_template=1&pid=' + pid + '&posttime=' + $('posttime').value + (!fid ? '' : '&fid=' + fid), 'imgattachlist');
...
function updatesortattach(aid, url, attachurl, identifier) {
	...
	serverid = get_serverid_by_cookie();
	$('sortserverid_' + identifier).value = serverid;
	$('sortfilename_' + identifier).value = url;
	attachurl = get_domain_by_cookie(attachurl);
...
function get_domain_by_cookie(url){
	var thisurl = getcookie('upload_attach_8264_attachmentname');
	if(thisurl == ''){
		thisurl = url;
	}else{
		thisurl = "http://"+thisurl+"/forum";
	}
	return thisurl;
}
function get_serverid_by_cookie(){
	var thisurl = getcookie('upload_attach_8264_attachmentserverid');
	if(thisurl == '' || thisurl == 0) thisurl = 0;
	return thisurl;
}

...
function editorsize(op, v) {
	...
	if($(editorid + '_iframe')) {
        ...
        $('upload_newimage_tools').style.height = $(editorid + '_iframe').contentWindow.document.body.style.height = editorcurrentheight + 'px';
	}
	if(framemObj) {
...
function ajaxalbumlist(picurl , picurllen) {
	...
	if(message) {
		...
		if(allcount > 2) {
			...
			showDialog('您好，为了提高网友的浏览速度，每一楼只限三张照片，其它的照片请以跟贴的形式上传。');
			return;
		}
	}	
	if(wysiwyg) {
...
function insertAttachimgTag(aid) {
	...
	if(wysiwyg) {
		insertText('<br/><br/><img src="' + $('image_' + aid).src + '" border="0" aid="attachimg_' + aid + '" alt="" />', false);
	} else {

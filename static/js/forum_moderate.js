...
//点评的回复 删除使用的方法
function modaction_delcomreply(action, pid, extra, mod,parentpid) {
	if(!action) {
		return;
	}
	var mod = mod ? mod : 'forum.php?mod=topicadmin';
	...
	if(!checked) {
		...
		$('modactions').action = mod + '&action='+ action +'&fid=' + fid + '&tid=' + tid + '&parentpid=' + parentpid + '&handlekey=mods&infloat=yes&nopost=yes' + (!pid ? '' : '&topiclist[]=' + pid) + extra + '&r' + Math.random();
		showWindow('mods', 'modactions', 'post');
		if(BROWSER.ie) {
			doane(event);
		}
...
//区别点评和点评回复
function modaction_comreply(action, pid, extra, mod,count) {
	if(!action) {
		return;
	}
	var mod = mod ? mod : 'forum.php?mod=topicadmin';
	...
	if(!checked) {
		...
		$('modactions').action = mod + '&action='+ action +'&fid=' + fid + '&tid=' + tid + '&count=' + count + '&handlekey=mods&infloat=yes&nopost=yes' + (!pid ? '' : '&topiclist[]=' + pid) + extra + '&r' + Math.random();
		showWindow('mods', 'modactions', 'post');
		if(BROWSER.ie) {
			doane(event);
		}
...
function modthreads(optgroup, operation, typeid) {
	var operation = !operation ? '' : operation;
	$('modactions').action = 'forum.php?mod=topicadmin&action=moderate&typeid=' + typeid + '&fid=' + fid + '&moderate[]=' + tid + '&handlekey=mods&infloat=yes&nopost=yes' + (optgroup != 3 && optgroup != 2 ? '&from=' + tid : '');

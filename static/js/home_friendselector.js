...
var friendSelector = function(parameter) {
	...
	this.allNumber = isUndefined(parameter['allNumber']) ? 0 : parseInt(parameter['allNumber']);
...
friendSelector.prototype.append = function(uid, filtrate) {
	...
	if(this.showType == 1) {
		...
		liObj.innerHTML = '<a href="javascript:;" id="' + liObj.userid + '" onclick="' + this.handleKey + '.select(this.id)" class="cl"><img src="' +
		this.dataSource[uid]['avatar'] + '"/><span>' + username+ '</span></a>';
	} else {
...
friendSelector.prototype.select = function(uid) {
	...
	if(uid){
		...
		if(typeof this.selectUser[uid] == 'undefined') {
			...
			this.selectNumber++;
			if(this.showType == '1') {
				$(uid).parentNode.className = 'selected';
			}
			select = true;
		} else {

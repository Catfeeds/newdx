...
0
|| checktplrefresh('./template/8264/portal/list_880.htm', './template/8264/common/float_guide.htm', 1509518094, 'diy', './data/template/2_diy_portal_list_880.tpl.php', './template/8264', 'portal/list_880')
|| checktplrefresh('./template/8264/portal/list_880.htm', './template/8264/common/ewm_l.htm', 1509518094, 'diy', './data/template/2_diy_portal_list_880.tpl.php', './template/8264', 'portal/list_880')
|| checktplrefresh('./template/8264/portal/list_880.htm', './template/8264/common/footer_8264_new.htm', 1509518094, 'diy', './data/template/2_diy_portal_list_880.tpl.php', './template/8264', 'portal/list_880')
;?><?php include template('common/header2014'); ?><!--[name]勇者先行[/name]-->
<link href="http://static.8264.com/static/css/portal/list_880.css" rel="stylesheet" type="text/css">
<style id="diy_style" type="text/css"></style>
<div class="layout" style="padding-bottom:30px;">
<div class="pioneerHead">
<p><strong>《勇者先行》</strong>
栏目意在为大家介绍在户外探险的各个领域中最专业、最极限的那些先行者，他们是各自领域内走在最前沿的人，引领着自己所在的探险领域的发展。他们的故事，将永远流传，激励着每一个热爱生活、勇于探索的人不断向前。
</p>
</div><?php if(is_array($array)) foreach($array as $item) { ?><div class="pioneerCol">
<a href="<?php echo $item['url'];?>" class="mainPic" target="_blank">
<img src="<?php echo $item['pic'];?>"/>
</a>
<h3><?php echo $item['titles'];?></h3>
<h4><?php echo $item['shortname'];?></h4>
<div class="blackLayout"></div>
<div class="tips">
<span class="attention">关注 （<?php echo $item['viewnum'];?>）</span>
<span class="comment">评论 （<?php echo $item['commentnum'];?>）</span>
</div>
</div>
<?php } ?><?php $count = DB::result_first("SELECT COUNT(*) FROM ".DB::table('portal_article_title')." WHERE catid IN (880) AND pic<>''"); ?><?php $ppp = 100; ?><?php $multi = multi($count, $ppp, $page, 'portal.php?mod=list&catid=880'); if($multi) { ?>
<div style=" width:100%; height:36px; text-align:center; padding-top:20px;"><?php echo $multi;?></div>
<?php } ?>
</div>
</div>

<?php if(!defined('IN_DISCUZ')) exit('Access Denied'); hookscriptoutput('spacecp_credit_base', 'common/header_8264_new');
0
|| checktplrefresh('./template/8264/home/spacecp_credit_base.htm', './template/8264/common/header_8264_new.htm', 1509517975, '2', './data/template/2_2_home_spacecp_credit_base.tpl.php', './template/8264', 'home/spacecp_credit_base')
|| checktplrefresh('./template/8264/home/spacecp_credit_base.htm', './template/8264/home/space_header_uc.htm', 1509517975, '2', './data/template/2_2_home_spacecp_credit_base.tpl.php', './template/8264', 'home/spacecp_credit_base')
|| checktplrefresh('./template/8264/home/spacecp_credit_base.htm', './template/8264/home/space_footer_uc.htm', 1509517975, '2', './data/template/2_2_home_spacecp_credit_base.tpl.php', './template/8264', 'home/spacecp_credit_base')
|| checktplrefresh('./template/8264/home/spacecp_credit_base.htm', './template/8264/common/footer_8264_new.htm', 1509517975, '2', './data/template/2_2_home_spacecp_credit_base.tpl.php', './template/8264', 'home/spacecp_credit_base')
|| checktplrefresh('./template/8264/home/spacecp_credit_base.htm', './template/8264/common/header_common.htm', 1509517975, '2', './data/template/2_2_home_spacecp_credit_base.tpl.php', './template/8264', 'home/spacecp_credit_base')
|| checktplrefresh('./template/8264/home/spacecp_credit_base.htm', './template/8264/common/index_ad_top.htm', 1509517975, '2', './data/template/2_2_home_spacecp_credit_base.tpl.php', './template/8264', 'home/spacecp_credit_base')
|| checktplrefresh('./template/8264/home/spacecp_credit_base.htm', './template/8264/common/ewm_r.htm', 1509517975, '2', './data/template/2_2_home_spacecp_credit_base.tpl.php', './template/8264', 'home/spacecp_credit_base')
;
block_get('6905');?>
<!doctype html>
<html>
...
<h3 class="tit"><?php if($space['self']) { ?>我<?php } else { ?><?php echo $space['username'];?><?php } ?>的积分</h3>
</div>				
...
<tr>
<th>驴币</th>
<td><?php echo $space['extcredits2'];?></td>
</tr>
<tr>
<th>8264币</th>
<td><?php echo $space['extcredits5'];?> 枚&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="http://bbs.8264.com/forum-483-1.html" target="_blank" style="color: red;">上限40枚，超过40系统自动扣除，请及时兑换礼品</a></td>
</tr>
<tr>
<th>积分</th>
<td><?php echo $space['credits'];?> ( <?php echo $_G['setting']['creditsformulaexp'];?> )</td>
</tr>
</table>
<?php if(($_G['setting']['exchangestatus'] || $_G['setting']['transferstatus'] || $_G['setting']['ec_ratio']) && $space['self']) { ?>
<!--8264币兑换驴币-->
<?php if($_G['setting']['exchangestatus'] && ($_G['setting']['extcredits'] || $_CACHE['creditsettings'])) { ?>
...
<select name="tocredits" id="tocredits" class="sel-ps" onChange="exchangecalcredit()"><?php if(is_array($_G['setting']['extcredits'])) foreach($_G['setting']['extcredits'] as $id => $ecredits) { if($ecredits['allowexchangein'] && $ecredits['ratio']) { ?>
...
<?php } } ?><?php $i=0; if(is_array($_CACHE['creditsettings'])) foreach($_CACHE['creditsettings'] as $id => $data) { ?><?php $i++; if($data['title']) { ?>
...
<select name="fromcredits" id="fromcredits_0"class="sel-ps"  style="display: none" onChange="exchangecalcredit();"><?php if(is_array($_G['setting']['extcredits'])) foreach($_G['setting']['extcredits'] as $id => $credit) { if($credit['allowexchangeout'] && $credit['ratio']) { ?>
...
<?php } } ?>
</select><?php $i=0; if(is_array($_CACHE['creditsettings'])) foreach($_CACHE['creditsettings'] as $id => $data) { ?><?php $i++; ?><select name="fromcredits_<?php echo $i;?>" class="sel-ps" id="fromcredits_<?php echo $i;?>" style="display: none" onChange="exchangecalcredit()"><?php if(is_array($data['creditsrc'])) foreach($data['creditsrc'] as $id => $ratio) { ?><option value="<?php echo $id;?>" unit="<?php echo $_G['setting']['extcredits'][$id]['unit'];?>" title="<?php echo $_G['setting']['extcredits'][$id]['title'];?>" ratiosrc="<?php echo $data['ratiosrc'][$id];?>" ratiodesc="<?php echo $data['ratiodesc'][$id];?>"><?php echo $_G['setting']['extcredits'][$id]['title'];?></option>
<?php } ?>
</select>
<?php } ?>
<script type="text/javascript">
...
<button type="submit" name="exchangesubmit" class="pn-org" id="exchangesubmit" value="true" tabindex="2"><em>兑换</em></button>
</td>
<td class="d">
...
if(!tocredit) {
return;
}<?php $i=0; if(is_array($_CACHE['creditsettings'])) foreach($_CACHE['creditsettings'] as $id => $data) { ?><?php $i++; ?>$('fromcredits_<?php echo $i;?>').style.display = 'none';
<?php } ?>
...
<!--我的积分 end-->	
<?php } elseif($_G['gp_op'] == 'rule') { ?>
<!--积分规则-->
<div class="basic-panel" id="basic_info">
<div class="basic-info-tit">
<h3 class="tit">积分规则</h3>
</div>				
<div class="basic-msg-panel"><?php $_TPL['cycletype'] = array(
'0' => '一次性',
'1' => '每天',
'2' => '整点',
'3' => '间隔分钟',
...
<th>动作名称</th>
<th>周期范围</th>
<th>周期内最多奖励次数</th><?php if(is_array($_G['setting']['extcredits'])) foreach($_G['setting']['extcredits'] as $key => $value) { ?><th><?php echo $value['title'];?></th>
<?php } ?>
...
<td><?php if($value['rewardnum']) { ?><?php echo $value['rewardnum'];?><?php } else { ?>不限次数<?php } ?></td><?php if(is_array($_G['setting']['extcredits'])) foreach($_G['setting']['extcredits'] as $key => $credit) { ?><?php $creditkey  = 'extcredits'.$key; ?><?php $creditUnit = $key == 2 ? '' : '枚'; ?><td><?php if($value[$creditkey] > 0) { ?>+<?php echo $value[$creditkey];?><?php echo $creditUnit;?><?php } elseif($value[$creditkey] < 0) { ?><?php echo $value[$creditkey];?><?php echo $creditUnit;?><?php } else { ?>&nbsp;<?php } ?></td>
<?php } ?>
</tr>						
<?php } ?>
<tr>
...
<li class="menu-nav <?php if($_G['gp_op'] == 'base') { ?>menu-nav-current<?php } ?>">
<a href="home.php?mod=spacecp&amp;ac=credit&amp;uid=<?php echo $space['uid'];?>&amp;op=base">
<i class="menu-nav-icon icon-basic"></i>
<em><?php if($space['self']) { ?>我<?php } else { ?><?php echo $space['username'];?><?php } ?>的积分</em>
</a>
</li>
<li class="menu-nav <?php if($_G['gp_op'] == 'rule') { ?>menu-nav-current<?php } ?>">
<a href="home.php?mod=spacecp&amp;ac=credit&amp;uid=<?php echo $space['uid'];?>&amp;op=rule">

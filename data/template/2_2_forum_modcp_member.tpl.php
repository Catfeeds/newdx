...
<form method="post" autocomplete="off" action="<?php echo $cpscript;?>?mod=modcp&action=<?php echo $_G['gp_action'];?>&op=<?php echo $op;?>" class="schresult">
<input type="hidden" name="formhash" value="<?php echo FORMHASH;?>">
<input type="hidden" name="username" value="<?php echo $_G['gp_username'];?>">
...
<input type="radio" name="bannew" id="bannew_0" value="0" checked="checked" class="pr" /> <label for="bannew_0">正常状态</label> 
<?php } if($member['groupid'] != 4) { ?><input type="radio" name="bannew" id="bannew_4" class="pr" value="4" <?php if($member['groupid'] != 4 && $member['groupid'] != 5) { ?>checked="checked"<?php } ?> /> <label for="bannew_4">禁止发言</label> <?php } if($member['groupid'] != 5) { ?><input type="radio" name="bannew" id="bannew_5" class="pr" value="5" /> <label for="bannew_5">禁止访问</label><?php } ?>
</td>
</tr>
<tr>
<th>期限:</th>
...
&nbsp;&nbsp;* 表示整个 IP 段，如 192.168.*.* 表示禁止所有以 192.168 开头的IP。
</td>
</tr>
<tr>
<th width="15%">期限:</th>
...
<?php } ?>
<tr class="bw0_all">
<td><input type="checkbox" name="chkall" id="chkall" class="pc" onclick="checkall(this.form)" /> <label for="chkall">全选</label></td>

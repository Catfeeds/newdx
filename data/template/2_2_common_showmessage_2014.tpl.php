...
<?php if(!$param['wechatshare']) { ?><!--当展示二维码分享提示框时不加载这段js-->
...
<div class="bigfont">
<?php if($param['info_activity'] == '报名成功') { ?>
            <!--报名成功-->
            ...
            <div class="smallfont">报名该活动的用户还喜欢以下活动</div>
              ...
              <?php if(is_array($huodong_zaiwai)) foreach($huodong_zaiwai as $k => $hd) { ?>                <div class="hdone">
                    <a target="_blank" href="<?php echo $hd['url_pc'];?>">
                        <div class="hdimg">
                            ...
                            <div class="hdjiadate">
                                <span><?php echo date("m/d", $hd['start_time']);; ?> <?php echo $hd['start_place'];?>出发</span>
            ...
            <?php } ?>
    		<div class="yhzbox">8264活动平台，百种玩法数千线路，总能找到你想要的<a href="http://hd.8264.com/" target="_blank" class="gohdindex small">去看看</a></div>
<?php } else { ?>


<a href="http://bbs.8264.com/thread-5422622-1-1.html" target="_blank" style="text-decoration:underline;">长期征集各地俱乐部团队户外活动作业（图文并茂） 8264帮你宣传俱乐部</a>
<?php } ?>
</div>
...
<a href="<?php echo $url_forward;?>" class="hard_s">如果你的浏览器没有自动跳转，请点击此链接</a>
<?php } else { ?>
<a href="<?php echo $url_forward;?>" class="link_ts">如果 <?php echo $refreshsecond;?> 秒后下载仍未开始，请点击此链接</a>
...
<a href="http://www.8264.com/zb" class="link_ts">点击这里返回装备分享主页</a>
<?php } ?>
</div>
<div class="r_102"></div>
</div>
...
<?php if($url_forward) { if(!$param['redirectmsg']) { ?>
<a href="<?php echo $url_forward;?>" class="link_ts">如果你的浏览器没有自动跳转，请点击此链接</a>
<?php } else { ?>
<a href="<?php echo $url_forward;?>" class="link_ts">如果 <?php echo $refreshsecond;?> 秒后下载仍未开始，请点击此链接</a>
<?php } } elseif($allowreturn) { ?>
<script type="text/javascript">
...
<?php } if($_GET['mod']=='post'&&$_GET['fid']==437&&$_G['uid']) { ?>
<a href="http://www.8264.com/zhuangbei.html" class="link_ts">点击这里返回装备分享主页</a>
<?php } ?>
</p>
</div>
<?php } if($_GET['aid']) { ?>
...
_BFD.BFD_INFO = {
iid : '<?php echo $_GET['aid'];?>'; 	//本文章的id号（被删除文章的id号）
};
...
<?php } else { ?>
<p class="o pns"><button type="button" class="pn pnc" onclick="hideWindow('<?php echo $_G['gp_handlekey'];?>');"><strong>关闭</strong></button></p>
<?php } } else { ?>
<?php echo $show_message;?>

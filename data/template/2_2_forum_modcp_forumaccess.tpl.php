...
<h1 class="mt">用户权限</h1>
<div class="mbm">通常情况下，用户在版块的权限是根据他的用户组决定的，此处你可以限制某个用户在某版块的权限。<br />注意: 看帖是基本权限，一旦禁止, 其他权限会同时进行禁止。<br />图例说明: <img src="static/image/common/access_normal.gif" class="vm" /> 默认权限&nbsp;&nbsp;&nbsp;&nbsp;<img src="static/image/common/access_disallow.gif" class="vm" /> 强制禁止&nbsp;&nbsp;&nbsp;&nbsp;<img src="static/image/common/access_allow.gif" class="vm" />强制允许</div>
...
<form method="post" autocomplete="off" action="<?php echo $cpscript;?>?mod=modcp&action=<?php echo $_G['gp_action'];?>">
...
管理员设置此用户某些权限为强制允许，你不能变更管理员的这些设置
<?php } ?>
</span>
</td>
</tr>
...
<th>权限变更:</th>
<td>
<input type="checkbox" value="1" name="deleteaccess" id="deleteaccess" onclick="disallaccess(this)" class="pc" /> <label for="deleteaccess">恢复默认</label>&nbsp;
<span id="customaccess">
<input type="checkbox" value="-1" name="new_view" id="new_view" onclick="chkallaccess(this)" class="pc" /> <label for="new_view">禁止查看主题</label>&nbsp;
<input type="checkbox" value="-1" name="new_post" id="new_post" class="pc" /> <label for="new_post">禁止发表主题</label>&nbsp;
<input type="checkbox" value="-1" name="new_reply" id="new_reply" class="pc" /> <label for="new_reply">禁止发表回复</label>&nbsp;
<input type="checkbox" value="-1" name="new_getattach" id="new_getattach" class="pc" /> <label for="new_getattach">禁止下载附件</label>&nbsp;
<input type="checkbox" value="-1" name="new_postattach" id="new_postattach" class="pc" /> <label for="new_postattach">禁止上传附件</label>&nbsp;
<input type="checkbox" value="-1" name="new_postimage" id="new_postimage" class="pc" /> <label for="new_postimage">禁止上传图片</label>
</span>
</td>
</tr>
<tr>
...
<?php } } else { ?>
<tr><td colspan="10"><p class="emp">当前没有特殊权限用户</p></td></tr>
<?php } ?>
</table>

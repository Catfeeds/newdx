<?php if(!defined('IN_DISCUZ')) exit('Access Denied'); hookscriptoutput('register_wechat', 'common/header_8264_new');?><?php include template('common/header_8264_new'); ?><link href="http://static.8264.com/static/css/common/style.css?<?php echo VERHASH;?>" rel="stylesheet" type="text/css">
...
<script>
    jQuery(document).ready(function($){
        ...
        function regIsAppearVerify(){
            var flag = false;
            var url = "<?php echo $_G['config']['web']['portal'];?>"+'forum.php?mod=ajax&inajax=yes&infloat=register&handlekey=register&ajaxmenu=1&action=regIsAppearVerify';
            url = decodeURI(url);
            jQuery.ajax({
                async:false ,
                type: 'POST',
...
<form method="post" autocomplete="off" name="register" id="registerform" enctype="multipart/form-data"  action="<?php echo $_G['config']['web']['portal'];?>connect.php?action=login&method=wechat&action=register&isSubmit=yes">
                    <input type="hidden" name="wechat_reg_submit" value="yes" />
...
<form method="post" autocomplete="off" name="login" id="bindform_<?php echo $loginhash;?>" class="cl" action="<?php echo $_G['config']['web']['portal'];?>connect.php?action=login&method=wechat&op=bind">
<input type="hidden" name="wechat_bind_submit" value="yes">
                        <div class="inputlist">
...
! jQuery(this).val() && jQuery("#n_password3_<?php echo $loginhash;?>").html("密码");
                                ...
                                function loginIsAppearVerify(){
                                    ...
                                    var url = "<?php echo $_G['config']['web']['portal'];?>"+'forum.php?mod=ajax&inajax=yes&infloat=register&handlekey=register&ajaxmenu=1&action=loginIsAppearVerify';
                                    url = decodeURI(url);
                                    $.ajax({
                                        async:false ,
                                        type: 'POST',

...
;?>
<?php $_G[home_tpl_titles] = array($album[albumname], '相册'); include template('common/header'); if($_GET['op']=='edit' || $_GET['op']=='editpic') { ?>
...
<?php } } ?><div class="z"><a href="./" class="nvhm" title="首页"><?php echo $_G['setting']['bbname'];?></a> <em>&rsaquo;</em> <a href="home.php"><?php echo $_G['setting']['navs']['4']['navname'];?></a> <em>&rsaquo;</em> <a href="home.php?mod=space&amp;do=album">相册</a><?php if($album['albumname']) { ?> <em>&rsaquo;</em> <a href="home.php?mod=space&amp;uid=<?php echo $album['uid'];?>&amp;do=album&amp;id=<?php echo $albumid;?>"><?php echo $album['albumname'];?></a><?php } ?> <em>&rsaquo;</em> 编辑相册</div>
</div>
...
<table cellspacing="0" cellpadding="0" class="tfm">
<caption>提示：你可以指定一张图片作为当前相册的封面图片，但是，在下次上传新的图片后，系统会自动选择一张新图片来更新本相册的封面图片。</caption><?php $common = ''; if(is_array($list)) foreach($list as $value) { ?><tr>
...
<select name="newalbumid" class="ps"><?php if(is_array($albumlist)) foreach($albumlist as $key => $value) { if($albumid != $value['albumid']) { ?><option value="<?php echo $value['albumid'];?>"><?php echo $value['albumname'];?></option><?php } } if($albumid>0) { ?><option value="0">默认相册</option><?php } ?>
</select>
<?php } ?>

<p class="d">删除图片提示：如果你要删除的图片出现在你的帖子、日志、话题中，删除后，会导致内容里面的图片同时无法显示。,</p>
</td>
</tr>
</table>
...
<option value="0">转移到 默认相册</option><?php if(is_array($albums)) foreach($albums as $value) { if($albumid != $value['albumid']) { ?>
<option value="<?php echo $value['albumid'];?>">转移到 <?php echo $value['albumname'];?></option>
<?php } } ?>
</select>				
</li>
<li>
<label></label>
<button type="submit" name="deletesubmit" value="true" class="button_confirm">确定</button>
</li>
</ul>
</div>
</div>
...
<h3 class="flb">
<em id="return_<?php echo $_G['gp_handlekey'];?>">编辑说明</em>

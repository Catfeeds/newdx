...
<?php if(!$post['message'] && (($_G['forum']['ismoderator'] && $_G['group']['alloweditpost'] && (!in_array($post['adminid'], array(1, 2, 3)) || $_G['adminid'] <= $post['adminid'])) || ($_G['forum']['alloweditpost'] && $_G['uid'] && $post['authorid'] == $_G['uid']))) { ?><div class="ptw pbm cl"><a href="forum.php?mod=post&amp;action=edit&amp;fid=<?php echo $_G['fid'];?>&amp;tid=<?php echo $_G['tid'];?>&amp;pid=<?php echo $post['pid'];?><?php if(!empty($_G['gp_modthreadkey'])) { ?>&amp;modthreadkey=<?php echo $_G['gp_modthreadkey'];?><?php } ?>&amp;page=<?php echo $page;?>" class="z pn"><span class="z">添加柜台介绍</span></a></div><div class="pbw xs1">您可以在本帖中继续<a href="forum.php?mod=post&amp;action=reply&amp;fid=<?php echo $_G['fid'];?>&amp;tid=<?php echo $_G['tid'];?>&amp;firstpid=<?php echo $post['pid'];?>&amp;addtrade=yes<?php if(!empty($_G['gp_from'])) { ?>&amp;from=<?php echo $_G['gp_from'];?><?php } ?>"">添加多个商品</a>,并可以针对帖内所有商品添加统一的 "<strong>柜台介绍</strong>"</div><?php } else { ?><?php echo $post['message'];?><?php } ?>
</td></tr></table>
...
<a href="forum.php?mod=misc&amp;action=tradeorder&amp;tid=<?php echo $_G['tid'];?><?php if(!empty($_G['gp_from'])) { ?>&amp;from=<?php echo $_G['gp_from'];?><?php } ?>" onclick="showWindow('tradeorder', this.href)">柜台商品管理</a>
<?php if($_G['uid'] == $_G['forum_thread']['authorid']) { if($_G['group']['allowposttrade']) { ?>
<a href="forum.php?mod=post&amp;action=reply&amp;fid=<?php echo $_G['fid'];?>&amp;tid=<?php echo $_G['tid'];?>&amp;firstpid=<?php echo $post['pid'];?>&amp;addtrade=yes<?php if(!empty($_G['gp_from'])) { ?>&amp;from=<?php echo $_G['gp_from'];?><?php } ?>">+ 添加商品</a>
<?php } ?>
...
<div class="xs1 cl"><?php if(is_array($trades)) foreach($trades as $key => $trade) { ?><div class="trdb">
<div id="trade<?php echo $trade['pid'];?>" class="trdl cl">
<div class="tt" onclick="display('trade<?php echo $trade['pid'];?>');ajaxget('forum.php?mod=viewthread&do=tradeinfo&tid=<?php echo $_G['tid'];?>&pid=<?php echo $trade['pid'];?>','tradeinfo<?php echo $trade['pid'];?>','tradeinfo<?php echo $trade['pid'];?>')">
...
<script type="text/javascript" onload="1">display('trade<?php echo $trade['pid'];?>');ajaxget('forum.php?mod=viewthread&do=tradeinfo&tid=<?php echo $_G['tid'];?>&pid=<?php echo $trade['pid'];?>','tradeinfo<?php echo $trade['pid'];?>','tradeinfo<?php echo $trade['pid'];?>')</script>
<?php } } ?>
</div>
<?php } ?>

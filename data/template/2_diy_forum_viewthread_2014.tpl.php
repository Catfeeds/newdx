...
<a href="forum.php?mod=post&amp;action=newthread&amp;fid=<?php echo $_G['fid'];?>" title="发新帖"  style="display: block; width:133px; height:43px;"></a>
<?php } } if($_G['uid']) { ?>
...
<a class="huf_button" href="javascript:;" onclick="showWindow(<?php if($_G['uid']) { ?>'reply','forum.php?mod=post&action=reply&fid=<?php echo $_G['fid'];?>&tid=<?php echo $_G['tid'];?>'<?php } else { ?>'login','member.php?mod=logging&action=login'<?php } ?>)" title="回复"></a>
<?php } ?>
...
<span class="zi" style="font-size:12px;">到</span><span class="zi" style="font-size:12px;">第</span><span class="lcsr" title="按回车即可跳转到指定楼层"><input type="text" onkeydown="if(event.keyCode==13) {window.location='forum.php?mod=redirect&ptid=<?php echo $_G['tid'];?>&authorid=<?php echo $_G['gp_authorid'];?>&postno='+this.value;return false;}" /></span>
...
<div class="layout">
<div class="glxm" id="modmenu"><?php $modopt=0; if($_G['forum']['ismoderator']) { if($_G['group']['allowdelpost']) { ?><?php $modopt++ ?><a href="javascript:;" onclick="modthreads(3, 'delete', <?php echo $_G['forum_thread']['typeid'];?>)">删除</a><?php } if($_G['group']['allowbumpthread'] && !$_G['forum_thread']['is_archived']) { ?><?php $modopt++ ?><a href="javascript:;" onclick="modthreads(3, 'down', <?php echo $_G['forum_thread']['typeid'];?>)">升降</a><?php } if($_G['group']['allowstickthread'] && ($_G['forum_thread']['displayorder'] <= 3 || $_G['adminid'] == 1) && !$_G['forum_thread']['is_archived']) { ?><?php $modopt++ ?><a href="javascript:;" onclick="modthreads(1, 'stick', <?php echo $_G['forum_thread']['typeid'];?>)">置顶</a><?php } if($_G['group']['allowhighlightthread'] && !$_G['forum_thread']['is_archived']) { ?><?php $modopt++ ?><a href="javascript:;" onclick="modthreads(1, 'highlight', <?php echo $_G['forum_thread']['typeid'];?>)">高亮</a><?php } if($_G['group']['allowdigestthread'] && !$_G['forum_thread']['is_archived']) { ?><?php $modopt++ ?><a href="javascript:;" onclick="modthreads(1, 'digest', <?php echo $_G['forum_thread']['typeid'];?>)">精华</a><?php } if($_G['group']['allowrecommendthread'] && !empty($_G['forum']['modrecommend']['open']) && $_G['forum']['modrecommend']['sort'] != 1 && !$_G['forum_thread']['is_archived']) { ?><?php $modopt++ ?><a href="javascript:;" onclick="modthreads(1, 'recommend', <?php echo $_G['forum_thread']['typeid'];?>)">推荐</a><?php } if($_G['group']['allowstampthread'] && !$_G['forum_thread']['is_archived']) { ?><?php $modopt++ ?><a href="javascript:;" onclick="modaction('stamp')">图章</a><?php } if($_G['group']['allowstamplist'] && !$_G['forum_thread']['is_archived']) { ?><?php $modopt++ ?><a href="javascript:;" onclick="modaction('stamplist')">图标</a><?php } if($_G['group']['allowclosethread'] && !$_G['forum_thread']['is_archived'] && $_G['forum']['status'] != 3) { ?><?php $modopt++ ?><a href="javascript:;" onclick="modthreads(4, '', <?php echo $_G['forum_thread']['typeid'];?>)"><?php if(!$_G['forum_thread']['closed']) { ?>关闭<?php } else { ?>打开<?php } ?></a><?php } if($_G['group']['allowmovethread'] && !$_G['forum_thread']['is_archived'] && $_G['forum']['status'] != 3) { ?><?php $modopt++ ?><a href="javascript:;" onclick="modthreads(2, 'move', <?php echo $_G['forum_thread']['typeid'];?>)">移动</a><?php } if($_G['group']['allowedittypethread'] && !$_G['forum_thread']['is_archived']) { ?><?php $modopt++ ?><a href="javascript:;" onclick="modthreads(2, 'type', <?php echo $_G['forum_thread']['typeid'];?>)">分类</a><?php } if(!$_G['forum_thread']['special'] && !$_G['forum_thread']['is_archived']) { if($_G['group']['allowcopythread'] && $_G['forum']['status'] != 3) { ?><?php $modopt++ ?><a href="javascript:;" onclick="modaction('copy')">复制</a><?php } if($_G['group']['allowmergethread'] && $_G['forum']['status'] != 3) { ?><?php $modopt++ ?><a href="javascript:;" onclick="modaction('merge')">合并</a><?php } if($_G['group']['allowrefund'] && $_G['forum_thread']['price'] > 0) { ?><?php $modopt++ ?><a href="javascript:;" onclick="modaction('refund')">撤销付费</a><?php } } if($_G['group']['allowsplitthread'] && !$_G['forum_thread']['is_archived'] && $_G['forum']['status'] != 3) { ?><?php $modopt++ ?><a href="javascript:;" onclick="modaction('split')">分割</a><?php } if($_G['group']['allowrepairthread'] && !$_G['forum_thread']['is_archived']) { ?><?php $modopt++ ?><a href="javascript:;" onclick="modaction('repair')">修复</a><?php } if($_G['forum_thread']['is_archived'] && $_G['adminid'] == 1) { ?><?php $modopt++ ?><a href="javascript:;" onclick="modaction('restore', '', 'archiveid=<?php echo $_G['forum_thread']['archiveid'];?>')">取消存档</a><?php } if($_G['forum_firstpid']) { if($_G['group']['allowwarnpost']) { ?><?php $modopt++ ?><a href="javascript:;" onclick="modaction('warn', '<?php echo $_G['forum_firstpid'];?>')">警告</a><?php } if($_G['group']['allowbanpost']) { ?><?php $modopt++ ?><a href="javascript:;" onclick="modaction('banpost', '<?php echo $_G['forum_firstpid'];?>')">屏蔽</a><?php } } if($_G['group']['allowremovereward'] && $_G['forum_thread']['special'] == 3 && !$_G['forum_thread']['is_archived']) { ?><?php $modopt++ ?><a href="javascript:;" onclick="modaction('removereward')">移除悬赏</a><?php } if($_G['forum']['status'] == 3 && in_array($_G['adminid'], array('1','2')) && $_G['forum_thread']['closed'] < 1) { ?><a href="javascript:;" onclick="modthreads(5, 'recommend_group', <?php echo $_G['forum_thread']['typeid'];?>);return false;">推到版块</a><?php } } if($allowblockrecommend) { ?><?php $modopt++ ?><a href="javascript:;" onclick="modaction('recommend', '<?php echo $_G['forum_firstpid'];?>', 'op=recommend&idtype=<?php if($_G['forum_thread']['isgroup']) { ?>gtid<?php } else { ?>tid<?php } ?>&id=<?php echo $_G['tid'];?>', 'portal.php?mod=portalcp&ac=portalblock')">推送</a><?php } if($allowpusharticle && $allowpostarticle) { ?><?php $modopt++ ?><a href="javascript:;" onclick="modaction('push', '<?php echo $_G['forum_firstpid'];?>', 'op=push&idtype=tid&id=<?php echo $_G['tid'];?>', 'portal.php?mod=portalcp&ac=index')">生成</a><?php } ?>
...
<?php if(!IS_ROBOT) { ?>
<a href="<?php echo $_G['siteurl'];?>forum.php?mod=viewthread&tid=<?php echo $_G['tid'];?><?php echo $fromuid;?>" title="复制本帖链接" onclick="return copyThreadUrl(this)" class="ftcpy">[复制链接]</a>
<?php } ?>
<span class="ch_r tzrht">查看：<?php echo $_G['forum_thread']['views'];?> | 回复：<?php echo $_G['forum_thread']['replies'];?>
</span>
</div>
</div>

<?php if(!defined('IN_DISCUZ')) exit('Access Denied'); hookscriptoutput('spacecp', 'common/header');?>
<?php if($_G['gp_pluginop'] == 'share') { include template('common/header'); if($_G['gp_sh_type'] == 1 || $_G['gp_sh_type'] == 3) { ?>
<h3 class="flb">
<?php if($_G['gp_sh_type'] == 1) { ?>
<em id="return_<?php echo $_G['gp_handlekey'];?>">分享到QQ空间</em>
<?php } elseif($_G['gp_sh_type'] == 3) { ?>
<em id="return_<?php echo $_G['gp_handlekey'];?>">分享到腾讯朋友</em>
<?php } if($_G['inajax']) { ?>
<span><a href="javascript:void(0);" onclick="hideWindow('<?php echo $_G['gp_handlekey'];?>');" class="flbc" title="关闭">关闭</a></span>
<?php } ?>
</h3>
<form name="connect_share_form" id="connect_share_form" method="post" action="<?php echo $_G['gp_share_url'];?>" onsubmit="connect_share_submit();return false;">
...
<?php if($_G['gp_share_images']) { ?>
<div class="c" style="padding: 5px 15px;">
<p class="cl">
分享图片:
</p>
<div id="share_picture_list"><?php if(is_array($_G['gp_share_images'])) foreach($_G['gp_share_images'] as $image) { ?><div id="box_<?php echo $image['aid'];?>" class="share_picture_box select">
...
<input type="hidden" name="attach_image" id="attach_image" value="<?php echo $_G['gp_attach_image'];?>" />
<?php } ?>
...
<input type="hidden" name="dialog_id" id="dialog_id" value="<?php echo $_G['gp_handlekey'];?>" />
...
<input type="hidden" name="sh_type" id="sh_type" value="<?php echo $_G['gp_sh_type'];?>" />
</form>
<script type="text/javascript" reload="1">
var connect_share_url = '<?php echo $_G['gp_share_url'];?>';
...
if (hasClass($('box_' + sel_id), 'select')) {
...
ajaxpost('connect_share_form', 'return_<?php echo $_G['gp_handlekey'];?>', null, null, null, null);
return false;
}
...
<h3 class="flb">
<em id="return_<?php echo $_G['gp_handlekey'];?>">转播到腾讯微博</em>
<?php if($_G['inajax']) { ?>
<span><a href="javascript:void(0);" onclick="hideWindow('<?php echo $_G['gp_handlekey'];?>');" class="flbc" title="关闭">关闭</a></span>
<?php } ?>
</h3>
...
<div id="share_picture_list"><?php if(is_array($_G['gp_share_images'])) foreach($_G['gp_share_images'] as $image) { ?><div id="box_<?php echo $image['aid'];?>" class="share_picture_box">
<div class="badge" id="<?php echo $image['aid'];?>" onclick="connect_share_picture_select(this.id);"></div>
<a href="javascript:void(0);"><img class="share_picute_img" src="<?php echo $image['thumb'];?>" id="connect_share_picture_<?php echo $image['aid'];?>" rel="<?php echo $image['big'];?>" /></a>
</div>
<?php } ?>
...
var connect_reason_default = threadInfo.subject.substr(0, 120) + '\n' + connect_thread_url;
...
<?php } include template('common/footer'); } elseif($_G['gp_pluginop'] == 'new') { include template('common/header'); ?><script type="text/javascript" reload="1">
...
<th></th>
<td>
<label for="ispublisht"><input type="checkbox" id="ispublisht" name="ispublisht" class="pc" value="1" <?php if($_G['member']['conispublisht']) { ?>checked="checked"<?php } ?> /> 发表主题时默认转播到腾讯微博</label>
</td>
</tr>
<tr>
<th></th>
...
<?php if($_G['connect']['is_oauth_user'] && !$_G['cookie']['client_token']) { ?>
<div id="unbind" class="ptm pbm" style="display:none;">
<p>您需要使用QQ帐号登录才能解除帐号绑定关系，请点击<a href="member.php?mod=logging&amp;action=logout&amp;formhash=<?php echo FORMHASH;?>" class="xi2">这里退出</a>后</p>
<p>重新使用QQ帐号登录</p>
</div>
<?php } elseif($_G['member']['conisregister']) { ?>

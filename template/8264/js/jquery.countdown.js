...
function Countdown() {
	...
	this._defaults = {
    sinceExpire: null, // added by guohaochong
...
$.extend(Countdown.prototype, {
	...
	_attachCountdown: function(target, options) {
		var $target = $(target);
    // _attachCountdown method can overide the existed countdown widgets modified by guohaochong
    ...
    var expired; 
    // modified by guohaochong
    if(inst._since){
      if(inst._sinceExpire)
        expired = (inst._now.getTime() >= inst._sinceExpire.getTime());
      else
        expired = (inst._now.getTime() <= inst._since.getTime()); 
    } else {
        expired = (inst._now.getTime() >= inst._until.getTime());
    }
	...
	_changeCountdown: function(target, options, value) {
		...
		if (inst) {
			...
			if ((inst._since && inst._since < now) ||
					...
					(inst._sinceExpire && inst._sinceExpire > now)) {
    ...
    // added by guohaochong
    inst._sinceExpire = this._get(inst,'sinceExpire');
	...
	_buildLayout: function(inst, layout, compact) {
      ...
      ds: (inst._periods[D]+1), // added by guohaochong

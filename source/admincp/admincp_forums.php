...
?>';
var rowtypedata = [
	[[1, ''], [1,'<input type="text" class="txt" name="newcatorder[]" value="0" />', 'td25'], [5, '<input name="newcat[]" value="<?php
		cplang('forums_admin_add_category_name', null, true);
?>" size="20" type="text" class="txt" />']],
	[[1, ''], [1,'<input type="text" class="txt" name="neworder[{1}][]" value="0" />', 'td25'], [5, '<div class="board"><input name="newforum[{1}][]" value="<?php
		cplang('forums_admin_add_forum_name', null, true);
?>" size="20" type="text" class="txt" /><select name="newinherited[{1}][]"><option value=""><?php
		cplang('forums_edit_newinherited', null, true);
?></option>' + forumselect + '</select></div>']],
	[[1, ''], [1,'<input type="text" class="txt" name="neworder[{1}][]" value="0" />', 'td25'], [5, '<div class="childboard"><input name="newforum[{1}][]" value="<?php
		cplang('forums_admin_add_forum_name', null, true);
?>" size="20" type="text" class="txt" />&nbsp;<label><input name="inherited[{1}][]" type="checkbox" class="checkbox" value="1">&nbsp;<?php
		cplang('forums_edit_inherited', null, true);
?></label></div>']],
];
</script>
<?
		...
		showsubtitle(array(
			...
			'',
			'forums_moderators',
			'<a href="javascript:;" onclick="if(getmultiids()) location.href=\'' . ADMINSCRIPT . '?action=forums&operation=edit&multi=\' + getmultiids();return false;">' . $lang['multiedit'] . '</a>'));
		$forumcount = DB::result_first("SELECT COUNT(*) FROM " . DB::table('forum_forum') . " WHERE `status`<>3");
		$query = DB::query("SELECT f.fid, f.type, f.status, f.name, f.fup, f.nfup, f.displayorder, f.ndisplayorder, f.inheritedmod, ff.moderators, ff.password, ff.redirect,ff.serverid_icon,ff.serverid_banner
		...
		if (is_array($_G['gp_norder'])) {
			foreach ($_G['gp_norder'] as $fid => $value) {
				DB::update('forum_forum', array(
					'name' => $_G['gp_name'][$fid],
					'ndisplayorder' => $_G['gp_norder'][$fid],
					), "fid='$fid'");
				$orderfid[] = $fid;
			}
		}
		...
		if (is_array($_G['gp_newcat'])) {
			foreach ($_G['gp_newcat'] as $key => $forumname) {
				...
				$fid = DB::insert('forum_forum', array(
					'type' => 'group',
					'name' => $forumname,
					'status' => 1,
					'displayorder' => $_G['gp_newcatorder'][$key]), 1);
				...
				$cararr = array(
					...
					'name' => $forumname,
					'level' => 1,
					'boardlevel1' => $forumname,
					'todayposts' => 0,
					'posts' => 0,
					'threads' => 0,
					'action' => 1,
					);
				addRecordByType('fmboardinfo', $cararr);
				//end
			}
		}
		...
		updatecache('forums');
		//start 修改版块的操作
		foreach ($orderfid as $ofid) {
			addrecordforuminfo($ofid, 3);
		}
		//end
		//start 新建版块的操作
		foreach ($fidss as $vfid) {
			addrecordforuminfo($vfid, 1);
		}
		//end
	...
	if($_G['gp_typeid'] > 0){
		...
		$typeid = $_G['gp_typeid'];
		$childtype = array();
		$q = DB::query("SELECT typeid, name FROM " . DB::table('forum_threadclass') . " WHERE fatherid = {$typeid}");
		while($v = DB::fetch($q)){
			$childtype[$v['typeid']] = $v['name'];
		}
		if (! submitcheck('modsubmit')) {
			$childselect = '';
			foreach($childtype as $_id => $_name) {
				$childselect .= ",&nbsp;<b>[<a target='_blank' href='" . ADMINSCRIPT ."?action=forums&operation=moderators&fid={$fid}&typeid={$_id}'>{$_name}</a>]</b>&nbsp;";
			}
			$typeinfo = DB::fetch_first("SELECT * FROM " . DB::table('forum_threadclass') . " WHERE typeid = {$typeid}");
			$fatherinfo = array();
			if($typeinfo['fatherid'] > 0) $fatherinfo = array('typeid' => $typeinfo['fatherid'], 'name' => DB::result_first("SELECT name FROM " . DB::table('forum_threadclass') . " WHERE typeid = {$typeinfo[fatherid]}"));
			shownav('forum', 'forums_moderators_edit');
			showsubmenu(cplang('forums_moderators_edit') . ' - <a href="' . ADMINSCRIPT . '?action=forums&operation=moderators&fid=' . $fid . '" >' . (DB::result_first("SELECT name FROM " . DB::table('forum_forum') . " WHERE fid = {$fid}")) . '</a> - ' . ($fatherinfo ? "<a target='_blank' href='" . ADMINSCRIPT . "?action=forums&operation=moderators&fid={$fid}&typeid={$fatherinfo[typeid]}'>{$fatherinfo[name]}</a> - " : "") . $typeinfo['name']);
			$childselect ? showsubmenu("下级版块<br /><br />" . str_replace(array('[', ']', ','), '', $childselect)) : "";
			showtips('forums_moderators_tips');
			showformheader("forums&operation=moderators&fid=$fid&typeid=$typeid&");
			...
			$moder_select = "<option value='0'>请选择版主</option>";
			while ($mod = DB::fetch($query)) {
				if($mod['onlytype'] == 1 && ! in_array($typeid, explode(',', $mod['typeids']))){
					$moder_select .= "<option value='{$mod[uid]}'>{$mod[username]}</option>";
					continue;
				}
				showtablerow('', array('class="td25"', 'class="td28"'), array(
					$mod['onlytype'] == 1 ? '<input type="checkbox" class="checkbox" name="deletetype[]" value="' . $mod['uid'] . '"' . ($mod['inherited'] ? ' disabled' : '') . ' />' : '<input type="checkbox" name="delete" value="" disabled title="请去上级版块修改" />',
					$mod['displayorder'],
					"<a href=\"" . ADMINSCRIPT . "?mod=forum&action=members&operation=group&uid=$mod[uid]\" target=\"_blank\">$mod[username]</a>",
					cplang(! $mod['onlytype'] ? 'yes' : 'no'),
					));
			}
			if(! $moder_select){
				showtablerow('',array('class="td25"','colspan="3"'), array(
					cplang('add_new'),'因上层版主均为继承下级，所以无需添加，请去上层版块设定版主不继承到下级分类'));
			}else{
				showtablerow('', array('class="td25"', 'class="td28"'), array(
					cplang('add_new'),
					'继承上级排序',
					'<select name="newmoderator">' . $moder_select . '</select>',
					''));
				showsubmit('modsubmit', 'submit', 'del', $childselect ? "&nbsp;&nbsp;<input class='checkbox' type='checkbox' name='todowntypes' value='1' checked /> 应用到下级版块{$childselect}。" : "");
			}
			showtablefooter();
			showformfooter();
		} else{
			if($_G['gp_newmoderator'] > 0) {
				$_nowtype = DB::result_first("SELECT typeids FROM " . DB::table('forum_moderator') . " WHERE uid = {$_G[gp_newmoderator]} AND fid = {$fid}");
				$_nowtype = ! empty($_nowtype) ? explode(',', $_nowtype) : array();
				if(! in_array($_G['gp_typeid'], $_nowtype)) {
					$_nowtype[] = $_G['gp_typeid'];
					if($_G['gp_todowntypes']){
						$_nowtype = array_merge($_nowtype, array_keys($childtype));
					}
					DB::update('forum_moderator', array('typeids' => implode(',', $_nowtype)), array('fid' => $fid, 'uid' => $_G['gp_newmoderator']));
				}
			}
			if($_G['gp_deletetype']) {
				$typeidarr = array($_G['gp_typeid']);
				if($_G['gp_todowntypes']){
					$typeidarr = array_merge($typeidarr, array_keys($childtype));
				}
				foreach($_G['gp_deletetype'] as $del_id) {
					$nowtype = DB::result_first("SELECT typeids FROM " . DB::table('forum_moderator') . " WHERE uid = {$del_id} AND fid = {$fid}");
					$new_typeids = array();
					$nowtype = ! empty($nowtype) ? explode(',', $nowtype) : array(); 
					foreach($nowtype as $_nid) {
						if(! in_array($_nid, $typeidarr)) {
							$new_typeids_arr[] = $_nid;
						}
					}
					$new_typeids = empty($new_typeids_arr) ? '' : implode(',', $new_typeids_arr);
					DB::update('forum_moderator', array('typeids' => $new_typeids), array('fid' => $fid, 'uid' => $del_id));					
				}
			}
			$updatedisplay = $_G['gp_todowntypes'] ? array_keys($childtype) : array();
			$updatedisplay[] = $typeid;
			updatetypeidmoderators($fid, $updatedisplay);
			cpmsg('forums_moderators_update_succeed', "mod=forum&action=forums&operation=moderators&fid=$fid&typeid=" . $_G['gp_typeid'], 'succeed');
		}
	}else{
		if (! submitcheck('modsubmit')) {
			$q = DB::query("SELECT name, typeid, fatherid FROM " . DB::table('forum_threadclass') . " WHERE fid = {$fid}");
			$subforummenu = '';
			while($v = DB::fetch($q)) {
				$alltypename[$v['typeid']] = $v['name'];
				if($v['fatherid'] == 0) $subforummenu .= "&nbsp;<b><a target='_blank' href='" . ADMINSCRIPT . "?action=forums&operation=moderators&fid={$fid}&typeid={$v[typeid]}'>{$v[name]}</a></b>&nbsp;";
			}
			$forum = DB::fetch_first("SELECT * FROM " . DB::table('forum_forum') . " WHERE fid='$fid'");
			shownav('forum', 'forums_moderators_edit');
			showsubmenu(cplang('forums_moderators_edit') . ' - ' . $forum['name']);
			showsubmenu("下级版块<br /><br />" . $subforummenu);
			showtips('forums_moderators_tips');
			...
			showsubtitle(array(
				...
				'usergroups',
				'forums_moderators_inherited',
				'forums_moderators_inherited_down',));
			...
			while ($mod = DB::fetch($query)) {
				$moder_type = '';
				if($mod['typeids']){
					$mod['typeids'] = explode(',', $mod['typeids']);
					foreach($mod['typeids'] as $_id){
						if($_id > 0) $moder_type .= "&nbsp;<b>[<a target='_blank' href='" . ADMINSCRIPT . "?action=forums&operation=moderators&fid={$fid}&typeid={$_id}'>{$alltypename[$_id]}</a>]</b>&nbsp;";
					}
				}
				showtablerow('', array('class="td25"', 'class="td28"'), array(
					...
					"<select name='onlytypenew[{$mod[uid]}]'><option value='0' " . ($mod['onlytype'] == 0 ? 'selected' : '') . " >是</option><option value='1' " . ($mod['onlytype'] == 1 ? 'selected' : '') . ">否</option></select>&nbsp;&nbsp;" . ($moder_type ? "已设置&nbsp;{$moder_type}" : ""),
					));
			}
			...
			if (! empty($_G['gp_delete']) || $_G['gp_newmoderator'] || (bool)$forum['inheritedmod'] != (bool)$inheritedmodnew) {
				...
				if ($_G['gp_newmoderator']) {
					...
					if (! $member) {
						...
						DB::update('common_member', array('groupid' => $_G['gp_newgroup'], ), "uid='$member[uid]' AND adminid NOT IN (1,2,3,4,5,6,7,8,-1)");
						DB::update('common_member', array('adminid' => '3', ), "uid='$member[uid]' AND adminid NOT IN (1,2)");
						DB::insert('forum_moderator', array(
							'uid' => $member['uid'],
							'fid' => $fid,
						...
						$adarr = array(
							...
							'name' => $forum['name'],
							'uid' => $member['uid'],
							'username' => $_G['gp_newmoderator']);
						addrecordadminchange($adarr, 1);
						//end
					}
				}
				...
				DB::update('forum_forum', array('inheritedmod' => $inheritedmodnew, ), "fid='$fid'");
			}
			if (is_array($_G['gp_displayordernew'])) {
				foreach ($_G['gp_displayordernew'] as $uid => $order) {
					DB::update('forum_moderator', array('displayorder' => $order, ), "fid='$fid' AND uid='$uid'");
				}
			}
			//*此处增加对版主是否继承给下级分类的设定的修改*//
			if (is_array($_G['gp_onlytypenew'])) {
				foreach ($_G['gp_onlytypenew'] as $uid => $onlytype) {
					DB::update('forum_moderator', $onlytype == 0 ? array('onlytype' => $onlytype, 'typeids' => '') : array('onlytype' => $onlytype, ), "fid='$fid' AND uid='$uid'");
				}
			}
			//*结束*//
			$moderators = $tab = '';
			$query = DB::query("SELECT m.username FROM " . DB::table('common_member') . " m, " . DB::table('forum_moderator') . " mo WHERE mo.fid='$fid' AND mo.onlytype='0' AND mo.inherited='0' AND m.uid=mo.uid ORDER BY mo.displayorder");
			while ($mod = DB::fetch($query)) {
			...
			DB::update('forum_forumfield', array('moderators' => $moderators, ), "fid='$fid'");
			$q = DB::query("SELECT typeid FROM " . DB::table('forum_threadclass') . " WHERE fid = {$fid}");
			$alltypeids = array();
			while($v = DB::fetch($q)) {
				$alltypeids[] = $v['typeid'];
			}
			updatetypeidmoderators($fid, $alltypeids);
	...
	if (! submitcheck('mergesubmit') || $source == $target) {
		...
		DB::update('forum_forumfield', array('threadtypes' => addslashes(serialize($targethreadtypes))), "fid='$target'");
		...
		while ($access = DB::fetch($query)) {
			DB::insert('forum_access', array(
				'uid' => $access['uid'],
				'fid' => $target,
				'allowview' => $access['allowview'],
				'allowpost' => $access['allowpost'],
				'allowreply' => $access['allowreply'],
				'allowgetattach' => $access['allowgetattach']), 0, 0, 1);
		}
	...
	if (! submitcheck('detailsubmit')) {
		$anchor = in_array($_G['gp_anchor'], array(
			'basic',
			'extend',
			'posts',
			'credits',
		...
		foreach ($_G['cache']['forums'] as $forums) {
			...
			if ($forums['type'] == 'group') {
				$sgid = $forums['fid'];
				$forumselect .= '</div><em class="cl">' . '<span class="right"><input name="checkall_' . $forums['fid'] . '" onclick="checkAll(\'value\', this.form, ' . $forums['fid'] . ', \'checkall_' . $forums['fid'] . '\')" type="checkbox" class="vmiddle checkbox" /></span>' . '<span class="pointer" onclick="sdisplay(\'g_' . $forums['fid'] . '\', this)"><img src="static/image/admincp/desc.gif" class="vmiddle" /></span> <span class="pointer" onclick="location.href=\'' . ADMINSCRIPT . '?action=forums&operation=edit&switch=yes&fid=' .
					$forums['fid'] . '\'">' . $forums['name'] . '</span></em><div id="g_' . $forums['fid'] . '" style="display:">';
		...
		$forumselect = '<span id="fselect" class="right popupmenu_dropmenu" onmouseover="showMenu({\'ctrlid\':this.id,\'pos\':\'34\'});$(\'fselect_menu\').style.top=(parseInt($(\'fselect_menu\').style.top)-scrollTopBody())+\'px\';$(\'fselect_menu\').style.left=(parseInt($(\'fselect_menu\').style.left)-document.documentElement.scrollLeft-20)+\'px\'">' . cplang('forums_edit_switch') . '<em>&nbsp;&nbsp;</em></span>' . '<div id="fselect_menu" class="popupmenu_popup" style="display:none"><div class="fsel"><div>' .
			$forumselect . '</div></div><div class="cl"><input type="button" class="btn right" onclick="$(\'menuform\').submit()" value="' . cplang('forums_multiedit') . '" /></div></div>';
		...
		if (count($mforum) == 1 && $mforum[0]['type'] == 'group') {
			...
			if ($multiset && ! in_array($anchor, array(
				'basic',
				'extend',
				'posts',
				'perm',
			...
			showsubmenuanchors(cplang('forums_edit') . (count($mforum) == 1 ? ' - ' . $mforum[0]['name'] . '(fid:' . $mforum[0]['fid'] . ')' : ''), array(
				...
				! $multiset ? array(array('menu' => 'usergroups_edit_other', 'submenu' => array(
							...
							array(
								...
								'threadsorts',
								$anchor == 'threadsorts'),
							! $pluginsetting ? array() : array(
								'forums_edit_plugin',
								'plugin',
								$anchor == 'plugin'),
							))) : array(),
				$multiset && $pluginsetting ? array(
					'forums_edit_plugin',
					'plugin',
					$anchor == 'plugin') : array(),
				), $forumselect);
		...
		if (count($mforum) == 1 && $mforum[0]['type'] == 'group') {
			...
			showsetting('forums_edit_basic_class', 'extranew[classname]', $mforum[0]['extra']['classname'], 'text');
			showsetting('forums_edit_base_backpicclass', 'extranew[backpicclass]', $mforum[0]['extra']['backpicclass'], 'text');
			showsetting('forums_edit_basic_displayorder_new', 'ndisplayorder', $mforum[0]['ndisplayorder'], 'text');
			...
			foreach ($mforum as $forum) {
				...
				if (! $multiset) {
					...
					$fupselect .= '</select>';
					/*新版上级版区选项*/
					$nfupselect = "<select name=\"nfupnew\">\n";
					if (is_array($fups)) {
						foreach ($fups as $forum1) {
							if ($forum1['type'] == 'group') {
								$selected = $forum1['fid'] == $forum['nfup'] ? "selected=\"selected\"" : null;
								$nfupselect .= "<option value=\"$forum1[fid]\" $selected>$forum1[name]</option>\n";
					...
					while ($type = DB::fetch($query)) {
						...
						if ($type['special']) {
							...
							$sortselect .= $showtype ? showtablerow('', array('class="td25"'), array(
								...
								"<input class=\"radio\" type=\"radio\" name=\"threadsortsnew[defaultshow]\" value=\"$type[typeid]\" " . ($forum['threadsorts']['defaultshow'] == $type['typeid'] ? 'checked' : '') . " />"), true) : '';
						}
					}
				...
				showsetting('forums_edit_basic_localname', 'extranew[localname]', $forum['extra']['localname'], 'text');
				showsetting('forums_edit_basic_class', 'extranew[classname]', $forum['extra']['classname'], 'text');				
				if (! $multiset) {
					if ($forum['icon']) {
				...
				if (! $multiset) {
					...
					showsetting('forums_edit_basic_up_new', '', '', $nfupselect);
					showsetting('forums_edit_basic_displayorder_new', 'ndisplayorder', $forum['ndisplayorder'], 'text');
				}
                ...
                showsetting('forums_edit_basic_description_seo', 'description_seo', str_replace('&amp;', '&', html2bbcode($forum['description_seo'])), 'textarea');
				...
				showsetting('forums_edit_extend_defaultorderfield', array('defaultorderfieldnew', array(
						...
						array(3, cplang('forums_edit_extend_order_views')))), $forum['defaultorderfield'], 'mradio');
				...
				showsetting('forums_edit_posts_modposts', array('modnewpostsnew', array(
						...
						array(2, cplang('forums_edit_posts_modposts_posts')))), $forum['modnewposts'], 'mradio');
				...
				showsetting('forums_edit_posts_allowpostspecial', array('allowpostspecialnew', array(
						...
						cplang('thread_debate'))), $forum['allowpostspecial'], 'binmcheckbox');
				$threadpluginarray = '';
				if (is_array($_G['setting']['threadplugins']))
					foreach ($_G['setting']['threadplugins'] as $tpid => $data) {
				...
				showsetting('forums_edit_posts_autoclose', array('autoclosenew', array(
						array(
							0,
							cplang('forums_edit_posts_autoclose_none'),
							array('autoclose_time' => 'none')),
						array(
							1,
							cplang('forums_edit_posts_autoclose_dateline'),
							array('autoclose_time' => '')),
						array(
							-1,
							cplang('forums_edit_posts_autoclose_lastpost'),
							array('autoclose_time' => '')))), $forum['autoclose'], 'mradio');
				...
				if (! $multiset) {
					...
					if (is_array($_G['setting']['extcredits'])) {
						foreach ($rules as $rid => $rule) {
							...
							$tdarr = array(
								$rule['rulename'],
								$rule['rid'] ? cplang('setting_credits_policy_cycletype_' . $rule['cycletype']) : 'N/A',
				...
				if ($allowthreadtypes && ! $multiset) {
					...
					$fatheridselect = str_replace('[SELECT_NAME]', 'newfatherid[]', get_fathertype_by_fid($fid));					
					echo <<< EOT
	<script type="text/JavaScript">
		var rowtypedata = [
			[
...
EOT;
					...
					showsetting('forums_edit_threadtypes_status', array(
						...
						array(array(
								1,
								cplang('yes'),
								array('threadtypes_config' => '', 'threadtypes_manage' => '')), array(
								0,
								cplang('no'),
								array('threadtypes_config' => 'none', 'threadtypes_manage' => 'none'))),
					...
					showsetting('forums_edit_threadtypes_prefix', array(
					...
					showsubtitle(array(
						'delete',
						'enable',
						'forums_edit_threadtypes_typeid',
						'display_order',
						'forums_edit_threadtypes_name',
						'forums_moderators',
						'forums_edit_threadtypes_fathertypeid',
						'forums_edit_threadtypes_icon'));
					echo $typeselect;
					...
					showsetting('forums_edit_threadsorts_status', array(
						...
						array(array(
								1,
								cplang('yes'),
								array('threadsorts_config' => '', 'threadsorts_manage' => '')), array(
								0,
								cplang('no'),
								array('threadsorts_config' => 'none', 'threadsorts_manage' => 'none'))),
...
?>
	...
	<?php
					echo $extcreditsbtn;
?>
	<a href="###" onclick="insertunit(' regdate ')">&nbsp;<?php
					echo cplang('forums_edit_perm_formula_regdate')
?>&nbsp;</a>&nbsp;
	<a href="###" onclick="insertunit(' regday ')">&nbsp;<?php
					echo cplang('forums_edit_perm_formula_regday')
?>&nbsp;</a>&nbsp;
	<a href="###" onclick="insertunit(' regip ')">&nbsp;<?php
					echo cplang('forums_edit_perm_formula_regip')
?>&nbsp;</a>&nbsp;
	<a href="###" onclick="insertunit(' lastip ')">&nbsp;<?php
					echo cplang('forums_edit_perm_formula_lastip')
?>&nbsp;</a>&nbsp;
	<a href="###" onclick="insertunit(' buyercredit ')">&nbsp;<?php
					echo cplang('forums_edit_perm_formula_buyercredit')
?>&nbsp;</a>&nbsp;
	<a href="###" onclick="insertunit(' sellercredit ')">&nbsp;<?php
					echo cplang('forums_edit_perm_formula_sellercredit')
?>&nbsp;</a>&nbsp;
	<a href="###" onclick="insertunit(' digestposts ')"><?php
					echo cplang('forums_edit_perm_formula_digestposts')
?></a>&nbsp;
	<a href="###" onclick="insertunit(' posts ')"><?php
					echo cplang('forums_edit_perm_formula_posts')
?></a>&nbsp;
	<a href="###" onclick="insertunit(' threads ')"><?php
					echo cplang('forums_edit_perm_formula_threads')
?></a>&nbsp;
	<a href="###" onclick="insertunit(' oltime ')"><?php
					echo cplang('forums_edit_perm_formula_oltime')
?></a>&nbsp;
	...
	<a href="###" onclick="insertunit(' and ')">&nbsp;<?php
					echo cplang('forums_edit_perm_formula_and')
?>&nbsp;</a>&nbsp;
	<a href="###" onclick="insertunit(' or ')">&nbsp;<?php
					echo cplang('forums_edit_perm_formula_or')
?>&nbsp;</a>&nbsp;<br />
	<?php
					echo $profilefields;
?>
	<div id="formulapermexp" class="margintop marginbot diffcolor2"><?php
					echo $formulapermexp
?></div>
	</div>
	<textarea name="formulapermnew" id="formulapermnew" class="marginbot" style="width:80%" rows="3" onkeyup="formulaexp()"><?php
					echo dhtmlspecialchars($forum['formulaperm'])
?></textarea>
...
?></span>
	...
	<?php
		...
		foreach ($_G['gp_multinew'] as $k => $row) {
			...
			if (! $multiset) {
				...
				$formulapermary[1] = preg_replace(array("/(digestposts|posts|threads|oltime|extcredits[1-8])/", "/(regdate|regday|regip|lastip|buyercredit|sellercredit|field\d+)/"), array("getuserprofile('\\1')", "\$memberformula['\\1']"), $_G['gp_formulapermnew']);
			...
			if ($_G['gp_type'] == 'group') {
				if ($_G['gp_namenew']) {
					...
					if (! empty($_G['setting']['domain']['root']['forum'])) {
						...
						if (! empty($domain)) {
							...
							DB::insert('common_domain', array(
								'domain' => $domain,
								'domainroot' => $_G['setting']['domain']['root']['forum'],
								'id' => $fid,
								'idtype' => 'subarea'));
						}
					}
					DB::update('forum_forum', array(
						'name' => $_G['gp_namenew'],
						'ndisplayorder' => $_G['gp_ndisplayorder'],
					...
					DB::update('forum_forumfield', array('extra' => $extranew, ), "fid='$fid'");
					updatecache('forums');
				...
				/*存储新版块的上级版块修改*/
				if ($_G['gp_nfupnew'] != $forum['nfup'] && ! $multiset) {
					DB::update('forum_forum', array('nfup' => $_G['gp_nfupnew']), array('fid' => $fid), true);
				}
				if ($_G['gp_ndisplayorder'] != $forum['ndisplayorder'] && ! $multiset) {
					DB::update('forum_forum', array('ndisplayorder' => $_G['gp_ndisplayorder']), array('fid' => $fid), true);
				}
				...
				if (! empty($_G['setting']['domain']['root']['forum'])) {
					...
					if (! empty($domain)) {
						...
						DB::insert('common_domain', array(
							'domain' => $domain,
							'domainroot' => addslashes($_G['setting']['domain']['root']['forum']),
							'id' => $fid,
							'idtype' => 'forum'));
					}
				}
					...
					if ($allowthreadtypes) {
							...
							if ($newname) {
								foreach ($newname as $key => $val) {
									...
									if ($_G['gp_newenable'][$key] && $val) {
										$newtypeid = DB::fetch_first("SELECT typeid, fatherid FROM " . DB::table('forum_threadclass') . " WHERE fid='$fid' AND name='$val'");
										if (! $newtypeid) {
											$threadtypes_newdisplayorder = intval($_G['gp_newdisplayorder'][$key]);
											$threadtypes_newicon = trim($_G['gp_newicon'][$key]);
											$threadtypes_newfatherid = trim($_G['gp_newfatherid'][$key]);
											$newtypeid = DB::insert('forum_threadclass', array(
												'fid' => $fid,
												'name' => $val,
												'displayorder' => $threadtypes_newdisplayorder,
												'fatherid' => $threadtypes_newfatherid,
												'icon' => $threadtypes_newicon), 1);
										}
										...
										$threadtypesnew['options']['fatherid'][$newtypeid] = $threadtypes_newfatherid;
						...
						if ($threadtypesnew['status']) {
							if (is_array($threadtypesnew['options']) && $threadtypesnew['options']) {
								...
								while ($type = DB::fetch($query)) {
									if ($threadtypesnew['options']['name'][$type['typeid']] != $type['name'] || $threadtypesnew['options']['displayorder'][$type['typeid']] != $type['displayorder'] || $threadtypesnew['options']['icon'][$type['typeid']] != $type['icon'] || $threadtypesnew['options']['fatherid'][$type['typeid']] != $type['fatherid']) {
										DB::update('forum_threadclass', array(
											...
											'fatherid' => $threadtypesnew['options']['fatherid'][$type['typeid']],
											), "typeid='{$type['typeid']}'");
									}
								}
						...
						if ($threadtypesnew && $typeids) {
							...
							while ($type = DB::fetch($query)) {
								...
								$threadtypesnew['fatherid'][$type['typeid']] = trim($threadtypesnew['options']['fatherid'][$type['typeid']]);
							}
							$threadtypesnew = $threadtypesnew['types'] ? addslashes(serialize(array(
								...
								'fatherid' => $threadtypesnew['fatherid'],
								))) : '';
						}
                ...
                $description_seo = addslashes(preg_replace('/on(mousewheel|mouseover|click|load|onload|submit|focus|blur)="[^"]*"/i', '', discuzcode(dstripslashes($_G['gp_description_seo']), 1, 0, 0, 0, 1, 1, 0, 0, 1)));
				...
				$forum['extra']['localname'] = $extranew['localname'];
				$forum['extra']['classname'] = $extranew['classname'];
				if (! $multiset) {
					if ($_FILES['bannernew']) {
						$return_banner = upload_icon_banner($forum, $_FILES['bannernew'], 'banner');
						$bannernew = $return_banner['attachment'];
					} else {
						$bannernew = $_G['gp_bannernew'];
					}
					if ($bannernew) {
						$forumfielddata['banner'] = $bannernew;
						$forumfielddata['serverid_banner'] = $return_banner['serverid'];
					}
	...
	if ($_G['gp_ajax']) {
		...
		//start 删除版区操作
		$info = DB::fetch_first("SELECT fid,name,todayposts,threads,posts,lastpost FROM " . DB::table('forum_forum') . " WHERE fup=0 and type='group' and fid = {$fid}");
		if ($info) {
			$uparr = array(
				'fid' => $info['fid'],
				'name' => $info['name'],
				'level' => 1,
				'boardlevel1' => $info['name'],
				'todayposts' => $info['todayposts'],
				'posts' => $info['posts'],
				'threads' => $info['threads'],
				'action' => 2);
			addRecordByType('fmboardinfo', $uparr);
		}
		//end
		deletedomain($fid, 'forum');
		deletedomain($fid, 'subarea');
		...
		if (! $_G['gp_confirmed']) {
			...
			$threads = DB::result_first("SELECT COUNT(*) FROM " . DB::table('forum_thread') . " WHERE fid='$fid' " . getSlaveID());
...
function showforum(&$forum, $type = '', $last = '', $toggle = false)
{
	...
	if ($last == '') {
		$return = '<tr class="hover">' . '<td class="td25"' . ($type == 'group' ? ' onclick="toggle_group(\'group_' . $forum['fid'] . '\', $(\'a_group_' . $forum['fid'] . '\'))"' : '') . '>' . ($type == 'group' ? '<a href="javascript:;" id="a_group_' . $forum['fid'] . '">' . ($toggle ? '[+]' : '[-]') . '</a>' : '') . '</td>
			<td class="td25"><input type="text" class="txt" name="order[' . $forum['fid'] . ']" value="' . $forum['displayorder'] . '" />
			</td><td class="td25"><input type="text" class="txt" name="norder[' . $forum['fid'] . ']" value="' . $forum['ndisplayorder'] . '" /></td><td>';
		if ($type == 'group') {
...
{
	$q = DB::query("SELECT fm.*, cm.username FROM " . DB::table('forum_moderator') . " fm LEFT JOIN " . DB::table('common_member') . " cm ON fm.uid = cm.uid WHERE fid = {$fid} ORDER BY displayorder");
	while($v = DB::fetch($q)){
		$moderators[$v['uid']] = $v;
	}
	...
	$q = DB::query("SELECT * FROM " . DB::table('forum_threadclass') . " WHERE fid='$fid' AND fatherid > 0 ORDER BY displayorder");
	while($v = DB::fetch($q)){
		$enablechecked = '';
		$typeselected = array();
		...
		$_tmpselect = showtablerow('', array('class="td25"'), array(
			"<input type=\"checkbox\" class=\"checkbox\" name=\"threadtypesnew[options][delete][]\" value=\"{$v['typeid']}\" />",
			'<input type="checkbox" name="threadtypesnew[options][enable][' . $v['typeid'] . ']" value="1" class="checkbox"' . $enablechecked . ' />',
			$v['typeid'],
			"<input type=\"text\" size=\"2\" name=\"threadtypesnew[options][displayorder][{$v['typeid']}]\" value=\"{$v['displayorder']}\" />",
			"<span>└──&nbsp;&nbsp;</span><input type=\"text\" name=\"threadtypesnew[options][name][{$v['typeid']}]\" value=\"{$v['name']}\" />",
			showforum_moderators_typeid($moderators, $fid, $v['typeid']),
			str_replace('[SELECT_NAME]', "threadtypesnew[options][fatherid][{$v['typeid']}]", get_fathertype_by_fid($fid, $v['fatherid'])),
			"<input type=\"text\" name=\"threadtypesnew[options][icon][{$v['typeid']}]\" value=\"{$v['icon']}\" />",
			), true);
		$childrentypes[$v['fatherid']] .= $_tmpselect;
	}
	$query = DB::query("SELECT * FROM " . DB::table('forum_threadclass') . " WHERE fid='$fid' AND fatherid = 0 ORDER BY displayorder");
	while ($type = DB::fetch($query)) {
		$enablechecked = '';
		$typeselected = array();
		...
		$typeselect .= showtablerow('', array('class="td25"'), array(
			...
			showforum_moderators_typeid($moderators, $fid, $type['typeid']),
			str_replace('[SELECT_NAME]', "threadtypesnew[options][fatherid][{$type['typeid']}]", get_fathertype_by_fid($fid, $type['fatherid'])),
			"<input type=\"text\" name=\"threadtypesnew[options][icon][{$type['typeid']}]\" value=\"{$type['icon']}\" />",
			), true);
		$typeselect .= $childrentypes[$type['typeid']];
	}
	return $typeselect;
}
function showforum_moderators_typeid($moderators_arr, $fid, $typeid)
{
	if ($moderators_arr) {
		$moderators = array_keys($moderators_arr);
		$count = count($moderators);
		$max = $count > 2 ? 2 : $count;
		$mods = array();
		$i = 0;
		while($i < $count && $max > 0) {
			if($moderators_arr[$moderators[$i]]['onlytype'] != 1 || in_array($typeid, explode(',', $moderators_arr[$moderators[$i]]['typeids']))){
				$mods[] = $moderators_arr[$moderators[$i]]['inheritedmod'] ? '<b>' . $moderators_arr[$moderators[$i]]['username'] . '</b>' : $moderators_arr[$moderators[$i]]['username'];
				$max--;
			}
			$i++;
		}
		$r = implode(', ', $mods);
		if ($count > 2) {
			$r = '<span onmouseover="showMenu({\'ctrlid\':this.id})" id="mods_' . $fid . '_' . $typeid . '">' . $r . '</span>';
			$mods = array();
			foreach ($moderators as $moderator) {
				if($moderators_arr[$moderator]['onlytype'] == 1 && !in_array($typeid, explode(',', $moderators_arr[$moderator]['typeids']))) continue;
				$mods[] = $moderators_arr[$moderator]['inheritedmod'] ? '<b>' . $moderators_arr[$moderator]['username'] . '</b>' : $moderators_arr[$moderator]['username'];
			}
			$r = '<a href="' . ADMINSCRIPT . '?action=forums&operation=moderators&fid=' . $fid . '&typeid=' . $typeid . '" title="' . cplang('forums_moderators_comment') . '">' . $r . '</a> ...';
			$r .= '<div class="dropmenu1" id="mods_' . $fid . '_' . $typeid . '_menu" style="display: none">' . implode('<br />', $mods) . '</div>';
		} else {
			$r = '<a href="' . ADMINSCRIPT . '?action=forums&operation=moderators&fid=' . $fid . '&typeid=' . $typeid . '" title="' . cplang('forums_moderators_comment') . '">' . $r . '</a>';
		}
	} else {
		$r = '<a href="' . ADMINSCRIPT . '?action=forums&operation=moderators&fid=' . $fid . '&typeid=' . $typeid . '" title="' . cplang('forums_moderators_comment') . '">' . cplang('forums_admin_no_moderator') . '</a>';
	}
	return $r;
}
function get_fathertype_by_fid($fid, $fatherid = 0){
	$q = DB::query("SELECT * FROM " . DB::table('forum_threadclass') . " WHERE fid='$fid' AND fatherid = 0 ORDER BY displayorder");
	$typeselect = '<select name="[SELECT_NAME]"><option value="0">无上级分类</option>';
	while($v = DB::fetch($q)){
		$typeselect .= '<option value="' . $v['typeid'] . '" ' . ($v['typeid'] == $fatherid ? 'selected' : '') . '>' . $v['name'] . '</option>';
	}
	$typeselect .= '</select>';
	return $typeselect;
}
	...
	if ($threadtypes) {
		...
		foreach ($threadtypes['types'] as $key => $val) {
			$data = array(
				...
				'name' => addslashes($val),
				'displayorder' => $i++,
				'icon' => addslashes($threadtypes['icons'][$key]));
...
function updatetypeidmoderators($fid, $updatedisplay = array()){
	$q = DB::query("SELECT m.username, mo.typeids FROM " . DB::table('common_member') . " m, " . DB::table('forum_moderator') . " mo WHERE mo.fid = {$fid} AND mo.onlytype = '1' AND mo.inherited = '0' AND m.uid = mo.uid ORDER BY mo.displayorder");
	$all_moderator = $tab = '';
	$type_arr = array();
	while($v = DB::fetch($q)) {
		if(!empty($v['typeids'])) {
			foreach(explode(',', $v['typeids']) as $_typeid) {
				if($_typeid > 0) {
					$moderators_type[$_typeid] .= "\t" . $v['username'];
				}
			}
		}
	}
	foreach($updatedisplay as $_tyid) {
		DB::update('forum_threadclass', array('moderator_type' => ltrim($moderators_type[$_tyid], "\t")), array('typeid' => $_tyid));
	}
}
?>
...
<input type="button" class="btn" name="searchsubmit" value="'.cplang('search').'" onclick="if(this.form.title.value==\''.cplang('adv_inputtitle').'\'){this.form.title.value=\'\'}location.href=\''.ADMINSCRIPT.'?action=adv&operation=ad'.($type ? '&type='.rawurlencode($type) : '').'&title=\'+this.form.title.value+\'&starttime=\'+this.form.starttime.value+\'&endtime=\'+this.form.endtime.value+\'&orderby=\'+this.form.orderby.value;"> &nbsp;
	...
	if(!submitcheck('advsubmit')) {
		...
		if($operation == 'edit') {
			...
			$adv['parameters'] = unserialize($adv['parameters']);
            /*将原先的前端广告地址替换为新地址，再提交后就会变为正确*/
            $adv['parameters']['url'] = str_replace(array('http://bbs.8264.com/data/attachment/','/data/attachment/','data/attachment/'),$_G['config']['web']['attach'],$adv['parameters']['url']);
            $adv['parameters']['html'] = str_replace(array('http://bbs.8264.com/data/attachment/','/data/attachment/','data/attachment/'),$_G['config']['web']['attach'],$adv['parameters']['html']);
			$type = $adv['type'];
		} else {
		...
		if($advnew['style'] == 'image' || $advnew['style'] == 'flash') {
			if($_FILES['advnew'.$advnew['style']]) {
				...
				if($upload->init($_FILES['advnew'.$advnew['style']], 'common') && $upload->save('adv_file')) {
				    if(isset($upload->attach['serverid']) && $upload->attach['serverid']>0){
				        require_once DISCUZ_ROOT."./source/plugin/attachment_server/attachment_server.class.php";
                        $attachserver = new attachserver;
                        $domain = "http://".$attachserver->get_server_domain($upload->attach['serverid'])."/common/";
				    }else{
				        $domain = $_G['setting']['attachurl'].'common/';
				    }
					$advnew[$advnew['style']]['url'] = $domain.$upload->attach['attachment'];
				}
			} else {
...
function encodeadvcode($advnew) {
	switch($advnew['style']) {
		case 'code':
			...
			if(strpos($advnew['code'], '.8264.com') === false && strpos($advnew['code'], 'nofollow') === false) {
				$advnew['code'] = str_replace('href=', 'rel="nofollow" href=', $advnew['code']);
			}
			break;
		case 'text':
			$advnew['code'] = '<a href="'.$advnew['text']['link'].'" target="_blank" '.(strpos($advnew['text']['link'], '.8264.com' ) === false ? 'rel="nofollow" ' : '').($advnew['text']['size'] ? 'style="font-size: '.$advnew['text']['size'].'"' : '').'>'.$advnew['text']['title'].'</a>';
			break;
		case 'image':
			$advnew['code'] = '<a href="'.$advnew['image']['link'].'" target="_blank" '.(strpos($advnew['image']['link'], '.8264.com' ) === false ? 'rel="nofollow"' : '').'><img src="'.$advnew['image']['url'].'"'.($advnew['image']['height'] ? ' height="'.$advnew['image']['height'].'"' : '').($advnew['image']['width'] ? ' width="'.$advnew['image']['width'].'"' : '').($advnew['image']['alt'] ? ' alt="'.$advnew['image']['alt'].'"' : '').' border="0"></a>';
			break;
		case 'flash':

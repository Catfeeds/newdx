 ...
 *
 *      $Id: admincp_group.php 18356 2010-11-22 01:44:46Z liulanbo $
 */
...
if($operation == 'setting') {
	...
	if(!submitcheck('updategroupsetting')) {
		...
		if($recommends) {
			$query = DB::query("SELECT f.fid, f.name, ff.description, ff.icon,ff.serverid_icon FROM ".DB::table('forum_forum')." f LEFT JOIN ".DB::table('forum_forumfield')." ff USING(fid) WHERE f.fid IN (".dimplode($recommends).") AND f.status='3' AND f.type='sub'");
			while($row = DB::fetch($query)) {
				$row['icon'] = get_groupimg($row['icon'], 'icon',$row['serverid_icon']);
				$temp[$row[fid]] = $row;
			}
		...
		if(count($group_recommend) < $recommend_num) {
			$query = DB::query("SELECT f.fid, f.name, ff.description, ff.icon,ff.serverid_icon FROM ".DB::table('forum_forum')." f LEFT JOIN ".DB::table('forum_forumfield')." ff USING(fid) WHERE f.status='3' AND f.type='sub'
...
<?
	...
	require_once libfile('function/group');
	require_once libfile('function/discuzcode');
	$groupicon = get_groupimg($group['icon'], 'icon',$group['serverid_icon']);
	$groupbanner = get_groupimg($group['banner'],'',$group['serverid_banner']);
	...
	if(!submitcheck('editsubmit')) {
		...
		$icondata = array();
        $return_icon = upload_icon_banner($group, $_FILES['iconnew'], 'icon');
		$iconnew = $return_icon['attachment'];
        $return_banner = upload_icon_banner($group, $_FILES['bannernew'], 'banner');
		$bannernew = $return_banner['attachment'];
		if($iconnew) {
			$icondata['icon'] = $iconnew;
            $icondata['serverid_icon'] = $return_icon['serverid'];
		}
		if($bannernew) {
			$icondata['banner'] = $bannernew;
            $icondata['serverid_banner'] = $return_banner['serverid'];
		};

		if($_G['gp_deleteicon']) {
			@unlink($_G['setting']['attachurl'].'group/'.$group['icon']);
			$icondata['icon'] = '';
            $icondata['serverid_icon'] = 0;
		}
		if($_G['gp_deletebanner']) {
			@unlink($_G['setting']['attachurl'].'group/'.$group['banner']);
			$icondata['banner'] = '';
            $icondata['serverid_banner'] = 0;
		}

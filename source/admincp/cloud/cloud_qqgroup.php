 ...
 *
 *      $Id: cloud_qqgroup.php 23017 2011-06-14 05:34:37Z yexinhao $
 */
	...
	if(submitcheck('setMiniportalThreadsSubmit')) {
		...
		if (submitcheck('uploadImageSubmit')) {
			...
			if($uploadImage = QQGroupUpload($tid)) {
				echo '<div id="upload_msg_success">'.cplang('qqgroup_msg_upload_succeed').'</div><div id="upload_msg_imgpath" style="display:none;">'.$uploadImage['thumbTarget'].'</div><div id="upload_msg_imgurl" style="display:none;">'.$uploadImage['serverdomain'].$uploadImage['thumbTarget'].'</div>';
			} else {
...
function getTopicThread() {
	...
	$posttableid = intval($_G['thread']['posttableid']);
	...
	if(is_exists($_G['config']['web']['attach'].$thumbTarget,250)) {
		$imagePath = $thumbTarget;
		$imageUrl = $_G['config']['web']['attach'].$imagePath;
	} else {
		$attachment = DB::fetch_first("SELECT attachment,serverid FROM ".DB::table('forum_attachment')." WHERE pid='$pid' AND (isimage='1' OR isimage='-1') AND remote='0'");
		if($attachment['attachment']) {
			$imagePath = 'forum/'.$attachment['attachment'];
			$serverid = $attachment['serverid'];
			$imageUrl = ($serverid > 0 ? "http://".$alldomain[$serverid]."/" : $_G['setting']['attachurl']).$imagePath;
		}
	}
...
function QQGroupUpload($tid) {
    ...
    $alldomain = $attachserver->get_server_domain('all','*');
	if($serverid > 0){
        $imgwidth = $attach['width'];
        if($imgwidth < 75){
            $attachserver->delete($alldomain[$serverid]['domain'] , $alldomain[$serverid]['api'] , 'temp/'.$attach['attachment'] , 1);
            return false;
        }
        $thumb_return = $attachserver->Thumb($alldomain[$serverid]['domain'] , $alldomain[$serverid]['api'] , 'temp/'.$attach['attachment'] , $thumbTarget , 75 , 75 , 1);
        $attachserver->delete($alldomain[$serverid]['domain'] , $alldomain[$serverid]['api'] , 'temp/'.$attach['attachment'] , 1);
        if(!$thumb_return) return false;
        $attach['serverdomain'] = "http://".$alldomain[$serverid]['name']."/";
	}else{
    	...
    	@unlink($attach['target']);
        $attach['serverdomain'] = "/data/attachment/";
	}
...
function QQGroupLoadThread() {
	global $_G;

	$cachelist = array('threadtableids', 'threadtable_info');
	loadcache($cachelist);
	...
	$_G['thread'] = DB::fetch_first("SELECT * FROM ".DB::table($threadtable)." t WHERE tid='{$_G['tid']}' AND displayorder>='0'");

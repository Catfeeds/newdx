...
if($operation == 'app') {
	...
	if(!submitcheck('submit')) {
		...
		foreach($appkeyarr as $key => $desc) {
			showtablerow('', array('class="td25"', ''), array(
					$desc,
					"<input title='当前设置已移至config文件内，此处无法编辑' readonly type=\"text\" class=\"txt\" style=\"width:50%;\" name=\"appnew[$key]\" value=\"".$_G['setting']['domain']['app'][$key]."\">"
				));
		}
		showsubmit('submit');
		echo "<P><b><font color='red'>当前配置在config中修改，此处仅为显示无法修改</font></b></p>";
		showtablefooter();
		showformfooter();
	} else {
	...
	if(!submitcheck('submit')) {
		...
		foreach($roottype as $type => $desc) {
			...
			showtablerow('', array('class="td25"', ''), array(
					$desc,
					"<input title='当前设置已移至config文件内，此处无法编辑' readonly type=\"text\" class=\"txt\" style=\"width:50%;\" name=\"domainnew[$type]\" value=\"$domainroot\">"
				));
		}
		showsubmit('submit');
		echo "<P><b><font color='red'>当前配置在config中修改，此处仅为显示无法修改</font></b></p>";

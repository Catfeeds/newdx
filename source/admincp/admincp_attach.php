		...
		foreach($attachmentarray as $attachment) {
			if(!$attachment['remote']) {
                if($attachment['serverid']>0){
                    @set_time_limit(0);
    				if(@fclose(@fopen("http://".$all_server[$attachment['serverid']]."/forum/".$attachment['attachment'], 'r'))) {
    					$matched = '';
    				} else {
    					$matched = cplang('attach_lost');
    				}
    				$attachment['url'] = "http://".$all_server[$attachment['serverid']]."/forum/";
                }else{
			...
			if(!$_G['gp_nomatched'] || ($_G['gp_nomatched'] && $matched)) {
				...
				$attachments .= showtablerow('', array('class="td25"', 'title="'.$attachment['description'].'" class="td21"'), array(
					...
					($attachment['remote']||$attachment['serverid']>0) ? "<span class=\"diffcolor3\">$attachment[filename]" : $attachment['filename'],
...
EOT;
	...
	if($ids = dimplode($_G['gp_delete'])) {
		...
		$tids = $pids = 0;
		$query = DB::query("SELECT tid, pid, attachment, thumb, remote, aid, serverid FROM ".DB::table('forum_attachment')." WHERE aid IN ($ids)");

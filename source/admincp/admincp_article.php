 ...
 *
 *      $Id: admincp_article.php 22550 2011-05-12 05:21:39Z monkey $
 */
...
if($operation == 'tag') {
		...
		if($_POST['optype'] == 'recover') {
			...
			if($catids) {
				foreach($catids as $catid) {
					$cnt = DB::result_first('SELECT COUNT(*) FROM '.DB::table('portal_article_title')." WHERE catid = '$catid' " . getSlaveID());
	...
	if(submitcheck('articlesubmit')) {
		...
		if(!empty($_GET['catid'])) {
			...
			$catids[] = $_GET['catid'];
		    $wherearr[] = 'at.catid IN ('.dimplode($catids).')';
		}else{
			$wherearr[] = 'at.catid != 910';//除非特意筛选否则不展示榜单分类下的文章
		}
		if(!empty($_GET['tag'])) {
		...
		'article_dateline','article_pubtime','skipall','skipurl','skipcon','article_id', 'article_title', 'article_uid', 'article_username', 'article_category', 'article_tag');
		foreach ($keys as $key) {
				...
				<table cellspacing="3" cellpadding="3">
					...
					<tr>
						...
						<td colspan="3">
							...
							<select name="skip">
							<option value="" $skip[0]>$searchlang[skipall]</option>
							<option value="1" $skip[1] >$searchlang[skipurl]</option>
							<option value="2" $skip[2]>$searchlang[skipcon]</option>
							</select>
...
SEARCH;
		...
		showsubtitle(array('', 'article_title', 'article_category', 'article_username', 'article_clicktimes','UV','article_dateline','article_pubtime', '最后修改', 'operation'));

		$multipage = '';
		$count = DB::result(DB::query("SELECT COUNT(*) FROM ".DB::table('portal_article_title')." as at WHERE $wheresql"), 0);
		if($count) {
			//此处代码做了修改
			$wheresql2 = preg_replace('/(\w+?)\s*(=|like|<|>|<=|>=)/', 'at.\1\2', $wheresql);
			$query = DB::query("SELECT at.*,ac.viewnum,ac.uvnum FROM ".DB::table('portal_article_title')." at LEFT JOIN ".DB::table('portal_article_count')." ac ON at.aid=ac.aid WHERE $wheresql2 $ordersql LIMIT $start,$perpage");
			while ($value = DB::fetch($query)) {
				...
				showtablerow('', array('class="td25"', 'width="480"', 'class="td28"'), array(
						...
						"<a href=\"".ADMINSCRIPT."?action=article&uid=$value[uid]&skip=$_GET[skip]\">$value[username]</a>",
						$value[viewnum],//此处为多添加的一个字段
						$value[uvnum],//用户唯一浏览数
						dgmdate($value[dateline]),
						$value['pubtime']?dgmdate($value[pubtime]):0,
						"<a href=\"http://u.8264.com/space-uid-{$value[lastuid]}.html\" target=\"_blank\" title=\"修改时间：".dgmdate($value[lasttime])." IP:{$value[ip]}\">$value[lastname]</a>",

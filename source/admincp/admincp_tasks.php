...
if(!($operation)) {
	...
	if(!submitcheck('tasksubmit')) {
		...
		showsubmenu('nav_tasks', array(
			...
			array('nav_task_manager', 'tasks&operation=manager', 0)
		));
	...
	if(!submitcheck('addsubmit')) {
		...
		showsetting('tasks_need_checkit', array('checkit',array(array('1','需要审核'),array('0','不需审核',0))), 0, 'mradio');
		showtagfooter('tbody');
		...
		if(is_array($task_conditions)) {
			foreach($task_conditions as $taskvarkey => $taskvars) {
				if($taskvars['title']) {
					...
					//将多个版块组合后存储
					if($taskvarkey == 'forumid') { $_G['gp_forumid'] = implode(',', $_G['gp_forumid']); }
					$data = array(
						'taskid' => $taskid,
						'sort' => $taskvars['sort'],
	...
	if(!submitcheck('editsubmit')) {
		...
		showsetting('tasks_need_checkit', array('checkit',array(array('1','需要审核'),array('0','不需审核',0))), $task['checkit'], 'mradio');
		showtagfooter('tbody');
		...
		if($taskvars['complete']) {
			foreach($taskvars['complete'] as $taskvar) {
				...
				if(in_array($taskvar['type'], array('mcheckbox', 'mselect'))) {
					$taskvar['value'] = $taskvar['variable'] == 'forumid' ? explode(',', $taskvar['value']) : unserialize($taskvar['value']);
				}
		...
		foreach($itemarray as $item) {
			...
			//将多个版块组合后存储
			if($item == 'forumid') { $value = implode(',', $_G['gp_forumid']); }
			if($value !== null) {
...
}elseif($operation == 'manager'){/*新增对于任务的审核*/
	if(!submitcheck('updatesubmit')) {
		...
		showsubmenu('nav_tasks', array(
			...
			array(array('menu' => 'add', 'submenu' => $submenus), '', 0),
			array('nav_task_type', 'tasks&operation=type', 0),
			array('nav_task_manager', 'tasks&operation=manager', 1)
		));
		showformheader('tasks&operation=manager');
		showtableheader('members_verify_securitylist', 'fixpadding');
		showsubtitle(array('', 'nav_user', 'nav_task_type', 'tasks_reward', 'apply_time' , 'detail'));
		$tasks = array();
		if($count = DB::result_first('SELECT count(*) FROM '.DB::table('common_mytask').' WHERE status = 0 and checked = 0 and csc = \'100\'')){
			$query = DB::query('SELECT * FROM '.DB::table('common_task'));
			while($values = DB::fetch($query)){
				$tasks[$values['taskid']] = $values;
			}
			$limit_start = 20 * ($page - 1);
			$query = DB::query('SELECT * FROM '.DB::table('common_mytask').' mt LEFT JOIN '.DB::table('plugin_taskrecord').' pt on mt.uid = pt.uid and mt.taskid = pt.taskid  where mt.status = 0 and mt.checked = 0 and mt.csc = \'100\' and pt.state = 0 order by mt.dateline desc limit '.$limit_start.', 20');
			while($value = DB::fetch($query)){
				$multipage = multi($count, 20, $page, ADMINSCRIPT."?action=tasks&operation=manager");
				if($tasks[$value['taskid']]['reward'] == 'credit') {
					$reward = cplang('credits').' '.$_G['setting']['extcredits'][$tasks[$value['taskid']]['prize']]['title'].' '.$tasks[$value['taskid']]['bonus'].' '.$_G['setting']['extcredits'][$tasks[$value['taskid']]['prize']]['unit'];
				} elseif($tasks[$value['taskid']]['reward'] == 'magic') {
					$magicname = DB::result_first("SELECT name FROM ".DB::table('common_magic')." WHERE magicid='".$tasks[$value['taskid']]['prize']."'");
					$reward = cplang('tasks_reward_magic').' '.$magicname.' '.$tasks[$value['taskid']]['bonus'].' '.cplang('magic_unit');
				} elseif($tasks[$value['taskid']]['reward'] == 'medal') {
					$medalname = DB::result_first("SELECT name FROM ".DB::table('forum_medal')." WHERE medalid='".$tasks[$value['taskid']]['prize']."'");
					$reward = cplang('medals').' '.$medalname.($tasks[$value['taskid']]['bonus'] ? ' '.cplang('validity').$tasks[$value['taskid']]['bonus'].' '.cplang('days') : '');
				} elseif($tasks[$value['taskid']]['reward'] == 'invite') {
					$reward = cplang('tasks_reward_invite').' '.$tasks[$value['taskid']]['prize'].($tasks[$value['taskid']]['bonus'] ? ' '.cplang('validity').$tasks[$value['taskid']]['bonus'].' '.cplang('days') : '');
				} elseif($tasks[$value['taskid']]['reward'] == 'group') {
					$grouptitle = DB::result_first("SELECT grouptitle FROM ".DB::table('common_usergroup')." WHERE groupid='".$tasks[$value['taskid']]['prize']."'");
					$reward = cplang('usergroup').' '.$grouptitle.($tasks[$value['taskid']]['bonus'] ? ' '.cplang('validity').' '.$tasks[$value['taskid']]['bonus'].' '.cplang('days') : '');
				} else {
					$reward = cplang('none');
				}
				showtablerow('',array('class="td25"','','','','','class="record_ajax"'), array(
					"<input type=\"checkbox\" class=\"checkbox\" name=\"updates[]\" value=\"".$value['uid'].'_'.$value['taskid']."\">",
					"<a target='_blank' href='".$_G['config']['web']['home'].$value['uid']."'>".$value['username']."</a>",
					$tasks[$value['taskid']]['name'],
					$reward,
					date('Y-m-d H:i:s',$value['dateline']),
					$value['message'].'<b style="color:blue;margin-left:5px;">'.cplang('logs_medal_operation_2').'</b>'
				));
			}
		}else{
			showtablerow('',array('','colspan="4"'),array('','当前没有需要审核的任务'));
		}
		$multipage = isset($multipage) ? $multipage : '';
		showsubmit('updatesubmit', 'submit', '<input type="checkbox" name="chkall" id="chkall" class="checkbox" onclick="checkAll(\'prefix\', this.form, \'updates\')" /><label for="chkall">'.cplang('select_all').'</label>','<input type="radio" name="changetype" value="1" checked >'.cplang('members_verify_pass').'<input type="radio" name="changetype" value="0" >'.cplang('members_verify_refusal').'<input type="checkbox" name="sendmail" value="1" checked>'.cplang('members_newsletter_members'), $multipage);
		showtablefooter();
		showformfooter();
echo <<<EOF
<style>
...
<div id="tips_post"></div>
<script type="text/javascript" src="static/js/jquery-1.6.1.min.js"></script>
<script type="text/javascript">
jQuery.noConflict();
(function($){
	var tips = $('#tips_post');
	$("td.record_ajax a:contains('回帖')").mouseover(function(e){
		tips.css({left:e.pageX-50-tips.width()+'px',top:e.pageY+20+'px'}).fadeIn(500).html("<img src='static/images/loadding.gif'/>正在读取回帖详情……");
			$.get('plugin.php?id=api:ajax_getpost',{
				inajax:1,
				url:this.href
	...
	$("td.record_ajax a").mouseout(function(){
		tips.fadeOut(500);
	})
})(jQuery);
jQuery.noConflict();
...
EOF;
		...
		$updateview = $_G['gp_changetype'];
		$updates = $_G['gp_updates'];
		$sendmail = $_G['gp_sendmail'];
		if(!$updates) cpmsg('members_verify_null_selection',"action=tasks&operation=manager", 'error');
		$sql = 'UPDATE '.DB::table('common_mytask').' SET checked = '.($updateview == 1 ? '1' : '-1 , status = -1').'  WHERE ';
		$sql_b = 'UPDATE '.DB::table('plugin_taskrecord').' SET state = '.($updateview == 1 ? '1' : '-1').' WHERE ';
		foreach($updates as $ids){
			$ids = explode('_',$ids);
			$sql .= ' ( uid = '.$ids[0].' and taskid = '.$ids[1].') or';
			$sql_b .= ' ( uid = '.$ids[0].' and taskid = '.$ids[1].' and state = 0) or';
			$uids[$ids[0]][] = $ids[1];
		}
		$sql = substr($sql,0,-2);
		$sql_b = substr($sql_b,0,-2);
		require_once DISCUZ_ROOT.'./source/plugin/logs/logs.class.php';
		$logs = new logs('task_checked');
		$logs->log_str('管理员<b><font color="red">'.$_G['username'].'</font></b> 审核通过了用户ID=>任务ID <font color="blue">'.print_r($uids,true).'</font>');
		DB::query($sql);
		DB::query($sql_b);
		if($sendmail){
			$query = DB::query('SELECT taskid, name FROM '.DB::table('common_task'));
			while($values = DB::fetch($query)){
				$taskname[$values['taskid']] = $values['name'];
			}
			foreach($uids as $huid => $htaskid){
				foreach($htaskid as $dtaskid){
					notification_add($huid, 'task', 'task_reward_checked_'.($updateview == 1 ? 'ok' : 'error'), array(
							'taskid' => $dtaskid,
							'name' => $taskname[$dtaskid]
						));
				}
			}
		}
		cpmsg( $updateview ? 'members_verify_pass_success':'members_verify_ignore_success', "action=tasks&operation=manager", 'succeed');
	}
}

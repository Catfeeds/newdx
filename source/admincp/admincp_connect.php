...
if($operation == 'setting') {
	...
	if(!submitcheck('connectsubmit')) {
		...
		showsubmenu('menu_setting_qqconnect', array(
			array('connect_menu_setting', 'connect&operation=setting',  1),
			//$staturl ? array('connect_menu_stat', $staturl, 0, 0, 1) : array(),
		));
		...
		$connectnew = addslashes(serialize($_G['gp_connectnew']));
		...
		updatecache(array('setting', 'fields_connect_register'));

		...
		if($albums) {
			...
			if($_POST['optype'] == 'delete') {
				...
				foreach($catids as $catid) {
					$catid = intval($catid);
					$cnt = DB::result_first('SELECT COUNT(*) FROM '.DB::table('home_album')." WHERE catid = '$catid' " . getSlaveID());
...
if(submitcheck('searchsubmit', 1) || $newlist) {
	...
	if(!$error) {
		if($detail) {
			...
			$albumcount = DB::result_first("SELECT count(*) FROM ".DB::table('home_album')." a WHERE 1 $sql " . getSlaveID());

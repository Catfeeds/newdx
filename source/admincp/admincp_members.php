 ...
 *
 *      $Id: admincp_members.php 22725 2011-05-18 06:45:29Z monkey $
 */
...
if($operation == 'search') {
			...
			if($uids) {
				...
				while($member = DB::fetch($query)) {
					...
					$members .= showtablerow('', array('class="td25"', '', 'title="'.implode("\n", $memberextcredits).'"'), array(
                        ...
                        "<a href=\"javascript:;\" onclick=\"ajaxget('plugin.php?id=api:userinfoupdate&uid={$member[uid]}', '{$member[uid]}_update')\" >更新缓存</a><span id=\"{$member[uid]}_update\"></span>"
					), TRUE);
				}
			}
		}
	...
	if($uids && is_array($uids)) {
		...
		$query = DB::query("SELECT p.uid,m.username AS username,m.adminid,m.groupid,p.realname,p.gender,p.birthyear,p.birthmonth,p.birthday,p.constellation,
		...
		while($v = DB::fetch($query)) {
			foreach($v as $key => $value) {
				$value = preg_replace('/\s+/', ' ', $value); //把空格都过滤掉，免得导出时会错位
				if($key == 'gender'){$value = lang('space', 'gender_'.$value);}
				if($key == 'adminid'){$value =strip_tags($usergroups[$value]['grouptitle']);}
				if($key == 'groupid'){$value = strip_tags($usergroups[$value]['grouptitle']);}
//				$value = preg_replace('/\s+/' , ' ', nl2br($value));
	...
	//插入用户组
	$groupid = array_pop($title);
	$adminid = array_pop($title);
	array_splice($title,0,0,array($adminid,$groupid));
	foreach($title as $v) {
		$subject .= $v.",";
	}
	...
	if(empty($_G['gp_uid']) && empty($_G['gp_username']) && empty($_G['gp_ip'])) {
		...
		if(!empty($_G['gp_username'])) {
			$searchmember = DB::fetch_first("SELECT m.username AS username, ms.regip AS regip, ms.lastip AS lastip
				...
				WHERE m.username='$_G[gp_username]' ".getSlaveID());
			$urladd .= '&username='.$_G['gp_username'];
		} elseif(!empty($_G['gp_uid'])) {
			$searchmember = DB::fetch_first("SELECT m.username AS username, ms.regip AS regip, ms.lastip AS lastip
				FROM ".DB::table('common_member')." m LEFT JOIN ".DB::table('common_member_status')." ms ON m.uid=ms.uid
				WHERE m.uid='$_G[gp_uid]' ".getSlaveID());
			...
			//$search_condition['regip'] = $_G['gp_ip'];
			$urladd .= '&ip='.$_G['gp_ip'];
		}
		...
		$membernum = DB::result_first("SELECT COUNT(*) FROM ".DB::table('common_member_status')." ms WHERE 1 $repeatip ".getSlaveID());

		$members = '';
		if($membernum) {// 搜索到用户
			$usergroups = array();
			$query = DB::query("SELECT groupid, type, grouptitle FROM ".DB::table('common_usergroup').' '.getSlaveID());
			while($group = DB::fetch($query)) {
				switch($group['type']) {
					case 'system': $group['grouptitle'] = '<b>'.$group['grouptitle'].'</b>'; break;
					case 'special': $group['grouptitle'] = '<i>'.$group['grouptitle'].'</i>'; break;
			...
			$query = DB::query("SELECT m.uid AS uid, m.username AS username, m.adminid AS adminid, m.groupid AS groupid, m.credits AS credits,
				...
				LEFT JOIN ".DB::table('common_member_status')." ms ON m.uid=ms.uid WHERE 1 $repeatip LIMIT $start_limit, {$_G[setting][memberperpage]} ".getSlaveID());
			while($member = DB::fetch($query)) {
				$memberextcredits = array();
				foreach($_G['setting']['extcredits'] as $id => $credit) {
					$memberextcredits[] = $_G['setting']['extcredits'][$id]['title'].': '.$member['extcredits'.$id];
...
//					my_thread_log('delete', array('uid' => $member['uid']));
					deletepost("pid IN ($pidsdelete)");
					deletepost("tid IN ($tidsdelete)");
					deletethread("tid IN ($tidsdelete)");
					//有部分用户点评回帖数据不在他的回帖中，所以需要再清理一次这部分冗余数据
					DB::query("DELETE FROM ".DB::table('forum_postcomment')." WHERE authorid='$member[uid]'", 'UNBUFFERED');

					//删除新版用户发帖动态
					DB::query("DELETE FROM ".DB::table('home_feed_ucenter')." WHERE uid='$member[uid]' AND type IN (1, 2, 3, 6, 7)", 'UNBUFFERED');
					...
					if(file_exists(DISCUZ_ROOT."/source/plugin/forumoption/include.php")){
						...
						$qy = DB::query("select * FROM ".DB::table('dianping_star_logs')." WHERE uid IN ($member[uid])");
			...
			// TODO 以下的旧动态记录，在新版正常没什么需要的时候可以清理
			//删除日志
			if($_G['gp_delblog']) {
				...
				DB::query("DELETE FROM ".DB::table('home_feed_ucenter')." WHERE uid='$member[uid]' AND type = 4", 'UNBUFFERED');
			}
			//删除相册
			if($_G['gp_delalbum']) {
				...
				DB::query("DELETE FROM ".DB::table('home_feed_ucenter')." WHERE uid='$member[uid]' AND type = 5", 'UNBUFFERED');
			}
			//删除分享
			if($_G['gp_delshare']) {
			...
			//删除留言与评论
			if($_G['gp_delcomment']) {
				DB::query("DELETE FROM ".DB::table('home_comment')." WHERE id='$member[uid]' AND idtype='uid'");
				DB::query("DELETE FROM ".DB::table('home_comment')." WHERE uid='$member[uid]'");
				DB::query("DELETE FROM ".DB::table('home_comment')." WHERE authorid='$member[uid]'");
			}
	...
	$member = DB::fetch_first("SELECT m.*, m.telephone as bindtel, mf.*, mc.*, mh.*, ms.*, mp.*,".(!empty($_G['setting']['connect']['allow']) ? " mqc.*," : '')." m.uid AS muid, u.type, uf.allowsigbbcode, uf.allowsigimgcode ".
	...
	if(!submitcheck('editsubmit')) {
		...
		showsetting('members_edit_telephone', 'telephonenew', $member['bindtel'], 'text');
		showsetting('members_edit_telstatus', 'telstatusnew', $member['telstatus'], 'radio');
...
//		showsetting('members_edit_bday', 'bdaynew', $member['birthday'], 'text');
//		showsetting('members_edit_bio', 'bionew', $member['bio'], 'textarea');
		...
		if($_G['gp_clearavatar']) {
			...
			uc_user_deleteavatar($_G['gp_uid']);
		}
		...
		$telephonenew = $_G['gp_telephonenew'];
		$telstatusnew = intval($_G['gp_telstatusnew']);
		if(!empty($_G['setting']['connect']['allow'])) {
		...
		DB::query("UPDATE ".DB::table('common_member')." SET $emailadd regdate='$regdatenew', emailstatus='$emailstatusnew', telephone='$telephonenew', telstatus='$telstatusnew', status='$status', timeoffset='{$_G['gp_timeoffsetnew']}' $passwordadd WHERE uid='{$_G['gp_uid']}'");
		...
		/*DB::query("REPLACE INTO ".DB::table('onlinetime')." (uid, thismonth, total)
	...
	if($fieldid) {
		...
		if(!submitcheck('editsubmit')) {
			...
			// 特殊的同步值
			if($_GET['fieldid'] == 'birthday') {// 设置 birthmonth birthyear相同值
			...
			} elseif($_GET['fieldid'] == 'birthcity') {// 设置 birthprovince 相同
				DB::update('common_member_profile_setting', $setarr, array('fieldid'=>'birthprovince'));
			} elseif($_GET['fieldid'] == 'residecity') {// 设置resideprovince相同
	...
	if(!empty($_GET['fieldid']) && !isset($options[$_GET['fieldid']])) {//若指定了 fieldid，验证其合法性
...
function showsearchform($operation = '') {
	...
	foreach($_G['cache']['profilesetting'] as $fieldid=>$value) {
		...
		} elseif($fieldid == 'constellation') {// 特殊：生肖
			$select = "<option value=\"\">".cplang('nolimit')."</option>\n";
			for($i=1; $i<=12; $i++) {
		...
		} elseif($value['formtype'] == 'select' || $value['formtype'] == 'list') {// 列表框
			$select = "<option value=\"\">".cplang('nolimit')."</option>\n";
...
//	$option = $ms->cachecheck($condition, 600);// 缓存十分钟
//	if($option==false) {
	...
	if(!empty($_G['gp_current'])) {
		...
		if($operation == 'reward') {
			...
			if(!empty($medals)) {
				...
				/*
				$uids = array();
				$query = DB::query("SELECT uid FROM ".DB::table('common_member')." WHERE $conditions");
				while ($medaluid = DB::fetch($query)) {
					$uids[] = $medaluid['uid'];
				}
				*/
				$uids = searchmembers($search_condition);

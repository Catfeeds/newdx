...
function imgpre_update(id, obj) {
	...
	if(url) {
		...
		$('bgpre_' + id).style.backgroundImage = 'url(http://static.8264.com/static/image/common/none.gif)';
	}
}
		...
		foreach($predefinedvars as $predefinedvar => $v) {
			if($v !== array()) {
				...
				if(substr($predefinedvar, -7, 7) == 'bgcolor') {
					...
					$bgimgpre = $bgimg ? (preg_match('/^http:\/\//i', $bgimg) ? $bgimg : ($stylestuff['styleimgdir']['subst'] ? $stylestuff['styleimgdir']['subst'] : ($stylestuff['imgdir']['subst'] ? $stylestuff['imgdir']['subst'] : 'static/image/common')).'/'.$bgimg) : 'http://static.8264.com/static/image/common/none.gif';

...
if(!$operation) {
	...
	if(is_array($data['Data'])) foreach($data['Data'] as $row) {
		...
		echo '<tr><th colspan="3" class="partition">'.($row['Time'] != '' ? '<div class="right">'.$row['Time'].'</div>' : '').'<a href="'.$row['Url'].'" target="_blank">'.($row['Greenplugin'] ? '<img class="vmiddle" title="'.$lang['addons_greenplugin'].'" src="static/image/admincp/greenplugin.gif" /> ' : '').$row['Name'].($row['Version'] != '' ? ' '.$row['Version'] : '').'</a></th></tr>'.
			'<tr><td width="110">'.($row['Thumb'] != '' ? '<a href="'.$row['Url'].'" target="_blank"><img onerror="this.src=\'http://static.8264.com/static/image/common/none.gif\'" src="'.$row['Thumb'].'" width="100" /></a>' : '').'</td>'.
	...
	if(empty($_G['gp_providerkey'])) {
		...
		if(is_array($data) && $data) {
			...
			foreach($data as $row) {
				...
				($row['logo'] ? '<img width="100" height="50" src="'.$row['logo'].'" />' : '<img width="100" height="50" src="http://static.8264.com/static/image/common/none.gif" />').
...
function showproviderinfo($addon, $simple) {
	...
	echo (!$simple ? '<tr><th colspan="3" class="partition"><a href="'.ADMINSCRIPT.'?action=addons&operation=list&provider='.$addon['key'].'">'.$addon['title'].'</a></th></tr>' : '').
		'<tr><td width="110" valign="top"><a href="'.ADMINSCRIPT.'?action=addons&operation=list&provider='.$addon['key'].'"><img onerror="this.src=\'http://static.8264.com/static/image/common/none.gif\'" src="'.$addon['logo'].'" width="100" height="50" /></a></td>'.

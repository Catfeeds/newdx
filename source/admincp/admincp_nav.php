 ...
 *
 *      $Id: admincp_nav.php 22550 2011-05-12 05:21:39Z monkey $
 */
		...
		if(!submitcheck('submit')) {
			...
			foreach($navlist as $nav) {
				...
				if(!preg_match("/^".preg_quote(STATICURL, '/')."/i", $navicon) && !(($valueparse = parse_url($navicon)) && isset($valueparse['host']))) {
					$navicon = ($nav['serverid']>0 ? "http://".$alldomain[$nav['serverid']]['name']."/" : $_G['setting']['attachurl']).'common/'.$nav['icon'].'?'.random(6);
				}
				showtablerow('', array('class="td25"', 'class="td25"', '', ''), array(
					in_array($nav['type'], array('2', '1')) ? "<input class=\"checkbox\" type=\"checkbox\" name=\"delete[]\" value=\"$nav[id]\">" : '',
					"<input type=\"text\" class=\"txt\" size=\"2\" name=\"displayordernew[$nav[id]]\" value=\"$nav[displayorder]\">",
...
EOT;
		...
		if(!submitcheck('editsubmit')) {
			...
			if($_G['gp_deleteicon'] && $nav['icon']) {
                if($serverid>0){
                    //$attachserver->post_method($alldomain[$serverid]['domain'],$alldomain[$serverid]['api'],"del_pic&pic_name=common/".$nav['icon']."&havethumb=0");
                    $attachserver->delete($alldomain[$serverid]['domain'] , $alldomain[$serverid]['api'] , 'common/'.$nav['icon']);
                }else{
			...
			$iconadd = ", icon='$iconnew', serverid=$serverid";

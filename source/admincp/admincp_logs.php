<?php
	...
	$state=$operation.'log';
	/*数据库中读取数据
	*	@param  $state string 日志类型
	*/
	$logs = select_from_db($state);
	...
	//根据不同的传入数据  查询相应的值
	function select_from_db($action = 'action') {
		
		//新的添加数据
		//拼装sql语句
		$monthtime = date('Ymd', strtotime('-3month'));
		$result=DB::query("select logvalue from ".DB::table('common_redislog')." where filename='{$action}' and datetime>='{$monthtime}' order by datetime desc " . getSlaveID());
		while($row=DB::fetch($result)){
			//数据解码 并配装成规定的格式
			if(!empty($row['logvalue'])){
				$loglists[]=unserialize(base64_decode($row['logvalue']));
			}
		}
		foreach($loglists as $value){
			foreach($value as $v){
				$logs[]=$v;
			}
		}

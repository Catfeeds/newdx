 ...
 *
 *      $Id: admincp_moderate.php 19099 2010-12-16 02:25:32Z monkey $
 */
...
if($operation == 'threads') {
	...
	if(!submitcheck('modsubmit') && !$_G['gp_fast']) {
		...
		$modcount = getcountofposts(DB::table('forum_thread')." t INNER JOIN ".DB::table('forum_post')." p ON p.tid=t.tid AND p.first='1'", "$fidadd[t]$fidadd[fids]$fidadd[and] t.displayorder='$displayorder'".($modfid == '-1' ? " AND isgroup='1'": '')."$sqlwhere");
	...
	if(!submitcheck('modsubmit') && !$_G['gp_fast']) {
		...
		do {
			$start_limit = ($pagetmp - 1) * $tpp;
			$query = DB::query("SELECT p.picid, p.albumid, p.uid, p.username, p.title, p.dateline, p.filepath, p.thumb, p.remote, p.postip, a.albumname, p.serverid
				FROM ".DB::table('home_pic')." p
		...
		while($pic = DB::fetch($query)) {
            ...
            /*ÐÞ¸Ä£¬Ôö¼Óserverid*/
			$pic['url'] = pic_get($pic['filepath'], 'album', $pic['thumb'], $picture['remote'], 1, $pic['serverid']);

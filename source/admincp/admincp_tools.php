...
if($operation == 'updatecache') {
	...
	if($step == 1) {
		...
		memory('set', 'update_time', TIMESTAMP, 3600);
		$type = explode('_', $_G['gp_type']);
		if(in_array('data', $type)) {
			updatecache();
			require_once libfile('function/group');
			$groupindex['randgroupdata'] = $randgroupdata = grouplist('lastupdate', array('ff.membernum', 'ff.icon','ff.serverid_icon'), 80);
			$groupindex['topgrouplist'] = $topgrouplist = grouplist('activity', array('f.commoncredits', 'ff.membernum', 'ff.icon','ff.serverid_icon'), 10);

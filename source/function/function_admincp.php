 ...
 *
 *      $Id: function_admincp.php 22990 2011-06-13 03:10:14Z monkey $
 */
...
function siteinformation() {
	...
	$updatetime = memory('get', 'updatetime_lock');
	if(empty($updatetime) || (TIMESTAMP - $updatetime > 3600 * 4)) {
		memory('set', 'updatetime_lock', mktime());
...
function showheader($key, $url) {
	// 130827 rinne 判断是否modules功能模块来决定参数
	global $ca;
	$ctl = in_array($key, $ca) ? 'ctl' : 'action';
	$act = in_array($key, $ca) ? 'act' : 'operation';
...
//	echo '<li><em><a href="'.ADMINSCRIPT.'?action='.$url.'" id="header_'.$key.'" hidefocus="true" onmouseover="previewheader(\''.$key.'\')" onmouseout="previewheader()" onclick="toggleMenu(\''.$key.'\', \''.$url.'\');doane(event);">'.cplang('header_'.$key).'</a></em></li>';
	$url = $action.($operation ? "&{$act}=".$operation.($do ? '&do='.$do : '') : '');
	echo '<li><em><a href="'.ADMINSCRIPT."?{$ctl}=".$url.'" id="header_'.$key.'" hidefocus="true" onmouseover="previewheader(\''.$key.'\')" onmouseout="previewheader()" onclick="toggleMenu(\''.$key.'\', \''.$url.'\');doane(event);">'.cplang('header_'.$key).'</a></em></li>';
}
...
//				$body .= '<li><a href="'.(substr($menu[1], 0, 4) == 'http' ? $menu[1] : ADMINSCRIPT.'?action='.$menu[1]).'" hidefocus="true" target="'.($menu[2] ? $menu[2] : 'main').'"'.($menu[3] ? $menu[3] : '').'><em onclick="menuNewwin(this)" title="'.cplang('nav_newwin').'"></em>'.cplang($menu[0]).'</a></li>';
				$menu[1] = $action.($operation ? "&{$act}=".$operation.($do ? '&do='.$do : '') : '');
				$body .= '<li><a href="'.(substr($menu[1], 0, 4) == 'http' ? $menu[1] : ADMINSCRIPT."?{$ctl}=".$menu[1]).'" hidefocus="true" target="'.($menu[2] ? $menu[2] : 'main').'"'.($menu[3] ? $menu[3] : '').'><em onclick="menuNewwin(this)" title="'.cplang('nav_newwin').'"></em>'.cplang($menu[0]).'</a></li>';
			}
		}
	}
	if(!$return) {
...
<body>
<script type="text/JavaScript">
var admincpfilename = '$basescript', IMGDIR = '$IMGDIR', STYLEID = '$STYLEID', VERHASH = '$VERHASH', IN_ADMINCP = true, ISFRAME = $frame, STATICURL='http://static.8264.com/static/', SITEURL = '$_G[siteurl]';
</script>
...
function showtableheader($title = '', $classname = '', $extra = '', $titlespan = 16) {
	global $_G;
...
function album_picnum_stat($start, $perpage) {
	...
	while ($value = DB::fetch($query)) {
		$next = true;
		$count = DB::result(DB::query("SELECT COUNT(*) FROM ".DB::table('home_pic')." WHERE albumid='$value[albumid]' " . getSlaveID()),0);
...
function get_pluginsetting($type) {
	...
	if ($allowmem){
		$pluginsetting = memory('get', 'cache_pluginsetting');
	}
	if ($isfilecache && empty($pluginsetting)){

	...
	if(!$bbcodeoff) {
		...
		if(strpos($msglower, '[/img]') !== FALSE) {
			$message = preg_replace(array(
				"/\[img\](static.8264.com\/static\/image\/common\/back.gif)\[\/img\]/ies",
				"/\[img\](http:\/\/static.8264.com\/static\/image\/common\/back.gif)\[\/img\]/ies",
				"/\[img\]\s*([^\[\<\r\n]+?)\s*\[\/img\]/ies",
				"/\[img=(\d{1,4})[x|\,](\d{1,4})\]\s*([^\[\<\r\n]+?)\s*\[\/img\]/ies"
			), $allowimgcode ? array(
				...
				"bbcodeurl('\\1', '<img class=\"zoom\"".(($_G['fid']==$_G['config']['fids']['zbfx'] || $_G['fid']==$_G['config']['fids']['produce']) ? ' onload="thumbImg(this)"' : '')." onload=\"thumbImg(this)\" src=\"{url}\" alt=\"\" />')",
				"bbcodeurl('\\1', '<img class=\"zoom\"".(($_G['fid']==$_G['config']['fids']['zbfx'] || $_G['fid']==$_G['config']['fids']['produce']) ? ' onload="thumbImg(this)"' : '')." onload=\"thumbImg(this)\" onclick=\"zoom(this,this.src)\" src=\"{url}\" alt=\"\" />')",
				"parseimg('\\1', '\\2', '\\3')"
			) : array(
...
function parseurl($url, $text, $scheme) {
	...
	if(!$url && preg_match("/((https?|ftp|gopher|news|telnet|rtsp|mms|callto|bctp|thunder|synacast){1}:\/\/|www\.)[^\[\"']+/i", trim($text), $matches)) {
		...
		$nofllow = (strpos($url, '.8264.com') !== false || strpos($url, '.lvyoumall.com') !== false) ? '' : ' rel="nofollow"';	//非本站链接增加nofollow属性
		return '<a href="'.(substr(strtolower($url), 0, 4) == 'www.' ? 'http://'.$url : $url).'" target="_blank"'.$nofllow.'>'.$text.'</a>';
	} else {
		$url = substr($url, 1);
		...
		$nofllow = (strpos($url, '.8264.com') !== false || strpos($url, '.lvyoumall.com') !== false) ? '' : ' rel="nofollow"';	//非本站链接增加nofollow属性
		return '<a href="'.$url.'" target="_blank"'.$nofllow.'>'.$text.'</a>';
	}
}

	...
	if(@rename(DISCUZ_ROOT.$logfile, $newlog)) {
		...
		if(is_array($viewlog) && !empty($viewlog)) {
			...
			foreach($viewlog as $id => $views) {
				//modify by wangqi addview
				if ($table == 'forum_thread' && $idcol = 'tid') {
					$beinum = rand(1,3);
					$views = rand($views, $views*$beinum);
				}
				//end
...
* @author JiangHong
*/
function updateviews_redis($table, $idcol, $viewscol, $model, $cronrun = false){
    global $_G;
	if($_G['config']['ignor_redisviews'] === true && ! $cronrun){
		return;
	}
    $viewlog = $viewarray = array();
    ...
    if($myredis->connected){
        $doing = $model.'_doing';
    	if ($myredis->obj->exists($doing) === false && ($myredis->obj->hlen($model.'_number') > 800 || $cronrun)){
            $myredis->obj->rename($model.'_number',$doing);
            $viewlog = $myredis->obj->hgetall($doing);
            $numberid = 0;
            foreach($viewlog as $id => $views) {
            	$numberid++;
            	if((strlen($viewarray[$views]) > 10000 || $numberid > 20000) && ! $cronrun){
            		$myredis->obj->rpush($model . '_queue_watting', serialize(array('tid' => $id, 'views' => $views)));
            	}else{
            		if ($table == 'forum_thread' && $idcol == 'tid') {
	                    $trueviewarray[$views] .= ($id > 0) ? ','.intval($id) : '';
	            		$beinum = rand(2,4);
	            		$views = rand($views, $views*$beinum);
	            	}
            		$viewarray[$views] .= ($id > 0) ? ','.intval($id) : '';
            ...
            foreach($viewarray as $views => $ids) {
            	$sql = "UPDATE LOW_PRIORITY ".DB::table($table)." SET $viewscol=$viewscol+'$views' WHERE $idcol IN (0$ids)";
            	if( ! $cronrun){
	            	DB::query($sql, 'UNBUFFERED');
            	}else{
	            	try{
	            		$redisqueue->addQueue('viewshandle', 'todatabase', array('viewscol' => $viewscol, 'views' => $views, 'idcol' => $idcol, 'ids' => $ids, 'table' => $table), 'viewstodatabase');
	            	}catch(Exception $e){
	            		DB::query($sql, 'UNBUFFERED');
	            	}
            	}
            ...
            if ($table == 'forum_thread' && $idcol == 'tid') {
                foreach($trueviewarray as $views => $ids){
                	$sql = "UPDATE LOW_PRIORITY ".DB::table($table)." SET trueviews=trueviews+'$views' WHERE $idcol IN (0$ids)";
                	if( ! $cronrun ){
                		DB::query($sql, 'UNBUFFERED');
                	}else{
	                	try{
	                		$redisqueue->addQueue('viewshandle', 'todatabase', array('viewscol' => 'trueviews', 'views' => $views, 'idcol' => $idcol, 'ids' => $ids, 'table' => $table), 'viewstodatabase');
	                	}catch(Exception $e){
		         			DB::query($sql, 'UNBUFFERED');
	         			}
         			}
            ...
            $myredis->obj->rename($doing, $doing.'_end');
        }elseif($myredis->obj->hlen($model.'_number')>1500){
            fix_redis_views($doing,$model.'_number',array());
        }
        $limitrun = $cronrun ? $myredis->obj->llen($model . '_queue_watting') : 500;
        for($_i = 0; $_i <= $limitrun; $_i++){
        	$_re = $myredis->obj->lpop($model . '_queue_watting');
        	if($_re){
        		$_re = unserialize($_re);
        		if($_re['tid'] > 0 && $_re['views'] > 0){
        			$myredis->obj->hincrby($model . '_number', $_re['tid'], $_re['views']);
        		}
        	}else{
        		break;
        	}
...
/**
 * 新增，用于解决redis缓存点击量出现问题的情况，次函数将doing中的所有点击加回原先缓存中
 * @author JiangHong
 * @param $model string 当前出问题的入库key
 * @param $oldmodel string 此入库key的原先缓存key
 */
function fix_redis_views($model,$oldmodel,$args){
    require_once libfile('class/myredis');
    $myredis = & myredis::instance(6381);
    if($myredis->connected){
        $num = sprintf("%03d",rand(1,999));
        $newname = $model.'_bak_'.$num;
        if(!$myredis->islock_process('fixedviews_redis')){
            $myredis->obj->renamenx($model,$newname);
            $baks = $myredis->obj->keys($model.'_bak_???');
            foreach($baks as $bak){
                if(empty($args)){
                    if($allthreads = $myredis->obj->hgetall($bak)){
                        foreach($allthreads as $tid=>$views){
                                if($myredis->obj->hincrby($oldmodel,$tid,$views)){
                                    $myredis->hashdel($bak,$tid);
                                }
                        }
                    }
                }else{
                    updateviews_redis($args['table'],$args['idcol'],$args['viewscol'],$bak);
                }
            }
            $myredis->unlock('fixedviews_redis');
        }
    }
}

 ...
 *
 *      $Id: function_member.php 23107 2011-06-20 03:12:04Z zhengqingpeng $
 */
...
function userlogin($username, $password, $questionid, $answer, $loginfield = 'username') {
	...
	if($isuid == 3) {
		if(preg_match('/^[1-9]\d*$/', $username)) {
			$return['ucresult'] = uc_user_login($username, $password, 1, 0, $questionid, $answer);
		} elseif(isemail($username)) {
			$return['ucresult'] = uc_user_login($username, $password, 2, 0, $questionid, $answer);
		}
		if($return['ucresult'][0] <= 0 && $return['ucresult'][0] != -3) {
			$return['ucresult'] = uc_user_login($username, $password, 0, 0, $questionid, $answer);
		}
	} else {
		$return['ucresult'] = uc_user_login($username, $password, $isuid, 0, $questionid, $answer);
	}
	//变量初始化
	$tmp = array();
	$duplicate = '';
...
/*手机登陆，去掉了安全提问*/
function userlogin_m($username, $password, $questionid, $answer, $loginfield = 'username') {
	$return = array();
...
function logincheck($username, $ip = '') {
	global $_G;
	$return = 0;
    $ip ? true : $ip = $_G[clientip];
	$username = addslashes(trim(stripslashes($username)));
	$login = DB::fetch_first("SELECT count, lastupdate FROM ".DB::table('common_failedlogin')." WHERE ip='$ip' AND username='$username'");
	$return = (!$login || (TIMESTAMP - $login['lastupdate'] > 900)) ? 5 : max(0, 5 - $login['count']);

	if(!$login) {
		DB::query("REPLACE INTO ".DB::table('common_failedlogin')." (ip, username, count, lastupdate) VALUES ('$ip', '$username', '0', '$_G[timestamp]')");
...
function loginfailed($username, $ip = '') {
    ...
    $ip ? true : $ip = $_G[clientip];
	$username = addslashes(trim(stripslashes($username)));
	DB::query("UPDATE ".DB::table('common_failedlogin')." SET count=count+1, lastupdate='$_G[timestamp]' WHERE ip='$ip' AND username='$username'");
}
...
function clearcookies() {
	...
	foreach($_G['cookie'] as $k => $v) {
		// if($k != 'widthauto') {
		if(!in_array($k, array('widthauto', 'app-download-dialog'))) {	//临时用，不清除APP浮层广告cookie
			dsetcookie($k);
		}
	}
	$_G['uid'] = $_G['adminid'] = 0;

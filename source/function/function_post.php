...
function getattach($pid, $posttime = 0, $showdir = 'forum', $limit = 100) {
	global $_G;
	...
	$query = DB::query("SELECT a.*, af.description
		...
		WHERE (a.uid='$_G[uid]' AND a.tid='0' $sqladd1) $sqladd2 ORDER BY a.aid ASC LIMIT {$limit}");
...
function updateattach($postattachcredits, $tid, $pid, $attachnew, $attachdel, $attachupdate = array(), $uid = 0) {
	...
	if($attachnew) {
        ...
        /*查询中加入查找附件服务器ID*/
		$query = DB::query("SELECT aid, tid, attachment, serverid FROM ".DB::table('forum_attachment')." WHERE aid IN (".dimplode($newaids).")$uidadd");
		while($attach = DB::fetch($query)) {
			...
			if(!$attach['tid']) {
                ...
                /*记录附件服务器ID*/
                $newattachserverid[$attach['aid']] = $attach['serverid'];
			}
		}
		...
		foreach($attachnew as $aid => $attach) {
			...
			if($_G['setting']['watermarkstatus'] && empty($_G['forum']['disablewatermark']) && !empty($newattachfile[$aid])) {
                if($newattachserverid[$aid]>0){
                    /*上传方法中直接做了水印处理，去掉此处的水印*/
                    //$attachserver->Watermark($all_serverinfo[$newattachserverid[$aid]]['domain'],$all_serverinfo[$newattachserverid[$aid]]['api'],'forum/'.$newattachfile[$aid],'forum');
                }else{
			...
			if(!empty($_G['gp_albumaid']) && isset($albumattach[$aid])) {
				...
				$picdata = array(
                    ...
                    'serverid' => $albumattach[$aid]['serverid']
				);
...
/*查询中加入查附件服务器ID*/
	$query = DB::query("SELECT aid, attachment, thumb, remote, serverid FROM ".DB::table('forum_attachment')." WHERE uid='$uid'");
	$delaids = array();
	while($attach = DB::fetch($query)) {
		$aids[] = $attach['aid'];
	...
	if($attachupdate) {
        ...
        /*查询中加入附件服务器ID*/
		$query = DB::query("SELECT aid, width, filename, filetype, filesize, attachment, isimage, thumb, remote, serverid FROM ".DB::table('forum_attachment')." WHERE aid IN ($uaids)$uidadd");
		...
		while($attach = DB::fetch($query)) {
			...
			if($_G['setting']['watermarkstatus'] && empty($_G['forum']['disablewatermark'])) {
                if($attach['serverid'] > 0){
                    /*水印整合在上传中，此处不需要再做水印*/
                    //$attachserver->Watermark($all_serverinfo[$attach['serverid']]['domain'],$all_serverinfo[$attach['serverid']]['api'],'forum/'.$attach['attachment'],'forum');
                }else{
	...
	$attachment = $attachcount ? (DB::result_first("SELECT count(*) FROM ".DB::table('forum_attachment')." WHERE tid='$tid'".($pid > 0 ? " AND pid='$pid'" : '')." AND isimage != 0 " . getSlaveID()) ? 2 : 1) : 0;

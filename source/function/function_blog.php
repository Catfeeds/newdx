 ...
 *
 *      $Id: function_blog.php 22550 2011-05-12 05:21:39Z monkey $
 */
...
/*	if($POST['friend'] == 2) {
	...
	if($uploads) {
		...
		if(!empty($mathes[1])) {
			...
			foreach ($mathes[1] as $key => $value) {
				if(!empty($uploads[$value])) {
					$picurl = pic_get($uploads[$value]['filepath'], 'album', $uploads[$value]['thumb'], $uploads[$value]['remote'], 0, $uploads[$value]['serverid']);
					$searchs[] = "[imgid=$value]";
		...
		foreach ($uploads as $value) {
			$picurl = pic_get($value['filepath'], 'album', $value['thumb'], $value['remote'], 0, $value['serverid']);
...
*/
        ...
        // 2012-07-02 新增 jianghong  用于查询是否为品牌商，是的话将更新空间日志模块
        require_once DISCUZ_ROOT.'./source/plugin/forumoption/include.php';
        $type_module = "blogs";
        $search_id = $blogarr['uid'];
        $produce_info = $forumOption->get_tid_by_uid($search_id);
    	if($produce_info){
            for($b_i=0;$b_i<count($produce_info);$b_i++){
                $newthread = array('otherid' => $blogid,
                                'id' =>$produce_info[$b_i],
                                'type' =>$type_module,
                                'dateline' =>time()
                                );
	...
	if(!empty($titlepic)) {
        ...
        /*将服务器标识插入home_blogfield*/
        $fieldarr['serverid'] = $serverid;
        /*结束*/
	}
...
$html = '<span id="'.$randomid.'"></span><script type="text/javascript" reload="1">$(\''.$randomid.'\').innerHTML=AC_FL_RunContent(\'width\', \''.$width.'\', \'height\', \''.$height.'\', \'allowNetworking\', \'internal\', \'allowScriptAccess\', \'none\', \'src\', \'http://static.8264.com/static/image/common/flvplayer.swf\', \'flashvars\', \'file='.rawurlencode($swf_url).'\', \'quality\', \'high\', \'wmode\', \'transparent\', \'allowfullscreen\', \'true\');</script>';
}
	}
	return $html;
}

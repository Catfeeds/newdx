...
function get_uploadcontent($attach) {
	$return = '';
	if($attach['isimage']) {
		$pic = pic_get($attach['attachment'], 'portal', $attach['thumb'], $attach['remote'], 0,$attach['serverid'], $attach['nowater']);
		$small_pic = $attach['thumb'] ? ($pic.'.thumb.jpg') : '';
		if($small_pic && $attach['serverid'] > 50){
			$small_pic = preg_replace("/(!|_|-)(t\d+w\d+h\d+)?(x\d+m\d+)?/i", "!t1w140h140", $pic);
		}
		$check  = $attach['pic'] == $attach['attachment'] ? 'checked' : '';
		$mcheck = $attach['mpic'] == $attach['attachment'] ? 'checked' : '';
		$aid    = $check || $mcheck ? $attach['aid'] : '';
		$isMpic = $mcheck ? 'yes' : '';
		...
		$return .= '<td class="bbs">';
		$return .= '<label for="setconver'.$attach['attachid'].'"><input type="radio" name="setconver" id="setconver'.$attach['attachid'].'" class="pr" value="1" onclick=setConver(\''.addslashes(serialize(array('pic'=>$attach['attachment'], 'thumb'=>$attach['thumb'], 'remote'=>$attach['remote'], 'serverid'=>$attach['serverid']))).'\') '.$check.'> '.lang('portalcp', 'set_to_conver').'</label><br>';
		$return .= '<label for="setmconver'.$attach['attachid'].'"><input type="radio" name="setmconver" id="setmconver'.$attach['attachid'].'" class="pr" value="1" onclick=setMConver(\''.addslashes(serialize(array('pic'=>$attach['attachment'], 'thumb'=>$attach['thumb'], 'remote'=>$attach['remote'], 'serverid'=>$attach['serverid']))).'\') '.$mcheck.'> '.lang('portalcp', 'set_to_m_conver').'</label><br>';
		...
		$return .= '<a href="javascript:void(0);" onclick="insertImage(\''.$pic.($attach['ifwater'] ? '!x9m2' : '').'\');return false;">'.lang('portalcp', 'insert_large_image').'</a><br>';
		$return .= '<a href="javascript:void(0);" onclick="deleteAttach(\''.$attach['attachid'].'\', \'portal.php?mod=attachment&id='.$attach['attachid'].'&aid='.$aid.'&op=delete&isMpic='.$isMpic.'\');return false;">'.lang('portalcp', 'delete').'</a>';
		$return .= '</td>';
		$return .= '</table>';
...
function save_diy_data($primaltplname, $targettplname, $data, $database = false, $optype = '') {
	...
	$dateline = DB::result_first("SELECT dateline FROM ".DB::table('common_diy')." WHERE diyname = 'data/diy/{$targettplname}.htm'");
	//备份diy数据
	$insertdiyarr_bak = array(
		'diyname' => 'data/diy/'.$targettplname.'.htm',
		'diycontent' => base64_encode($content),
		'username' => $_G['username'],
		'updatetime' => $_G['timestamp']
	);
	//end
	// DB::insert('common_diy_bak', $insertdiyarr_bak, true);
	if(!$dateline) {
		$insertdiyarr = array(
			'diyname' => 'data/diy/'.$targettplname.'.htm',
			'diycontent' => base64_encode($content),
		...
		$diyid = DB::insert('common_diy', $insertdiyarr, true);
		...
		$where = array('diyname' => 'data/diy/'.$targettplname.'.htm');
		$diyid = DB::update('common_diy', $updatediyarr, $where);
	}
	/*修复不停清空模板缓存的BUG*/

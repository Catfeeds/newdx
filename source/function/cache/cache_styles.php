...
function build_cache_styles() {
	...
	while($data = DB::fetch($query)) {
		...
		$datanew = array();
		$data['imgdir'] = $data['imgdir'] ? $data['imgdir'] : 'http://static.8264.com/static/image/common';
...
function writetocsscache($data) {
	...
	while(($entry = readdir($dh)) !== false) {
		if(fileext($entry) == 'css') {
            ...
            DB::query("REPLACE INTO ".DB::table('plugin_css_cache')."(`filename`,`values`,`dateline`,`type`) VALUES('{$filename}','".(base64_encode($cssdata))."',".time().",1)");

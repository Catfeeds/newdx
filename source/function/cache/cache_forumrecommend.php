...
function build_cache_forumrecommend() {
	...
	while($row = DB::fetch($query)) {
		...
		$squery = DB::query("SELECT f.fid, f.name, ff.icon,ff.serverid_icon FROM ".DB::table('forum_forum')." f LEFT JOIN ".DB::table('forum_forumfield')." ff ON ff.fid=f.fid WHERE recommend='$row[fid]'");

 ...
 *
 *      $Id: cache_setting.php 23027 2011-06-14 09:23:15Z monkey $
 */
...
function build_cache_setting() {
	...
	$serialized = array('reginput', 'memory', 'search', 'creditspolicy', 'ftp', 'secqaa', 'ec_credit', 'qihoo', 'spacedata',
		...
		'seccodedata', 'inviteconfig', 'connect'
		);
	...
	while($setting = DB::fetch($query)) {
		if($setting['skey'] == 'extcredits') {
			...
			/*修改域名存储，改为文件式，存在config中*/	
			if($setting['skey']=='domain' && !empty($_G['config']['host'])){
				$setting['svalue']['app'] = array_merge($setting['svalue']['app'],$_G['config']['host']);
				$setting['svalue']['root'] = array_merge($setting['svalue']['root'],$_G['config']['alldomain']);
			}
			/*结束*/
		}
	...
	while($nav = DB::fetch($query)) {
		if($nav['icon']) {
			...
			if(!preg_match("/^".preg_quote(STATICURL, '/')."/i", $navicon) && !(($valueparse = parse_url($navicon)) && isset($valueparse['host']))) {
				...
				$navicon = ($nav['serverid'] > 0 ? "http://".$alldomain[$nav['serverid']]."/" : $_G['setting']['attachurl']).'common/'.$nav['icon'].'?'.random(6);
			}

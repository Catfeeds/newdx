...
function updatecache($cachename = '' , $updatestyle = false) {
	...
	if(!$updatelist) {
		...
		while($entry = $cachedirhandle->read()) {
			if(!in_array($entry, array('.', '..')) && preg_match("/^cache\_([\_\w]+)\.php$/", $entry, $entryr) && $entryr[1] != 'setting' && substr($entry, -4) == '.php' && is_file($cachedir.'/'.$entry)) {
				if(!$updatestyle && $entryr[1] == 'styles') continue;
...
function writetocaches($script, $cachedata, $prefix = 'cache_') {
	global $_G;

	$dir = DISCUZ_ROOT.'./data/plugincache/';
	if(!is_dir($dir)) {
		@mkdir($dir, 0777);
	}
	if($fp = @fopen("$dir$prefix$script.php", 'wb')) {
...
function pluginsettingvalue($type) {
	...
	//Ôö¼Ómemcache»º´æÅÐ¶Ï modify by wangqi 20121011
	if($isfilecache === null) {

...
function deletethread($condition, $unbuffered = true) {
	//Modifyed by mengjin, 2011-08-24 16:49
	global $forumOption,$_G;
    if ($forumOption) {
		$forumOption->hookScriptDelete($condition);
	}
	if(empty($_G['fid']) && $_G['groupid']==1){
		...
		if(file_exists(DISCUZ_ROOT.'./source/plugin/forumoption/skiing.php')){			
			...
			ForumOptionSkiing::deletSkiingInfo($condition);
		}	
		ForumOptionShop::deletShopInfo($condition);
	...
	$tids = str_replace('tid', 'id', $condition);
	deletefeed($tids, 'thread');
		
	return $deletedthreads;
}

function deleteattach($condition, $unbuffered = true, $dir = 'forum') {
	$pics = array();
	$query = DB::query("SELECT attachment, thumb, remote, aid, picid, serverid FROM ".DB::table('forum_attachment')." WHERE $condition AND pid>0");
...
function deleteblogs($blogids) {
	...
	$newblogids = dimplode($newblogids);	
	DB::query("DELETE FROM ".DB::table('home_blog')." WHERE blogid IN ({$newblogids})");
	DB::query("DELETE FROM ".DB::table('home_blogfield')." WHERE blogid IN ({$newblogids})");
	DB::query("DELETE FROM ".DB::table('home_comment')." WHERE id IN ({$newblogids}) AND idtype='blogid'");
	DB::query("DELETE FROM ".DB::table('home_feed')." WHERE id IN ({$newblogids}) AND idtype='blogid'");
	DB::query("DELETE FROM ".DB::table('home_clickuser')." WHERE id IN ({$newblogids}) AND idtype='blogid'");	
	require_once libfile('function/feed');
	deletefeed($newblogids, 'blog');
...
/*结束*/
	$albumids = array();
	$query = DB::query("SELECT * FROM ".DB::table('home_pic')." WHERE picid IN (".dimplode($picids).") " . getSlaveID());
	while ($value = DB::fetch($query)) {
		if($allowmanage || $value['uid'] == $_G['uid']) {
            /*将附件服务器信息传入*/
            $value['post_domain'] = $value['serverid']>0?$domain[$value['serverid']]['domain']:'';
            $value['post_api'] = $value['serverid']>0?$domain[$value['serverid']]['api']:'';
            /*结束*/
			$pics[] = $value;
			$newids[] = $value['picid'];
...
function deletepicfiles($pics) {
	...
	foreach ($pics as $pic) {
		pic_delete($pic['filepath'], 'album', $pic['thumb'], $pic['remote'], $pic['post_domain'], $pic['post_api'],$pic['serverid']);
	}
}
...
$domain = $attachserver->get_server_domain('all','*');
/*结束*/
	$query = DB::query("SELECT * FROM ".DB::table('home_album')." WHERE albumid IN (".dimplode($albumids).") " . getSlaveID());
	while ($value = DB::fetch($query)) {
		if($allowmanage || $value['uid'] == $_G['uid']) {
            /*将附件服务器信息传入*/
            $value['post_domain'] = $value['serverid']>0?$domain[$value['serverid']]['domain']:'';
			...
			if(!empty($value['pic'])) {
				include_once libfile('function/home');
				pic_delete($value['pic'], 'album', 0, ($value['picflag'] == 2 ? 1 : 0), $value['post_domain'], $value['post_api'],$value['serverid']);
			}
		}
	...
	$pics = $picids = array();
	$query = DB::query("SELECT * FROM ".DB::table('home_pic')." WHERE albumid IN (".dimplode($newids).") " . getSlaveID());
	while ($value = DB::fetch($query)) {
        /*将附件服务器信息传入*/
        $value['post_domain'] = $value['serverid']>0?$domain[$value['serverid']]['domain']:'';
        $value['post_api'] = $value['serverid']>0?$domain[$value['serverid']]['api']:'';
...
function deletearticle($aids, $istrash = 1) {
	...
	require_once DISCUZ_ROOT."./source/plugin/attachment_server/attachment_server.class.php";
	$attachserver = new attachserver;
	$allinfo = $attachserver->get_server_domain('all','*');
	/*结束*/
	while ($value = DB::fetch($query)) {
	...
	if($dels) {
		foreach($article as $key => $value) {
			if($istrash) {
				...
				pic_delete($value['pic'], 'portal', $value['thumb'], $value['remote'],$allinfo[$value['serverid']]['domain'],$allinfo[$value['serverid']]['api'],$value['serverid']);
				$attachaid[] = $value['aid'];
			}
		}
		if($istrash) {
...
function deletearticlerelated($dels) {
	...
	foreach ($attachment as $value) {
		pic_delete($value['attachment'], 'portal', $value['thumb'], $value['remote'], $alldomain[$value['serverid']]['domain'], $alldomain[$value['serverid']]['api'],$value['serverid']);
	}
...
function deleteportaltopic($dels) {
	...
	while ($value = DB::fetch($query)) {
		if($value['picflag'] != '0') pic_delete(str_replace('portal/', '', $value['cover']), 'portal', 0, $value['picflag'] == '2' ? '1' : '0', $alldomain[$value['serverid']]['domain'],$alldomain[$value['serverid']]['api'],$value['serverid']);
	}

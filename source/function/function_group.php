 ...
 *
 *      $Id: function_group.php 22550 2011-05-12 05:21:39Z monkey $
 */
...
function grouplist($orderby = 'displayorder', $fieldarray = array(), $num = 1, $fids = array(), $sort = 0, $getcount = 0, $grouplevel = array()) {
	...
	while($group = DB::fetch($query)) {
		...
		isset($group['icon']) && $group['icon'] = get_groupimg($group['icon'], 'icon', $group['serverid_icon']);
		isset($group['banner']) && $group['banner'] = get_groupimg($group['banner'],'', $group['serverid_banner']);
...
function get_groupimg($imgname, $imgtype = '',$serverid = '') {
	global $_G;
	if($imgname) {
        /*加入附件服务器*/

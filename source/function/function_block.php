 ...
 *
 *      $Id: function_block.php 22550 2011-05-12 05:21:39Z monkey $
 */
	...
	if(is_object($obj)) {
		...
		if(in_array($block['blockclass'], array('forum_thread', 'forum_attachment', 'group_thread', 'group_attachment', 'space_blog', 'space_pic', 'portal_article'))) {
			...
			if($leftnum > 0) {
				...
				$return = $obj->getdata($thestyle, $block['param'], in_array($bid ,array(1412, 2191, 3889)) ? 't.tid > 1750000 AND' : '');
		...
		foreach($block['itemlist'] as $itemid=>$blockitem) {
			...
			foreach($fields as $key=>$field) {
				...
				if($field['datatype'] == 'int') {// int
				    ...
				    $picflag = empty($blockitem['picflag']) ? '0' : $blockitem['picflag'];
                    $serverid = $blockitem['serverid'];
				    if($picflag == '0' && strpos($replacevalue,"{$_G[setting][domain][app][attach]}/topicpreview")!==false){
        				$picflag = '1';
                        $serverid = 1;
				    }
					if($picflag == '1') {
						$replacevalue = str_replace($_G['setting']['attachurl'],"",$replacevalue);
    					$replacevalue = str_replace($_G['config']['web']['attach'],"",$replacevalue);
						if($serverid>0){
				            $filepath = $replacevalue;
							$replacevalue = "http://".$alldomain[$serverid]['name']."/".$replacevalue;
						}else{
					...
					if($block['picwidth'] && $block['picheight'] && $block['picwidth'] != 'auto' && $block['picheight'] != 'auto') {
						if($blockitem['makethumb'] == 1) {
							$replacevalue = ($serverid>0 ? "http://".$alldomain[$serverid]['name']."/" : $_G['setting']['attachurl']).block_thumbpath($block, $blockitem);
							...
							if($serverid>0){
				                if($attachserver->Thumb($alldomain[$serverid]['domain'] , $alldomain[$serverid]['api'] , $filepath , $thumbpath , $block['picwidth'] , $block['picheight'] , 2)){
				                    DB::update('common_block_item', array('makethumb'=>1), array('itemid'=>$blockitem['itemid']));
                                    $replacevalue = "http://".$alldomain[$serverid]['name']."/".$thumbpath;
	...
	foreach($itemlist as $value) {
		if($value) {
			...
			$inserts[] = "('$value[itemid]', '$bid', '$value[itemtype]', '$value[id]', '$value[idtype]', '$value[title]',
				 ...
				 '$value[fields]', '$value[displayorder]', '$value[startdate]', '$value[enddate]', '$value[picflag]','$value[serverid]', '$value[makethumb]')";
		}
	}
	if($inserts) {
		DB::query('REPLACE INTO '.DB::table('common_block_item')."(itemid, bid, itemtype, id, idtype, title, url, pic, summary, showstyle, related, `fields`, displayorder, startdate, enddate, picflag,serverid, makethumb) VALUES ".implode(',', $inserts));
	}

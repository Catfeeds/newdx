 ...
 *
 *      $Id: function_threadsort.php 22550 2011-05-12 05:21:39Z monkey $
 */
	...
	if(in_array($option['type'], array('number', 'text', 'email', 'calendar', 'image', 'url', 'range', 'upload', 'range'))) {
		if($option['type'] == 'calendar') {
			...
			$showoption[$option['identifier']]['value'] = '<button type="button" class="pn" onclick="uploadWindow(function (aid, url){updatesortattach(aid, url, \''.$_G['setting']['attachurl'].'forum\', \''.$option['identifier'].'\')})"><span>'.($optionvalue['value'] ? lang('forum/misc', 'sort_update') : lang('forum/misc', 'sort_upload')).'</span></button>
				...
				'<input type="hidden" name="typeoption['.$option['identifier'].'][serverid]" value="'.$optionvalue['value']['serverid'].'" id="sortserverid_'.$option[identifier].'" tabindex="1" />'.
				'<input type="hidden" name="typeoption['.$option['identifier'].'][filename]" value="'.$optionvalue['value']['filename'].'" id="sortfilename_'.$option[identifier].'" tabindex="1" />'.
			...
			if(in_array($arrayoption['type'], array('radio', 'checkbox', 'select'))) {
				...
				if(empty($templatearray[$sortid])) {
					...
					if($imgoptiondata['aid']) {
						if($fid == $_G['config']['fids']['mountain']){
							$sortthread['value'] = getimagethumb(120 ,120 ,1 ,'forum/'.$imgoptiondata['filename'], $imgoptiondata['aid'], $imgoptiondata['serverid']);
						}else{
	...
	foreach($sortoptionarray as $optionid => $option) {
		if($id) {
			...
			if($sortoptionarray[$optionid]['type'] == 'radio') {
				...
				/*
				 * Add by zhanghongliang start
				 * @datetime 2012-02-01 15:35
				 */
				$aid = $_G['forum_optionlist'][$optionid]['value']['aid'];
				if($attachment = DB::fetch_first("SELECT * FROM ".DB::table('forum_attachment')." WHERE `aid` = {$aid}")) {
					$_G['forum_optionlist'][$optionid]['value']['url'] = ($attachment['remote']>0 ? $_G['setting']['ftp']['attachurl'] : ($attachment['serverid']>0 ? "http://".$alldomain[$attachment['serverid']]."/" : "/data/attachment/"))."forum/{$attachment['attachment']}";
                    $_G['forum_optionlist'][$optionid]['value']['serverid'] = $attachment['serverid'];
                    $_G['forum_optionlist'][$optionid]['value']['filename'] = $attachment['attachment'];
				}
				/*
				 * Add by zhanghongliang end
				 */
			} else {

...
//添加参数：$isMpicAuto,用于个人中心的mpic的更新
function album_update_pic($albumid, $picid=0, $isMpicAuto = false) {
	global $_G;
	$setarr = array();
	if($picid) {
...
//		$piccount = getcount('home_pic', array('albumid'=>$albumid, 'status' => '0'));
		$piccount = DB::result_first( "SELECT COUNT(*) FROM " . DB::table('home_pic') . " WHERE albumid={$albumid} and status=0 ".getSlaveID());
	...
	$query = DB::query("SELECT * FROM ".DB::table('home_pic')." WHERE albumid='$albumid' $wheresql " . getSlaveID());
	...
	if($pic['remote']) {
		...
		$picsource = pic_get($pic['filepath'], $from > 1 ? 'forum' : 'album', $pic['thumb'], $pic['remote'], 0,$pic['serverid']);
	} else {
        if($pic['serverid']>0){
            $picsource = pic_get($pic['filepath'], $from > 1 ? 'forum' : 'album', $pic['thumb'], $pic['remote'], 0, $pic['serverid']);
        }else{
    ...
    if($from > 1) {
		...
		//论坛发帖上传图片，改为upyun form方式，原来生成缩略图方法弃用，by zhangwenchu 20170120
		/*
		if($pic['serverid'] > 0 && !$isMpicAuto){
			require_once DISCUZ_ROOT."./source/plugin/attachment_server/attachment_server.class.php";
			$attachserver = new attachserver;
			$serverinfo = $attachserver->get_server_domain($pic['serverid'],'*');
			$attachserver->Thumb($serverinfo['domain'] , $serverinfo['api'] , ($from > 1 ? 'forum' : 'album')."/".$pic['filepath'] , "album/{$picdir}{$albumid}.jpg" , 120 , 120 , 2, 0, true);
			$setarr['pic'] = $picdir.$albumid.'.jpg';
		}
		*/

	$setarr['pic'] = $picdir.$albumid.'.jpg';
	$setarr['serverid']   = $pic['serverid'];
	$setarr['mpic'] 	  = ($from > 1 ? 'forum' : 'album').'/'.$pic['filepath'];
	$setarr['mserverid']  = $pic['serverid'];

	//这是系统自动更新，不更新updatetime
	if (!$isMpicAuto) {
...
function pic_save($FILE, $albumid, $title, $iswatermark = true, $catid = 0, $thumbpic = true) {
	global $_G, $space;
    ...
    $serverid = $upload->attach['serverid'] ? $upload->attach['serverid'] : 0;
...
/*结束*/
	if($_G['setting']['maxthumbwidth'] && $_G['setting']['maxthumbheight'] && $thumbpic) {
        ...
        if($serverid>0){
        	if($serverid < 5){
            	$attachserver->Thumb($serverinfo['domain'] , $serverinfo['api'] , 'album/'.$upload->attach['attachment'] , '' , $_G['setting']['maxthumbwidth'] , $_G['setting']['maxthumbheight'] , 1 , 1);
            }
        }else{
        /*结束*/
	...
	/*测试附件上传*/
	/*增加判定是否为附件服务器的上传*/
	/*2015-1-21此处修改，将不再生成相册140的缩略图，展示均用又拍缩图后缀*/
	$thumb = 0;
    if($serverid > 0 && $serverid < 5){
        $thumb = $attachserver->Thumb($serverinfo['domain'] , $serverinfo['api'] , 'album/'.$upload->attach['attachment'] , '' , 140 , 140 , 1, 0, true) ? 1 : 0;
    }
    	//$pic_remote = 0;
		/*增加附件服务器水印*/
		if ($iswatermark && $serverid>0) {
            /*已经在上传中整合水印功能，此处不需要处理*/
            //$attachserver->Watermark($serverinfo['domain'],$serverinfo['api'],'album/'.$upload->attach['attachment'],'album');
    	}
		/*结束水印*/
    	/*结束*/
	/*结束*/
...
function deletenewinvite($mid){
    global $_G;
    $minfo = DB::fetch_first("SELECT * FROM ".DB::table('plugin_invite_message')." WHERE mid = {$mid}");
    if($minfo){
    	$relations = DB::fetch_first("SELECT * FROM " . DB::table('plugin_invite_relation') . " WHERE mid = {$mid}");
    	DB::update('plugin_invite_relation', array('tousers' => str_replace("({$_G[uid]})", "", $relations['tousers'])), array('mid' => $mid));
        //$reads = DB::result_first("SELECT readuid FROM ".DB::table('plugin_invite_readed')." WHERE mid = {$mid}");
//        $reads = $reads ? $reads.','.$_G['uid'] : $_G['uid'];
//        DB::query("REPLACE INTO ".DB::table('plugin_invite_readed')."(`mid`,`uid`,`readuid`,`dateline`) VALUES({$mid} , {$minfo[authorid]} , '{$reads}' , {$minfo[dateline]})");
        memory('rm' ,'plugin_invite_cache_uid_'.$_G['uid']);
        memory('rm' ,'plugin_invite_cache_mid_reads_'.$mid);
    }
}
?>

 ...
 *
 *      $Id: function_debug.php 15648 2010-08-25 04:27:10Z monkey $
 */
//$_config['debug']='discuz'; url: &debug=discuz
...
function debugmessage($ajax = 0) {
	...
	$ajaxhtml = 'data/'.$debugfile.'_ajax.htm';
	if($ajax) {
	...
	file_put_contents(DISCUZ_ROOT.'./'.$ajaxhtml, '<style>body,table { font-size:12px; }table { width:90%;border:1px solid gray; }</style><a href="javascript:;" onclick="location.href=location.href">Refresh</a><br />');
	foreach($sqlw as $k => $v) {
		$sqlw[$k] = $k.': '.$v;
	}
...
elseif(isset($_GET[\''.$viewcachek.'\'])) {
	...
	if(!isset($_GET[\'c\'])) {
		...
		while($names = DB::fetch($query)) {
			echo \'<a href="'.$debugfile.'?'.$viewcachek.'&c=\'.$names[\'cname\'].\'" target="_blank" style="float:left;width:200px">\'.$names[\'cname\'].\'</a>\';
		}
	} else {
		loadcache($_GET[\'c\']);
...
elseif(isset($_GET[\''.$mysqlplek.'\'])) {
	...
	while($row = DB::fetch($query)) {
		...
		echo \'<tr><td><a href="'.$debugfile.'?P&Id=\'.$row[\'Id\'].\'">[Kill]</a></td><td>&nbsp;\'.implode(\'&nbsp;</td><td>&nbsp;\', $row).\'&nbsp;</td></tr>\';
	}
	echo \'</table>\';
	exit;
}
...
EOF;
	...
	$debug .= '		
		...
		($_G['adminid'] == 1 ?
			'<img src="../static/image/common/arw_r.gif" /><a href="'.$debugfile.'?'.$phpinfok.'" target="_blank">phpinfo()</a>'.
			'<img src="../static/image/common/arw_r.gif" /><a href="'.$debugfile.'?'.$mysqlplek.'" target="_blank">MySQL Processlist</a>'.
			'<img src="../static/image/common/arw_r.gif" /><a href="'.$debugfile.'?'.$viewcachek.'" target="_blank">View Cache</a>'.
	...
	$debug .= $sqldebug.'';
	$debug .= '</ol></div>'.
		'<div id="__debug_c_4" style="display:none"><iframe id="sqldebug_ajax" name="sqldebug_ajax" src="../'.$ajaxhtml.'" frameborder="0" width="100%" height="800"></iframe></div>'.
	...
	echo '<iframe src="'.$fn.'" name="_debug_iframe" id="_debug_iframe" style="border-top:1px solid gray;overflow-x:hidden;overflow-y:auto" width="100%" height="70" frameborder="0"></iframe><div id="_debug_div"></div><iframe name="_debug_initframe" id="_debug_initframe" style="display:none"></iframe>';
}

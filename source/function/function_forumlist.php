...
function forum(&$forum) {
	...
	$forum['moderators'] = moddisplay(ForumOption::groupmanageroption($forum['fid'],'',0), $_G['setting']['moddisplay'], !empty($forum['inheritedmod']));
...
function getcacheinfo($tid) {
	...
	if(file_exists($cache['filename'])) {
		...
		/*已经改为memcache来存储页面缓存，因此去掉自动创建文件夹*/
		/*if(!is_dir($fulldir)) {
			dmkdir($fulldir);
		}*/
	}
...
function get_forumimg($imgname,$serverid='') {
	global $_G;
	if($imgname) {
		...
		if(isset($parse['host'])) {
			...
			if($_G['forum']['status'] != 3) {
                ...
                /*结束*/
                $serverid = empty($serverid) ? (($imgtype=='icon') ? $_G['forum']['serverid_icon'] : $_G['forum']['serverid_banner']) : $serverid;
				$imgpath = ($serverid > 0 ? "http://".$domainall[$serverid]."/" : $_G['setting']['attachurl']).'group/'.$imgname;
			}
		}
		return $imgpath;
	}
...
function get_redis_views($tid,$type){
    ...
    if($myredis->connected){
        /*$allbak = $myredis->obj->keys($type."_doing_bak_???");*/
        $view = $myredis->hashget($type."_number",$tid);
        $view += $myredis->hashget($type."_doing",$tid);
        /*foreach($allbak as $bakkey){
            $view += $myredis->hashget($bakkey,$tid);
        }*/
    }
    return $view;
}

function urlthreetoone(){
	global $_G;
	$array=array("1"=>$_G['config']['host']['portal'],"2"=>$_G['config']['host']['forum'],"3"=>$_G['config']['host']['home']);
	// 得到请求的域名
	$host= $_SERVER['HTTP_HOST'];
	$realurl = 'http://'.$host.'/forum-'.$_G['gp_fid'].'-1.html';

	$relists = memory('get','plugin_domain_indomainset_1');
	if(!$relists){
		$query = DB::query("SELECT resaddress FROM ".DB::table('plugin_domainset')." where 1");
		while ($value = DB::fetch($query)) {
			$relists[]= 'http://'.$value['resaddress'];
		}
		memory('set','plugin_domain_indomainset_1',$relists,3600);
	}

	if(!in_array($realurl,$relists)){
		if(in_array($host,$array)){
			$lists = array();
			$lists = memory('get','plugin_domain_indomain_1');
			if(!$lists){
				$query = DB::query("SELECT domain,fid,typeid FROM ".DB::table('plugin_domain')." where 1");
				$i=0;
				while ($value = DB::fetch($query)) {
					if(empty($value['domain'])){
						continue;
					}
					$lists[$i]['1']= $value['domain'];
					$lists[$i]['2']= $value['fid'];
					$lists[$i]['3']= $value['typeid'];
					++$i;
				}
				memory('set','plugin_domain_indomain_1',$lists,3600);
			}
			$_G['gp_page'] = empty($_G['gp_page'])?'1':$_G['gp_page'];

			if($_G['gp_page']==1){
				foreach($lists as $key => $value){
					if(!isset($_G['gp_filter']) || $_G['gp_filter']=='typeid'){
						if(empty($_G['gp_typeid'])){
							if($lists[$key]['3']=='0'){
								if($lists[$key]['2']== $_GET['fid']){
									$newurl = $lists[$key]['1'].'.8264.com';
									header("HTTP/1.1 301 Moved Permanently");
									header("Location: http://$newurl");
								}
							}
						}else{
							if($lists[$key]['3']==$_G['gp_typeid']){
								if($lists[$key]['2']== $_GET['fid']){
									$newurl = $lists[$key]['1'].'.8264.com';
									header("HTTP/1.1 301 Moved Permanently");
									header("Location: http://$newurl");

...
function profile_show2($fieldid, $space=array()) {
	global $_G;

	if(empty($_G['cache']['profilesetting2'])) {
		gethdfieldsetting();
	}
	$field = $_G['cache']['profilesetting2'][$fieldid];
	if(empty($field) || !$field['available'] || in_array($fieldid, array('uid', 'birthmonth', 'birthyear', 'birthprovince', 'resideprovince'))) {
		return false;
	}
...
function showdistrict($values, $elems=array(), $container='districtbox', $showlevel=null) {
	...
	// 120308 wistar 禁止出现三四级菜单
	$showlevel = $showlevel <= 2 ? $showlevel : 2;
	$upids = array(0);
	...
	for($i=0;$i<$showlevel;$i++) {
		...
		$html .= '<select name="'.$elems[$i].'" id="'.$elems[$i].'" onchange="'.$jscall.'" tabindex="1" class="select_g">';
...
/**
 * 当使用表单助手时 使用此函数获取对应的html
 * @author gtl
 * @version 20161129 测试完毕准备上线
 * @version 20160110 测试完毕准备上线
 * @global array $_G
 * @param int $fieldid
 * @param array $space
 * @return string
 */
function profile_setting2($fieldid, $space=array()) {
    global $_G;
	if(empty($_G['cache']['profilesetting2'])) {
		gethdfieldsetting();
	}
	$field = $_G['cache']['profilesetting2'][$fieldid];
	$html = ''; //text #password# #file# textarea select radio #checkbox#

	if($field['formtype']=='textarea') {
		$html = "<textarea name=\"$fieldid\" id=\"$fieldid\" rows=\"3\" cols=\"40\" class=\"pt\" tabindex=\"1\">$space[$fieldid]</textarea>";
	} elseif($field['formtype']=='select') {
		$field['choices'] = unserialize($field['choices']);
		$html = "<select name=\"$fieldid\" tabindex=\"1\">";
		foreach($field['choices'] as $op) {
			$html .= "<option value=\"{$op['value']}\"".($op['value']==$space[$fieldid] ? 'selected="selected"' : '').">{$op['text']}</option>";
		}
		$html .= '</select>';
	} /*elseif($field['formtype']=='checkbox') {
		$field['choices'] = unserialize($field['choices']);
		...
		foreach($field['choices'] as $op) {
			$html .= ''
				."<label><input type=\"checkbox\" name=\"{$fieldid}[]\" tabindex=\"1\" value=\"{$op['value']}\"".(in_array($op, $space[$fieldid]) ? ' checked="checked"' : '')." class=\"pc\" />"
				."$op</label>&nbsp;&nbsp;";
		}
	}*/ elseif($field['formtype']=='radio') {
		$field['choices'] = unserialize($field['choices']);
		foreach($field['choices'] as $op) {
			$html .= ''
					."<label><input type=\"radio\" name=\"{$fieldid}\" tabindex=\"1\" value=\"{$op['value']}\"".($op['value'] == $space[$fieldid] ? ' checked="checked"' : '')." class=\"pc\" />"
					."{$op['text']}</label>&nbsp;&nbsp;";
		}
	} /*elseif($field['formtype']=='file') {
		$html = "<input type=\"file\" value=\"\" name=\"$fieldid\" tabindex=\"1\" class=\"pf\" style=\"height:26px;\" /><input type=\"hidden\" name=\"$fieldid\" value=\"$space[$fieldid]\" />";

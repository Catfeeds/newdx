	...
	if($pt){
		while($vall = DB::fetch($pt)) {
			...
			if (deletepost('pid='.$vall['pid'])) {
				if($post){
					$replies = DB::result_first("SELECT count(*) FROM ".DB::table('forum_post')." WHERE tid='$post[tid]'");
			...
			$uname = $_G['username'];
			$time = time();
			DB::query("INSERT INTO ".DB::table('plugin_oper_info')." (uid, username, dateline, type,fild,val) ".
					  "VALUE ($uid, '$uname', $time,'delete','pid',$vall[pid])");
		}
...
if ($_GET['del'] == 1 && $_GET['pid']) {
	...
	$time = time();
	DB::query("INSERT INTO ".DB::table('plugin_oper_info')." (uid, username, dateline, type, fild, val) ".
						  "VALUE ($uid, '$uname', $time,'delete','pid',$_GET[pid])");
        cpmsg('删除成功', $url, 'succeed');
    } else {
        cpmsg('删除失败', $url, 'error');
    }
...
$where = " WHERE p.first=0 AND p.fid=".$_G['config']['fids']['dianpu'];
$ppp = 50;
...
<div style="overflow:hidden;width:99%;clear:both;padding:5px;border-top:1px solid #deeffb;">
	...
	<div style="float:right;">
		<?php echo multi($count, $ppp, $page, ADMINSCRIPT."?action=plugins&operation=config&do=$pluginid&identifier=outshop&pmod=admincp_comments");?>
	</div>
</div>
</form>

...
if ($_GET['del'] == 1 && $_GET['id']) {
	...
	DB::query("delete from ".DB::table('plugin_jb_info')." WHERE id = {$_GET['id']}");
	cpmsg('操作成功', $url, 'succeed');
}

$count = DB::result_first("SELECT COUNT(*) FROM ".DB::table('plugin_jb_info')." where 1");
$query = DB::query("SELECT * FROM ".DB::table('plugin_jb_info').' where 1 LIMIT '.($page - 1)*$ppp.",$ppp");
showtableheader();
?>
<tr class="header"><th>编号</th><th>标题</th><th>内容</th><th>用户名</th><th>添加时间</th><th>来自</th><th>操作</th></tr>
...
<tr>
	...
	<td>
		<?php
		$dq = DB::fetch_first("SELECT * FROM ".DB::table('dianping_shop_info')." WHERE tid = '$value[tid]'");
		if($dq){
			echo '<a href="http://www.8264.com/dianpu-'.$dq['tid'].'" target="_blank">'.$dq['subject'].'</a>';
		}
		?>
	</td>
		<td>

		<a class="digest" <?php echo ($value['isshow']==0 ?"style='display:none'":""); ?> href="<?php echo $_SERVER['REQUEST_URI']; ?>&id=<?php echo $value['id']; ?>&isshow=1">通过</a>
...
<?php }
showtablefooter();
echo multi($count, $ppp, $page, ADMINSCRIPT."?action=plugins&operation=config&do=$pluginid&identifier=outshop&pmod=admin_jb");
?>

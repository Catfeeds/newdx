...
class threadplugin_auction {
	...
	function newthread_submit_end($fid, $tid) {
		...
		if($auc['auctionaid']) {
			...
			*/
			$attachment = DB::fetch_first("SELECT * FROM ".DB::table('forum_attachment')." WHERE aid='$auc[auctionaid]' AND uid='$_G[uid]' AND isimage='1'");
			...
			DB::query("UPDATE ".DB::table('forum_post')." SET attachment=2 WHERE pid='$pid'");

			$aucaid = 1;
			// $threadimage = DB::fetch_first("SELECT tid, pid, attachment, remote FROM ".DB::table('forum_attachment')." WHERE aid='$aid'");
			
			// setthreadcover(0, 0, $auc['auctionaid']);exit;
	...
	function editpost($fid, $tid) {
		...
		if(DB::result_first("SELECT COUNT(*) FROM ".DB::table('plugin_auction')." WHERE tid='$tid'")) {
			...
			if($auction['aid']) {
				// ------------------------------
				// $auctionatt = DB::fetch_first("SELECT remote,attachment,thumb,serverid FROM ".DB::table('forum_attachment')." WHERE aid='{$auction[aid]}'");
				...
				$auctionatt = DB::fetch_first("SELECT aid,remote,attachment,serverid FROM ".DB::table('forum_attachment')." WHERE aid='{$auction[aid]}'");
				...
				$url = ($auctionatt['remote'] ? $_G['setting']['ftp']['attachurl'] : ($auctionatt['serverid'] > 0 ? 'http://'.$attachserverlist[$auctionatt['serverid']].'/' : $_G['setting']['attachurl'])).'forum/';
				$auctionatt['attachment'] = $url.$auctionatt['attachment'];
	...
	function editpost_submit_end($fid, $tid) {
		...
		if(!DB::result_first("SELECT COUNT(*) FROM ".DB::table('plugin_auction')." WHERE tid='$tid'")) {
			...
			if($auc['auctionaid'] && DB::result_first("SELECT COUNT(*) FROM ".DB::table('forum_attachment')." WHERE aid='{$auc[auctionaid]}' AND uid='{$_G[uid]}'")) {
				$aid = DB::result_first("SELECT aid FROM ".DB::table('plugin_auction')." WHERE tid='$tid'");
				if($aid) {
					$attach = DB::fetch_first("SELECT tid, pid, attachment, thumb, remote, aid, serverid FROM ".DB::table('forum_attachment')." WHERE aid='$aid'");
					dunlink($attach);

...
if($operation == 'join') {
	...
	if(!submitcheck('confirmsubmit')) {
		...
		if($auction['extra'] == 1 && $auction['virtual'] == 1) {
			...
			$sql = "UPDATE ".DB::table('plugin_auction_message')." SET uid='{$_G[uid]}' WHERE tid='{$thread[tid]}' AND uid='' LIMIT 1";
...
//get_thread_by_tid($tid, '*', "AND special='127'");
function get_thread_by_tid($tid, $fields = '*', $addcondiction = ''){

	$ret = array();
	if(!is_numeric($tid)) {
		return $ret;
	}
	$ret = DB::fetch_first("SELECT $fields FROM ".DB::table('forum_thread')." WHERE tid='$tid' $addcondiction LIMIT 1");
	if(!is_array($ret)) {
		$ret = array();
	}
	return $ret;	

...
if(!empty($whitelist)){
    ...
    for($i=$nowpage;$i<$min;$i++){
        showtablerow('' ,array('class="td25"','','','') ,array(
            ...
            $whitelist[$i].'<a class="blue" href="admin.php?action=plugins&operation=config&do='.$pluginid.'&identifier=ipbanspider&pmod=history&searchip='.$whitelist[$i].'">历</a><b onclick="getmessage(\''.$whitelist[$i].'\')" class="blue">详</b>' , $redis->ZCARD($pre.'COMMON_IP_'.$whitelist[$i].'_zset') , '<b class="red" onclick="ajaxget(\'plugin.php?id=ipbanspider:ajaxget&m=white&v=list&other='.$whitelist[$i].'|whitelist|unsafe\',\'update\')">移出白名单</b>',
        ));
    }
}else{
    showtablerow('' ,array('','colspan="5"'),array('','当前没有白名单IP'));
}

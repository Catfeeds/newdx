<?php
	...
	$search=array();
	foreach (explode(' ', 'time1 time2 type tablename') as $item) {
	...
	$search['tablename'] = empty($search['tablename'])?'pre_plugin_domain':$search['tablename'];

	if($search['tablename']!=NULL){
		$where.=" AND tablename = '{$search['tablename']}'";
		$where.="GROUP BY 'tablename'";
	}
	$result = DB::query("SELECT time,type,tablename,content FROM ".DB::table(plugin_record)." WHERE 1 {$where}");
?>
<div>
	<br/>
	<table>
		<form action='' method='post'>
			...
			<td>&nbsp;</td>
			<td>SQL操作表<input type='text' name='tablename' value='<?php echo $search['tablename']; ?>' size='50'/></td>
			<td>(请输入表全名)</td>
			<td><input type='submit' value='查询' /></td>
			</tr>
			<tr align='center'><td colspan='6'>！！提示：不输入表名将检索整张表，速度慢，查询量大！建议输入表名查询！！</td></tr>
		</form>
	</table>
	<br/>
	<?php
		while($row=DB::fetch($result)){
			$where1=" AND tablename = '{$row['tablename']}'";
			$total_count = DB::result_first("SELECT count(*) FROM ".DB::table('plugin_record')." WHERE 1 {$where1}");
			$select_count = DB::result_first("SELECT count(*) FROM ".DB::table('plugin_record')." where type='SELECT' {$where1}");
			$replace_count = DB::result_first("SELECT count(*) FROM ".DB::table('plugin_record')." where type='REPLACE' {$where1}");
			$insert_count = DB::result_first("SELECT count(*) FROM ".DB::table('plugin_record')." where type='INSERT' {$where1}");
			$delete_count = DB::result_first("SELECT count(*) FROM ".DB::table('plugin_record')." where type='DELETE' {$where1}");
			$update_count = DB::result_first("SELECT count(*) FROM ".DB::table('plugin_record')." where type='UPDATE' {$where1}");

			echo "<table border='1' width='800'>";
			echo "<caption>{$row['tablename']}</caption>";
			echo "<tr align='center'><td>时间</td><td>SQL次数</td><td>INSERT</td><td>REPLACE</td><td>DELETE</td><td>UPDATE</td><td>SELECT</td></tr>";
			echo "<tr align='center'><td>{$search[time1]} 到 {$search[time2]}</td><td>{$total_count}</td><td>{$insert_count}</td><td>{$replace_count}</td><td>{$delete_count}</td><td>{$update_count}</td><td>{$select_count}</td></tr>";

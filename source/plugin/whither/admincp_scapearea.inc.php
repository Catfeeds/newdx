...
@session_start();
$prefix = 'plugin_whither_scapearea_';
...
$count = DB::result_first("SELECT count(*) FROM ".DB::table('mudidi_thread_ralation')." AS r
						  LEFT JOIN ".DB::table('mudidi_scapearea')." AS s ON r.sn = s.sn
						  $where");
...
<?php
showtableheader();
echo '<tr><td colspan="4"><a style="float:right;" href="/forum.php?mod=post&action=newthread&fid=433&type=scapearea" target="_blank">发布景区</a></td></tr>';
...
echo multi($count, $ppp, $page, ADMINSCRIPT."?action=plugins&operation=config&do=$pluginid&identifier=whither&pmod=admincp_scapearea");

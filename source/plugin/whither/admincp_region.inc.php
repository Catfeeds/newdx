...
$count = DB::result_first("SELECT count(*) FROM ".DB::table('mudidi_thread_ralation')." AS r
						   LEFT JOIN ".DB::table('mudidi_region')." AS s ON r.sn = s.sn
						   $where");
...
<?php
showtableheader();
echo '<tr><td colspan="4"><a style="float:right;" href="/forum.php?mod=post&action=newthread&fid=433&type=region" target="_blank">发布地区</a></td></tr>';
...
echo multi($count, $ppp, $page, ADMINSCRIPT."?action=plugins&operation=config&do=$pluginid&identifier=whither&pmod=admincp_region");

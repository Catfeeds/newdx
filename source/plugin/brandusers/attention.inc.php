...
if (!empty($_GET['tid']) && $_G['uid']) {
	...
	$attentions = array('likeit', 'using', 'wantuse');
	$query = DB::query("SELECT * FROM ".DB::table('dianping_brand_users')." WHERE uid = {$_G['uid']} AND tid = {$tid}");
	...
	if ($_GET['op'] == 'new') {
		if (!$_G['uid'] || !$tid || !$type || !in_array($type, $attentions)) {
			...
			DB::query("INSERT INTO ".DB::table('dianping_brand_users')." (uid, tid, type) VALUES ({$_G['uid']}, {$tid}, '{$type}')");
			if ($_POST['product_select']) {
				$query = DB::fetch_first("SELECT id FROM ".DB::table('dianping_brand_users')." WHERE uid = {$_G['uid']} AND tid = {$tid} AND type = '{$type}' LIMIT 1");
				$id = $query['id'];

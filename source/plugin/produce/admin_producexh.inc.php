...
if ($_GET['op']=='newop') {
	$include_path = dirname(__file__).'/admincp/biaoqian/newoption.php';
	include $include_path;exit;
}
...
$count = DB::result_first("SELECT COUNT(*) FROM ".DB::table('plugin_produce_info')." where type = 1");
$query = DB::query("SELECT t.*,i.* FROM ".DB::table('plugin_produce_info')." as i left join ".DB::table('forum_thread')." as t on t.tid = i.tid  where t.fid = 437 and type = 1 order by t.dateline desc
				   LIMIT ".($page - 1)*$ppp.", $ppp");
echo '<tr><td colspan="5"><a target="_blank" style="float:right;" href="'.$_G['config']['web']['forum'].'forum-post-action-newthread-fid-437-bq-1.html">新建产品型号</a></td></tr>';
?>
...
<tr>
	...
	<td>
	<a href="<?php echo $_G['config']['web']['forum']; ?>forum.php?mod=post&action=edit&fid=437&tid=<?php echo $value['tid']; ?>&pid=<?php echo $value['pid']; ?>&page=1&bq=1" target="_blank">编辑</a>
	</td>
</tr>
<?php endwhile; ?>
<?php showtablefooter(); ?>

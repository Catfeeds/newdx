...
if (!empty($_POST)) {	
	...
	foreach ($list as &$value) {		
		$attach = DB::fetch_first("SELECT * FROM ".DB::table('forum_attachment')." where aid=".$value);		
		if(getglobal('setting/ftp/on')) {
			if(ftpcmd('upload', 'forum/'.$attach['attachment']) && (!$attach['thumb'] || ftpcmd('upload', 'forum/'.$attach['attachment'].'.thumb.jpg'))) {
				$pic_remote = 1;				
				DB::query("UPDATE ".DB::table('forum_attachment')." SET remote ={$pic_remote} WHERE aid={$attach['aid']}");
			}else{
...
$where = " where p.remote=0 and p.isimage = 1 and p.picid = 0";

$ppp = 20;
$page = max(1, intval($_G['gp_page']));
$count = DB::result_first("SELECT COUNT(*) FROM ".DB::table('forum_attachment')." p".$where);

$posts = array();
$query = DB::query("SELECT p.* FROM ".DB::table('forum_attachment')." p$where".
...
<form action="" method="post" id="albumForm">
	...
	<tr>
		<td width="10%"><input type="checkbox" name="guide[]" class="guideCheckbox" value="<?php echo $pic['aid']; ?>" /><?php echo $pic['aid']; ?></td>
		<td><a href="<?php echo $_G['setting']['attachurl'].'forum/'.$pic['attachment']; ?>" target="_blank"><?php echo $pic['filename']; ?></a></td>
		<td><?php echo $pic['dateline']; ?></td>
		<td><?php echo $pic['attachment']; ?></td>		
	</tr>
	<?php endforeach; ?>	
	<?php showtablefooter(); ?>
	<div style="overflow:hidden;width:99%;clear:both;padding:5px;border-top:1px solid #deeffb;">
		...
		<div style="float:right;">
			<?php echo multi($count, $ppp, $page, ADMINSCRIPT."?action=plugins&operation=config&do=$pluginid&identifier=forumoption&pmod=attach_upload&op=forum");?>
		</div>
	</div>
</form>

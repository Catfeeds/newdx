...
class ForumOptionSkiing {
	...
	function getRegionData($id=0) {
	    ...
	    $arr=array();	
		$query = DB::query("SELECT * FROM ".DB::table('plugin_shop_region')." WHERE 1 $sql order by catid asc");							
	...
	function getSkiingList($condition, $orderby='t.dateline desc', $limit='0,20'){
		...
		if ($orderby){
			$sql .= " ORDER BY s.orderby desc,$orderby";	
		}		
		if ($limit) {
			$sql .= " LIMIT $limit";
		}
	...
	function getProvince($reg){
		if($reg){
			$dq = DB::fetch_first("SELECT * FROM ".DB::table('plugin_shop_region')." WHERE name = '$reg' limit 1");
			if($dq){
				return $dq;
			}			
		}
	...
	function getProvincebycatid($catid){
		if($catid){
			$dq = DB::fetch_first("SELECT * FROM ".DB::table('plugin_shop_region')." WHERE catid = '$catid' limit 1");
			if($dq){
				return $dq;
			}			
		}
	...
	function getRegionById($id){
		if($id){
			$region = DB::fetch_first("SELECT * FROM ".DB::table('plugin_shop_region')." WHERE catid = $id limit 1");		
			return $region;
		}	
	}
	...
	function getRegionListbySki($cbrand=null,$brand=null){
		$sql = '';
		$arr = array();		
		$sq="select catid,name from ".DB::table('plugin_shop_region')." WHERE upid=0";
		$query = DB::query($sq);							
		while ($row = DB::fetch($query)) {			
	...
	function deletSkiingInfo($condition) {
		...
		if($condition){
			...
			while ($value = DB::fetch($query)) {
				...
				$qy = DB::fetch_first("select * FROM ".DB::table('plugin_star_statistics')." WHERE typeid=$value[tid]");
				if($qy){
					DB::query("delete from ".DB::table('plugin_star_logs')." WHERE starid='$qy[starid]'");
					DB::query("delete from ".DB::table('plugin_star_statistics')." WHERE id='$qy[id]'");
				}			
			}		

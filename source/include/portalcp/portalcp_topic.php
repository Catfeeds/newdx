...
if($topicid) {
	...
	if($topic['cover']) {
        if($topic['serverid'] > 0){
            ...
            $domain = $attachserver->get_server_domain($topic['serverid']);
            $topic['cover'] = "http://".$domain."/".$topic['cover'];
        }else{
    		if($topic['picflag'] == '1') {
	...
	$topic['endtime_date'] = $topic['endtime'] ? date("Y-m-d H:i", $topic['endtime']) : '';
}
...
// 对模板文件进行排序 修改人:孟津 12.05.16 12:23
asort($tpls);

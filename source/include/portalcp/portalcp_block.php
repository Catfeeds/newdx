...
if($op == 'block') {
	...
	if($item['picflag'] == '1') {
		if($item['serverid']>0){
            ...
            $serverdomain = $attachserver->get_server_domain($item['serverid']);
		}
		$item['thumbpic'] = getimagethumb(80, 80, 2, $item['pic'], 0, $item['serverid']);
		$item['pic'] = $item['pic'] ? (($item['serverid']>0 ? "http://".$serverdomain."/" : $_G['setting']['attachurl']).$item['pic']) : '';
	...
	if(submitcheck('itemsubmit') || submitcheck('recommendsubmit') || submitcheck('verifydatasubmit') || submitcheck('managedatasubmit')) {
		...
		if($_FILES['pic']['tmp_name']) {
			$result = pic_upload($_FILES['pic'], 'portal', 0, 0, 2, false);
			...
			$item['serverid'] = intval($_POST['serverid']);
			$item['makethumb'] = 0;
		}

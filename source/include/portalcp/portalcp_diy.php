...
if (submitcheck('uploadsubmit')) {
	...
	if($upload->error()) {
		...
		if($attach['isimage']) {
            if($attach['serverid']==0){
                ...
                $attach['thumb'] = $attachserver->Thumb($alldomain[$attach['serverid']]['domain'] , $alldomain[$attach['serverid']]['api'] , 'portal/'.$attach['attachment'] , '' , 80 , 50 , 1) ? 1 : 0;
            }
		}
	...
	/**
	 * Backup Diy
	 * Add by MengJin 11-10-18 13:57
	 */
	/* cancel at 12-11-15 11:00
	if (@include_once(DISCUZ_ROOT."/source/plugin/components/backupdiy.php")) {
		backupdiy($template);
	}*/
...
if($op == 'blockclass') {
	...
	if (submitcheck('importsubmit')) {
		...
		if($upload->error()) {
            ...
            if($serverid > 0){	
                $filename = "http://".$alldomain[$serverid]['name']."/temp/".$attach['attachment'];
            }else{

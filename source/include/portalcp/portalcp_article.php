...
$op = in_array($_GET['op'], array('addpage', 'edit', 'delpage', 'delete', 'related', 'batch', 'pushplus', 'verify', 'duestate','stick')) ? $_GET['op'] : 'add';
...
//值得买增加过期管理，ajax, 20161012，zhangwenchu
if($catid == 867 || ($catid >= 958 && $catid <= 965)){
    //验证权限
    if($op == 'duestate' && $_G['groupid'] == '1'){
        if(!$aid){
            echo '0';die();
        }
        $state = $_GET['state'] ? '0' : '1';

        DB::update('portal_article_title', array('duestate'=>$state), "aid='{$aid}'");
...
if(submitcheck("articlesubmit", 0, $seccodecheck, $secqaacheck)) {
	...
	//catid=867，值得买增加领券链接、购买链接
	if($catid == '867' || ($catid >= 958 && $catid <= 965)){
		//领券链接
		$_G['gp_couponurl'] = str_replace('&amp;', '&', dhtmlspecialchars($_G['gp_couponurl']));
		//购买链接
		$_G['gp_buyurl'] = str_replace('&amp;', '&', dhtmlspecialchars($_G['gp_buyurl']));
	}
	...
	//catid==867，值得买，增加领券链接和购买链接
	if($setarr['catid'] == '867' || ($setarr['catid'] >= 958 && $setarr['catid'] <= 965)){
		$setarr['couponurl'] = $_G['gp_couponurl'];
		$setarr['buyurl'] = $_G['gp_buyurl'];
	}
	...
	if($_G['gp_conver']) {
		...
		/*增加附件服务器操作*/
		$setarr['serverid'] = intval($converfiles['serverid']);
		/*结束*/
	}
	if($_G['gp_mconver']) {
		$mconverfiles = unserialize(stripcslashes($_G['gp_mconver']));
		$setarr['mpic'] = addslashes($mconverfiles['pic']);
		$setarr['mthumb'] = intval($mconverfiles['thumb']);
		$setarr['mremote'] = intval($mconverfiles['remote']);
		$setarr['mserverid'] = intval($mconverfiles['serverid']);
	}
	...
	if(empty($article)) {
        ...
        // 2012-07-03 新增 jianghong  用于查询是否为品牌商，是的话将更新文章模块
        require_once DISCUZ_ROOT.'./source/plugin/forumoption/include.php';
        $type_module = "articles";
        $search_arr = array($setarr['title']);
        $produce_info = $forumOption->get_produce_id_arr($search_arr);
    	if($produce_info){
            for($b_i=0;$b_i<count($produce_info);$b_i++){
                $newthread = array('otherid' => $aid,
                                'id' =>$produce_info[$b_i],
                                'type' =>$type_module,
                                'dateline' =>time()
                                );
                $forumOption->produce_update_cache_all($newthread,'brand',1);   //将发的新帖子，与关联的品牌放入总缓存表中永久保存
                $forumOption->update_cache_produce($produce_info[$b_i],$type_module); //更新缓存队列
           }
        }
        //以上为2012-07-03新增
		if($table) {
		...
		$setarr['id']=$_G['gp_from_id'];
		DB::update('portal_article_title', $setarr, array('aid' => $aid));

		// @chang 修改 用于查询是否为品牌商，是的话将更新文章模块
        require_once DISCUZ_ROOT.'./source/plugin/forumoption/include.php';
        $type_module = "articles";
    	...
    	if($produce_info){
            for($b_i=0;$b_i<count($produce_info);$b_i++){
				...
				$cache_brand=DB::fetch_first("SELECT * from ".DB::table('plugin_cache_brand_otherindex')." WHERE id=".$produce_info[$b_i]." and otherid=".$aid." and type='".$type_module."'");
                if($cache_brand==0){
					$forumOption->produce_update_cache_all($newthread,'brand',1);   //修改帖子时，与关联的品牌放入总缓存表中永久保存
	                $forumOption->update_cache_produce($produce_info[$b_i],$type_module); //更新缓存队列
                }
          	}
        }
	...
	// insert keyword
	DB::query("DELETE FROM ".DB::table('plugin_searchkey_keylink')." WHERE aid='$aid'");
	if($_POST['keywords']) {
		$keywordsarr = array();
		$keywordsarr = array_map('intval', $_POST['keywords']);
		$keywordsarr = array_unique($keywordsarr);
		$keywordsarr = array_filter($keywordsarr);
		if($keywordsarr) {
			$replaces = array();
			$displayorder = 0;
			foreach($keywordsarr as $relate) {
				$replaces[] = "('$aid', '$relate', '$displayorder')";
				$displayorder++;
			}
			if($replaces) {
				DB::query("REPLACE INTO ".DB::table('plugin_searchkey_keylink')." (aid,kid,displayorder) VALUES ".implode(', ', $replaces));
			}
		}
	}
...
if ($op == 'delpage') {
	...
	$currenturl = base64_decode($_GET['currenturlEncode']);
    $currenturl = str_replace('http://www.8264.com/', '', $currenturl);

	if(!$aid) {
		showmessage('article_edit_nopermission',$currenturl);
	}

	//验证权限
    if($_G['groupid'] == '1'){
       
        if($_GET['stickstate']==1){
        	$stickstate = $_G['timestamp'];
        }else{
        	$stickstate = 0;
        }

        DB::update('portal_article_title', array('stickstate'=>$stickstate), "aid='{$aid}'");
        showmessage('操作成功！',$currenturl);
    }else{
    	showmessage('权限不足！',$currenturl);
    }
	...
	$opts = array();
	// 必选参数
	$opts['save-key'] = '/portal/{year}{mon}/{day}/{hour}{min}{sec}{random}{.suffix}';   // 保存路径

...
if ($op == 'start') {
	...
	$list = array();
	if($count) {
		$query = DB::query("SELECT * FROM ".DB::table('home_pic')." WHERE $wheresql ORDER BY dateline DESC LIMIT $start,$perpage " . getSlaveID());
		while ($value = DB::fetch($query)) {
...
if (submitcheck('uploadsubmit')) {
	...
	$picurl = pic_get($uploadfiles['filepath'], 'album', $uploadfiles['thumb'], $uploadfiles['remote'],1,$uploadfiles['serverid']);

...
if($_GET['op'] == 'delete') {
	if(submitcheck('deletesubmit')) {
		...
		$url = $_G['gp_from'] ? "home.php?mod=space&uid={$space['uid']}&do=share&view=me&from=space" : dreferer();
 		showmessage('指定的分享已删除', $url, array('sid' => $sid), array('showdialog'=>1, 'showmsg' => true, 'closetime' => true));
	}
	...
	switch ($type) {
		...
		case 'blog':
			...
			$query = DB::query("SELECT b.*,bf.pic,bf.serverid,bf.message,bf.hotuser FROM ".DB::table('home_blog')." b
			...
			if($blog['pic']) {
				$arr['image'] = pic_cover_get($blog['pic'], $blog['picflag'],$alldomain[$blog['serverid']]);
		...
		case 'album':
			...
			$arr['image'] = pic_cover_get($album['pic'], $album['picflag'],$alldomain[$album['serverid']]);
		...
		case 'pic':
			...
			$arr['image'] = pic_get($pic['filepath'], 'album', $pic['thumb'], $pic['remote'],1,$pic['serverid']);
		...
		case 'thread':
			...
			$attachment = DB::fetch_first('SELECT attachment, isimage, thumb, remote,serverid FROM '.DB::table('forum_attachment')." WHERE tid='$id' AND isimage IN ('1', '-1') LIMIT 0,1");
			if($attachment) {
				$arr['image'] = pic_get($attachment['attachment'], 'forum', $attachment['thumb'], $attachment['remote'], 1,$attachment['serverid']);
		...
		case 'article':
			...
			if($article['pic']) {
				$arr['image'] = pic_get($article['pic'], 'portal', $article['thumb'], $article['remote'], 1,$article['serverid']);
	...
	if(submitcheck('sharesubmit', 0, $seccodecheck, $secqaacheck)) {
		...
		showmessage('分享成功', dreferer(), array('sid' => $sid), array('showdialog'=>1, 'showmsg' => true, 'closetime' => true));
	}

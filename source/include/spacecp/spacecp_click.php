...
if(empty($click)) {
	showmessage('click_error', '', array(), array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
}
...
switch ($idtype) {
	...
	default:
		$idtype = 'blogid';
		$sql = "SELECT b.*, bf.hotuser,bf.serverid FROM ".DB::table('home_blog')." b
...
if(!$item = DB::fetch($query)) {
	showmessage('click_item_error', '', array(), array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
}
...
if($_GET['op'] == 'add') {
	...
	if($item['uid'] == $_G['uid']) {
		showmessage('click_no_self', '', array(), array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
	}
	...
	if($value = DB::fetch($query)) {
		showmessage('click_have', '', array(), array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
	}
	...
	switch ($idtype) {
		...
		case 'picid':
			...
			$fs['images'] = array(pic_get($item['filepath'], 'album', $item['thumb'], $item['remote'],1,$item['serverid']));
	...
	showmessage('click_success', dreferer(), array('idtype' => $idtype, 'id' => $id, 'clickid' => $clickid), array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));

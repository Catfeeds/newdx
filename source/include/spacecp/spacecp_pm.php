...
if($_GET['op'] == 'getpmuser') {
	...
	if(submitcheck('pmsubmit')) {
		...
		if($return > 0) {
			...
			if(!empty($username)) {
				showmessage(count($users) ? 'message_send_result' : 'do_success', 'home.php?mod=space&do=notice&type=smessage', array('users' => implode(',', $users), 'succeed' => count($newusers)));
			} else {
				showmessage('do_success', 'home.php?mod=space&do=pm&subop=view&pmid='.$_GET['pmid'].'&touid='.$_GET['touid'].'&daterange='.$_GET['daterange'].'#bottom', array('pmid' => $return . ($_G['gp_news'] == 1 ? "&news=1" : "")), array('msgtype' => 3, 'showmsg' => false));
			}
		} else {
	...
	include template('home/space_pm_node_new');
...
include_once template('home/spacecp_pm' . (!$_GET['op'] ? '_new' : ''));

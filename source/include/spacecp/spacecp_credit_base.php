...
if($_G['gp_op'] == 'base') {
	...
	if(submitcheck('transfersubmit')) {
		...
		$msgparam = !submitcheck('confirm') ? array('msgtype' => 2) : array('showdialog' => 3, 'showmsg' => true, 'closetime' => true);
	...
	$_list 	  = array();
	$_listfid = array();
	$query = DB::query("SELECT * FROM ".DB::table('common_credit_rule')." ORDER BY rid ASC");
	
	$dealedfids = array();
	
	while($value = DB::fetch($query)) {
		if((empty($_G['gp_fid']) || in_array($value['action'], array('digest', 'post', 'reply', 'getattach', 'postattach'))) && ($value['extcredits2'] || $value['extcredits5'])) {			
			$_list[$value['action']] = $value;
		}
		
		//某些板块的特定规则
		if (!$value["fids"]) {continue;}
		$value["fids"] = explode(',', $value["fids"]);
		
		foreach ($value["fids"] as $fid) {
			
			$tempaction = $value['action'].$fid;
		
			if (empty($fid)) {continue;}
			$flist = unserialize(DB::result_first("SELECT creditspolicy FROM ".DB::table('forum_forumfield')." WHERE fid='{$fid}'"));
			foreach($flist as $v) {
				if (!$v['extcredits2'] && !$v['extcredits5']) {continue;}				
				$dealedfids[$v['action'].$fid] = $v;
			}
			if ($dealedfids[$tempaction]) {
				$_listfid[$fid][$tempaction] = $dealedfids[$tempaction];
				$_listfid[$fid][$tempaction]['rulename']  = $forums[$fid]['name'];
				$_listfid[$fid][$tempaction]['rulename'] .= $value['action'] == 'post' ? '发主题帖' : ($value['action'] == 'reply' ? '回帖' : '');
			}
		}				
	}
	
	//按指定顺序显示
	$arrOrder = array('post', 'reply', 'publishblog', 'comment', 'sendpm', 'getattach');
	foreach ($arrOrder as $v) {
		$list[$v] = $_list[$v];
		unset($_list[$v]);
	}
	$list = array_merge($list, $_list);
	foreach ($_listfid as $value) {

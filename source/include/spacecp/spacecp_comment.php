...
if(submitcheck('commentsubmit', 0, $seccodecheck, $secqaacheck)) {
	...
	if($waittime > 0) {
		showmessage('operating_too_fast', '', array('waittime' => $waittime), array('showdialog' => 3, 'showmsg' => true, 'closetime' => true));
	}

	$message = getstr($_POST['message'], 0, 1, 1, 2);
	if(mb_strlen($message, 'gbk') < 2) {
		showmessage('评论内容不能少于2个字符', '', array(), array('showdialog' => 3, 'showmsg' => true, 'closetime' => true));
	}
	if(mb_strlen($message, 'gbk') > 500) {
		showmessage('评论内容不能超过500个字符', '', array(), array('showdialog' => 3, 'showmsg' => true, 'closetime' => true));
	}
	...
	if($cid) {
		...
		if($comment && $comment['authorid'] != $_G['uid']) {
			...
			//日志的回复，更新样式 by shuaiquan 20141215
//			$message = $idtype == 'blogid' ? addslashes("回复{$comment['author']}：").$message : addslashes("<div class=\"quote\"><blockquote><b>".$comment['author']."</b>: ".getstr($comment['message'], 150, 0, 0, 2, 1).'</blockquote></div>').$message;
	...
	switch ($idtype) {
		...
		case 'picid':
			...
			$pic = DB::fetch($query);
			if(empty($pic)) {
				showmessage('view_images_do_not_exist', '', array(), array('showdialog' => 3, 'showmsg' => true, 'closetime' => true));
			}
		...
		case 'blogid':
			...
			$blog = DB::fetch($query);
			if(empty($blog)) {
				showmessage('view_to_info_did_not_exist', '', array(), array('showdialog' => 3, 'showmsg' => true, 'closetime' => true));
			}
			...
			if(!empty($blog['noreply'])) {
				showmessage('do_not_accept_comments', '', array(), array('showdialog' => 3, 'showmsg' => true, 'closetime' => true));
			}
			if($blog['target_ids']) {
		...
		case 'sid':
			...
			if(empty($share)) {
				showmessage('sharing_does_not_exist', '', array(), array('showdialog' => 3, 'showmsg' => true, 'closetime' => true));
			}
			...
			break;
		default:			
			showmessage('non_normal_operation', '', array(), array('showdialog' => 3, 'showmsg' => true, 'closetime' => true));
			break;
	}

	if(empty($tospace)) {
		showmessage('space_does_not_exist', '', array(), array('showdialog' => 3, 'showmsg' => true, 'closetime' => true));
	}
	...
	if(isblacklist($tospace['uid'])) {
		showmessage('is_blacklist', '', array(), array('showdialog' => 3, 'showmsg' => true, 'closetime' => true));
	}
	...
	switch ($_POST['idtype']) {
		...
		case 'picid':
			...
			$fs['images'] = array(pic_get($pic['filepath'], 'album', $pic['thumb'], $pic['remote'],1,$pic['serverid']));
...
if($_GET['op'] == 'edit') {
	...
	if(submitcheck('editsubmit')) {
		...
		$message = getstr($_POST['message'], 0, 1, 1, 2);
		if(mb_strlen($message, 'gbk') < 2) {
			showmessage('评论内容不能少于2个字符', '', array(), array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
		}
		if(mb_strlen($message, 'gbk') > 500) {
			showmessage('评论内容不能超过500个字符', '', array(), array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
		}
			
		$message = censor($message);
		$comment_status = censormod($message) ? 1 : 0;

		DB::update('home_comment', array('message'=>$message, 'status'=>$comment_status), array('cid' => $comment['cid']));
		showmessage('评论编辑完成', dreferer(), array('cid' => $comment['cid']), array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
	}
	...
	if(submitcheck('deletesubmit')) {
		...
		if(deletecomments(array($cid))) {
			showmessage('删除成功', dreferer(), array('cid' => $cid), array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
		} else {
			showmessage('no_privilege');
		}
	}

...
if($_GET['op'] == 'delete') {
	...
	if(empty($thevalue) || $thevalue['uid'] != $_G['uid']) {
		showmessage('favorite_does_not_exist', '', array(), array('return' => true));
	}

	if(submitcheck('deletesubmit')) {
		DB::query('DELETE FROM '.DB::table('home_favorite')." WHERE favid='$favid'");
		showmessage('已取消收藏', 'home.php?mod=space&uid='.$_G['uid'].'&do=favorite&view=me&type='.$_GET['type'].'&quickforward=1', array('favid' => $favid,'success'=>1), array('showdialog'=>1, 'showmsg' => true, 'closetime' => true));
	}
	...
	if(empty($idtype) || empty($title)) {
		showmessage('favorite_cannot_favorite', '', array(), array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
	}

	if(DB::result_first('SELECT * FROM '.DB::table('home_favorite')." WHERE uid='$_G[uid]' AND idtype='$idtype' AND id='$id'")) {
		showmessage('favorite_repeat', '', array(), array('showdialog' => 1, 'showmsg' => true, 'closetime' => true));
	}
	$description = '';
	...
	if(submitcheck('favoritesubmit')) {
		...
		showmessage('收藏成功', dreferer(), array('id'=>$id,'success'=>1), array('showdialog' => true, 'closetime' => true));
	}
}

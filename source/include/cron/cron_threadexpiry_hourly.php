 ...
 *
 *      $Id: cron_threadexpiry_hourly.php 17480 2010-10-19 09:11:13Z zhengqingpeng $
 */
...
while($expiry = DB::fetch($query)) {
	...
	if(in_array($expiry['action'], array('UES'))) {
...
if($actionarray) {
	foreach($actionarray as $action => $tids) {
		...
		switch($action) {
			...
			case 'UES':
				DB::query("UPDATE ".DB::table('forum_thread')." SET displayorder='0', typedorder='0' WHERE tid IN ($tids) && displayorder!='-1'", 'UNBUFFERED');	//增加地方版置顶解除 2015.1.14

...
//发帖数和回复数
$space['threads']  = getthreadbyuid($space, $notfids, $infids);		
$space['posts']    = getpostbyuid($space, $notfids, $infids);

//相册数
$wheresql  = "uid={$space['uid']}";
$wheresql .= !ckperm() ? ' and friend = 0 ' : '';	
$space['albums'] = DB::result_first("SELECT COUNT(*) FROM ".DB::table('home_album')." WHERE {$wheresql} " . getSlaveID());

//日志数
$wheresql  = "uid={$space['uid']}";
$wheresql .= !ckperm() ? ' and friend = 0 ' : '';
$privacyWhere   = !ckperm() ? ' and status = 0 and friend = 0 ' : '';
$space['blogs'] = DB::result_first("select count(*) from " . DB::table('home_blog') . " where uid={$space['uid']} {$privacyWhere} " . getSlaveID());

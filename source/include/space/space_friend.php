...
$view = $_GET['view'] == 'fans' ? $_GET['view'] : 'followee';

ckstart($start, $perpage);
if($view == 'fans') {
	$navtitle = "{$space['username']},粉丝列表";
	
	$url = 'home.php?mod=space&uid=' . $uid . '&do=friend&view=fans';
	require_once libfile('function/friend');
	$searchkey = $_GET['searchname'];
	$perpage = 10;
	if($searchkey) {

		$searchkey = dhtmlspecialchars($searchkey);
		$fans_count = get_fans_count_by_username($uid, $searchkey);
		if($fans_count) {
			$url .= "&searchname={$searchkey}";
			$fans = get_fan_users_by_username($uid, $page, $perpage, $searchkey);
		}
	}
	else {
		$fans_count = get_fans_count($uid);
		if($fans_count) {
			$fans = get_fan_users($uid, $page, $perpage);
		}
	}
	if($fans_count) {
		$fans_uid_str = implode(',', array_keys($fans));
		if($fans_uid_str) {
			$residences = get_user_residence($fans_uid_str);
			$slogons = get_user_slogon($fans_uid_str);
			$fans_statitic = get_user_statistic($fans_uid_str);
		}
		$multi =  multi($fans_count, 10, $page, $url);
		include template('home/fans/fans');
	} elseif(! $searchkey) {
		$recommended_users = get_recommonded_users($_G['clientip'], $_G['uid']);
		include template('home/fans/recommendation');
	}	
	exit;
}
elseif($view == 'followee') {
	$navtitle = "{$space['username']},关注列表";
	
	require_once libfile('function/friend');
	$searchkey = $_GET['searchname'];
	$perpage   = $space['self'] ? 18 : 10;
	$url = 'home.php?mod=space&uid=' . $uid . '&do=friend';
	if($searchkey) {
		$searchkey = dhtmlspecialchars($searchkey);
		$url .= "&searchname={$searchkey}";
		$following_count = get_following_count_by_username($uid, $searchkey);
		if($following_count) {
			$myfollowing = get_following_users_by_username($uid, $page, $perpage, $searchkey);
			$myfollowing_uid_str = implode(',', array_keys($myfollowing));
		}
	}
	else {
		$following_count = get_following_count($uid);
		if($following_count) {
			$myfollowing = get_following_users($uid, $page, $perpage);
			$myfollowing_uid_str = implode(',', array_keys($myfollowing));
		}		
	}

	if(! $following_count && ! $searchkey) {
		$recommended_users = get_recommonded_users($_G['clientip'], $_G['uid']);
		include template('home/fans/recommendation');
	}
	else {
		$multi =  multi($following_count, $perpage, $page, $url);
		if($space['self']) {
			$groups = friend_group_list();
		}
		$privacy = $space['privacy'] ? $space['privacy'] : get_privacy($uid);
		
		if($space['self']) {
			include template('home/fans/followee');
		} else {			
			if ($following_count) {
				$fans_count    = $following_count;
				$fans		   = $myfollowing;
				$fans_statitic = get_user_statistic($myfollowing_uid_str);
				$residences    = get_user_residence($myfollowing_uid_str);
				$slogons       = get_user_slogon($myfollowing_uid_str);				
			}
			$isfollow      = true;
			include template('home/fans/fans');
		}		
...
$view_name = $view == 'followee' ? '关注' : '粉丝';

$navtitle = lang('space', 'sb_friend', array('who' => $space['username'], 'view' => $view_name));
...
/*	$invite_thread_count = $_G['member']['newinvite'];
	...
	//include_once template("diy:home/space_friend_self");
} else {

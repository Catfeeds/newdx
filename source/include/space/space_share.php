...
if($id) {
	...
	if(!IS_ROBOT) {
		...
		if($count) {
			$query = DB::query("SELECT * FROM ".DB::table('home_comment')." WHERE $csql id='$id' AND idtype='sid' ORDER BY dateline desc LIMIT $start,$perpage");
	...
	include_once template("home/space_share_view");
	...
	include_once template("home/space_share_list");
}

 ...
 *
 *      $Id: post_editpost.php 22550 2011-05-12 05:21:39Z monkey $
 */
...
//限制点评访问
if($_G['fid']==$_G['config']['fids']['brand']||$_G['fid']==$_G['config']['fids']['equipment']||$_G['fid']==$_G['config']['fids']['skiing']){	
...
} elseif ((! $_G['forum']['ismoderator'] || ! $_G['group']['alloweditpost'] || (in_array($orig['adminid'], array(
	1,
	2,
	3)) && $_G['adminid'] > $orig['adminid'])) && ! (($_G['forum']['alloweditpost'] || $orig['invisible'] == -3) && $isorigauthor)) {
...
if (! submitcheck('editsubmit')) {
	...
	if ($isfirstpost) {
		...
		if ($thread['special'] == 1 && ($_G['group']['alloweditpoll'] || $thread['authorid'] == $_G['uid'])) {
			...
			$activity['expiration']    = dgmdate($activity['expiration'], 'Y-m-d H:i');
			...
			//新的活动内容
			$postinfo['activitymessage']      = explode('[----]', $postinfo['message']);	
			$postinfo['activitymessagecount'] = count($postinfo['activitymessage']);
	...
	if ($special == 4 && $activity['aid'] && ! empty($imgattachs['used']) && is_array($imgattachs['used'])) {
		...
		/*增加活动编辑时读取标题的附件服务器域名*/
		if ($activityattach['serverid'] > 0) {
			require_once DISCUZ_ROOT."./source/plugin/attachment_server/attachment_server.class.php";
			$attachserver = new attachserver;
			$domain = $attachserver->get_server_domain($activityattach['serverid'], '*');
			$activityattach['url'] = "http://".$domain['name']."/forum/";
			if(is_array($domain['api'])){
				$activityattach['attachment'] .= getUpYun( true, 200, 200, 1, 0, 1, true, $_G['config']['cdns']['ids'][$activityattach['serverid']]);
			}
		}
		/*结束*/
	}
	...
	if ($forumOption) {
		include $forumOption->hookScriptPost3();
	} else {
...
//			}
			...
			if ($thread['special'] == 3) {
				...
				$price = $thread['price'] < 0 && ! $thread['special'] ? ($isorigauthor || ! $price ? -1 : $price) : ($_G['group']['maxprice'] ? ($price <= $_G['group']['maxprice'] ? ($price > 0 ? $price : 0) : $_G['group']['maxprice']) : ($isorigauthor ? $price : $thread['price']));
			...
			if ($thread['special'] == 1 && ($_G['group']['alloweditpoll'] || $isorigauthor) && ! empty($_G['gp_polls'])) {
				...
				if ($thread['price'] > 0 && $thread['price'] != $_G['gp_rewardprice']) {
					...
					DB::update('common_credit_log', array('extcredits'.$_G['setting']['creditstransextra'][2] => $realprice), array(
						'uid' => $thread['authorid'],
						'operation' => 'RTC',
						'relatedid' => $_G['tid']));
				}
				...
				$activity['clubid']   = setClubList($activity['clubname'], $_G['gp_clubid']);
...
//				}
				...
				DB::update('forum_activity', $activity, "tid={$_G['tid']}");
				...
				foreach ($_G['forum_optiondata'] as $optionid => $value) {
					if ($_G['forum_optionlist'][$optionid]['type'] == 'image') {
						...
						if ($newsortaid && $newsortaid != $_G['gp_oldsortaid'][$identifier]) {
							$attach = DB::fetch_first("SELECT attachment, thumb, remote, aid, serverid,tid FROM ".DB::table('forum_attachment')." WHERE aid='{$_G['gp_oldsortaid'][$identifier]}'");
							DB::query("DELETE FROM ".DB::table('forum_attachment')." WHERE aid='{$_G['gp_oldsortaid'][$identifier]}'");
							$attach['have_threadpic'] = 1;
							dunlink($attach);
			...
			if ($forumOption) {
				$forumOption->hookScriptPost4($subject);
			}
		} else {
		...
		($_G['group']['allowpostattach'] || $_G['group']['allowpostimage']) && ($_G['gp_attachnew'] || $_G['gp_attachdel'] || $special == 2 && $_G['gp_tradeaid'] || $special == 4 && $_G['gp_activityaid'] || $isfirstpost && $sortid) && updateattach($postattachcredits, $_G['tid'], $pid, $new_img_edit, $_G['gp_attachdel'], $_G['gp_attachupdate'], $orig['authorid']);
		...
		//新的活动内容
		$_G['gp_activitymessagecount'] = intval($_G['gp_activitymessagecount']);
		if ($special == 4 && ($activity['credit'] == 0 && $_G['gp_activitymessagecount'] > 1)) {	
			$message  = $_G['gp_activitymessage'][1] ? censor( dhtmlspecialchars( trim( $_G['gp_activitymessage'][1] ) ) ) : '暂无内容';
			$message .= $_G['gp_activitymessage'][2] ? '[----]' . censor( dhtmlspecialchars( trim( $_G['gp_activitymessage'][2] ) ) ) : '[----]暂无内容';
			$message .= $_G['gp_activitymessage'][3] ? '[----]' . censor( dhtmlspecialchars( trim( $_G['gp_activitymessage'][3] ) ) ) : '[----]暂无内容';
		}
		...
		$parr = array(
			...
			'fid' => $_G['fid'],
			'name' => $_G['forum']['name'],  // lxp 20140124
			'subject' => $subject,
			'message' => $message,
			'ip' => $_G['clientip']);
		...
		if ($thread['special'] == 3 && $isfirstpost) {
			...
			DB::delete('common_credit_log', array(
				'uid' => $thread['authorid'],
				'operation' => 'RTC',
				'relatedid' => $_G['tid']));
		}
		...
		if (! empty($_G['gp_delete']) && $isfirstpost) {
			...
			if ($isfirstpost && $modnewthreads) {
				...
				if($special == 4 && $isfirstpost){
					...
					$extraparam['wechatshare']   = $activity['timediff'] <= 3 ? urlencode("http://wei.zaiwai.com/?d=forum&c=activity&m=activity_info&tid={$_G['tid']}") : urlencode("{$_G['config']['web']['mobile']}thread-{$_G['tid']}-1.html");					
					$extraparam['info_activity'] = "编辑成功";
					...
					if ($_G['uid'] == $thread['authorid']) {						
						$extraparam['tid']  = $_G['tid'];							

						//获得关注分组
						require_once libfile('function/friend');
						$extraparam['groups'] = friend_group_list();					
						$extraparam['groups_select'] = array();
						$tempgroup = DB::result_first("SELECT groups FROM ".DB::table('common_share_follow_group')." WHERE tid={$extraparam['tid']} " . getSlaveID());
						if ($tempgroup) {
							$tempgroup = trim($tempgroup, ',');
							$tempgroup = explode(',', $tempgroup);
							foreach ($tempgroup as $v) {
								$extraparam['groups_select'][$v] = 1;
							}
						}
					}
				}
				...
				//当是旅游攻略板块时，页面提示
				if($_G['fid'] == 52 && $isfirstpost){
					$extraparam['header'] = false;
					$extraparam['refreshtime']   = 1000;
					$extraparam['wechatshare']   = "{$_G['config']['web']['mobile']}thread-{$_G['tid']}-1.html?hasfid={$_G['fid']}";
					$extraparam['info_activity'] = "编辑成功";
					$extraparam['fid']           = 52;
				}
				...
				showmessage('post_edit_succeed', $redirecturl, $param, $extraparam);
			}
		}
	}
}

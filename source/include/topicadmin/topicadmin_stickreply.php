...
if(!submitcheck('modsubmit')) {
	...
	if($_G['gp_stickreply']) {
		...
		if($_G['fid']==$_G['config']['fids']['zbfx']){
			...
			ForumOptionProduce::calPostRankEvent($_G['tid'],8);
			$thread=ForumOptionProduce::getTheardBytId($_G['tid']);
			ForumOptionProduce::calPublisherRankEvent($thread['authorid'],$thread['author'],8);
		}
	} else {
		foreach($sticktopiclist as $pid => $postnum) {
		...
		if($_G['fid']==$_G['config']['fids']['zbfx']){
			require_once DISCUZ_ROOT.'/source/plugin/forumoption/produce.php';
			ForumOptionProduce::calPostRankEvent($_G['tid'],9);
			$thread=ForumOptionProduce::getTheardBytId($_G['tid']);
			ForumOptionProduce::calPublisherRankEvent($thread['authorid'],$thread['author'],9);
		}
	}

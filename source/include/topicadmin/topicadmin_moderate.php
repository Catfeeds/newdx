...
switch($frommodcp) {
	...
	case '2':
		...
		break;
	default:
		$typeid = $_G['gp_typeid'] ? '&typeid='.$_G['gp_typeid'] : '';	//适配地区等二级域名删帖后跳转地址404的问题
		$_G['referer'] = $operation != 'delete' ? $_G['gp_redirect'] : $_G['config']['web']['forum'].'forum.php?mod=forumdisplay&fid='.$_G['fid'].$typeid.(!empty($_G['gp_listextra']) ? '&'.rawurldecode($_G['gp_listextra']) : '');
		break;
}
...
if(!submitcheck('modsubmit')) {
	...
	if($_G['gp_optgroup'] == 1 && $single) {
        if(!empty($threadlist[$_G['tid']]['typedorder'])){
            $threadlist[$_G['tid']]['displayorder']=9;
        }
	...
	if($operation == 'stick') {
		$nobanlevel = false;
		/*
		if($_G['forum']['threadtypes']['fatherid'][$_G['gp_typeid']] > 0) {
			$mytypeids = explode(',', DB::result_first("SELECT typeids FROM " . DB::table('forum_moderator') . " WHERE uid = {$_G[uid]}"));
			var_dump($mytypeids);
			if(! in_array($_G['forum']['threadtypes']['fatherid'][$_G['gp_typeid']], $mytypeids)) {
				$nobanlevel = true;
			}
		}else{*/
			if(DB::result_first("SELECT onlytype FROM " . DB::table('forum_moderator') . " WHERE uid = {$_G[uid]}") == 1) {
				$nobanlevel = true;
			}
		//}
	}
	//$is_manager_forumorpart = $_G['gp_other'];
	...
	if(empty($operations)) {
		...
		foreach($operations as $operation) {
			...
			if($operation == 'stick') {
				...
				//增加判断，将地方版块sticklevel = 9排除在无效操作外
				if($sticklevel < 0 || ($sticklevel > 3 && $sticklevel !=9 )|| ($sticklevel > $_G['group']['allowstickthread'] && $sticklevel !=9 )) {
				...
				$displaytype = ($sticklevel==9) ? "typedorder=1" : "displayorder='$sticklevel' , typedorder=0";

				DB::query("UPDATE ".DB::table('forum_thread')." SET ".$displaytype.", moderated='1' WHERE tid IN ($moderatetids)");
				...
				foreach($threadlist as $thread) {
					...
					//增加删除帖子时，删除缓存中的相应的帖子2012-07-06 by jianghong
					$pid_list = array();
					$query_pid = DB::query("SELECT pid FROM ".DB::table('forum_post')." WHERE tid IN ($moderatetids)");
					while($valus = DB::fetch($query_pid)){
					    $pid_list[] = $valus['pid'];
					}
					$pid_list = implode(",",$pid_list);
					if(DB::result_first("SELECT count(*) FROM ".DB::table('plugin_cache_brand_index')." WHERE pid in ($pid_list)")>0){
					    DB::query("UPDATE ".DB::table('plugin_cache_brand_index')." SET displayorder=-1  WHERE pid in ($pid_list)");
					}
					//以上为2012-07-06新增
				}
				...
				if($_G['forum']['recyclebin']) {
					...
					deletefeed($moderatetids, 'thread');
					...
					foreach($threadtableids as $tableid) {
						...
						$query = DB::query("SELECT COUNT(*) AS threads, SUM(replies)+COUNT(*) AS posts FROM ".DB::table("forum_thread_$tableid")." WHERE fid='{$_G['fid']}' " . getSlaveID());
						while($row = DB::fetch($query)) {
...
function checkexpiration($expiration, $operation) {
	...
	if(!empty($expiration) && in_array($operation, array('recommend', 'stick', 'digest', 'highlight', 'close'))) {
		...
		if(dgmdate($expiration, 'Ymd') <= dgmdate(TIMESTAMP, 'Ymd') || ($expiration > TIMESTAMP + 86400 * 366)) {

<?php

/**
 *      [Discuz!] (C)2001-2099 Comsenz Inc.
 *      This is NOT a freeware, use is subject to license terms
 *
 *      $Id: lang_template.php 17263 2010-09-28 02:21:13Z zhangguosheng $
 *
 *      This file is automatically generate
 */

$lang = array (

//common/block_thread.htm
  'activity_about_member' => '剩余名额',
  'activity_already' => '已参加人数',
  'additional' => '附加',
  'debate_opponent' => '反方',
  'debate_square' => '正方',
  'thread_poll' => '投票',

  'uid' => 'UID',

//common/block_userinfo.htm
  'credits' => '积分',
  'login' => '登录',

//common/editor.htm
  'e_fontoptions' => '"仿宋_GB2312", "黑体", "楷体_GB2312", "宋体", "新宋体", "微软雅黑", "Trebuchet MS", "Tahoma", "Arial", "Impact", "Verdana", "Times New Roman"',
  'editor_increase' => '加大编辑框',
  'editor_narrow' => '缩小编辑框',
  'post_autosave_restore' => '恢复数据',
  'post_check_length' => '字数检查',
  'post_savedata' => '保存数据',
  'post_topicreset' => '清除内容',

//common/faq.htm
  'all' => '全部',
  'faq' => '帮助',
  'faq_search_nomatch' => '对不起，没有找到匹配结果',
  'homepage' => '首页',
  'keyword_faq' => '关键字为“<span class="xi1">$keyword</span>”的帮助',
  'open' => '展开',
  'search' => '搜索',

//common/footer.htm
  'certification' => '认证',
  'close' => '关闭',
  'focus_hottopics' => '站长推荐',
  'focus_show' => '查看',
  'mail_notifier' => '邮件提醒',
  'modify_avatar' => '修改头像',
  'password_security' => '密码安全',
  'personal_profile' => '个人资料',
  'privacy_filter' => '隐私筛选',
  'thread_groupupgrade' => '距离下一级还需',
  'time_now' => 'GMT{$_G[timenow][offset]}, {$_G[timenow][time]}',
  'usergroup' => '用户组',

//common/header.htm
  'activation' => '激活',
  'admincp' => '管理中心',
  'default' => '默认',
  'forum_manager' => '{$_G[setting][navs][2][navname]}管理',
  'friends' => '好友',
  'invite' => '邀请',
  'login_invisible_mode' => '隐身',
  'login_normal_mode' => '在线',
  'login_switch_invisible_mode' => '切换在线状态',
  'logout' => '退出',
  'my_center' => '我的中心',
  'open_diy' => '打开 DIY 面板',
  'pm_center' => '短消息',
  'portal_manage' => '门户管理',
  'qmenu_register' => '注册新用户，开通自己的个人中心',
  'remind' => '提醒',
  'setup' => '设置',
  'switch_narrow' => '切换到窄版',
  'switch_wide' => '切换到宽版',
  'task' => '任务',
  'task_doing' => '进行中的任务',
  'visit_my_space' => '访问我的空间',

//common/header_diy.htm
  'e_redo' => '重做',
  'e_undo' => '撤销',
  'header_add_frame' => '添加框架',
  'header_add_module' => '添加模块',
  'header_clearall' => '清空',
  'header_clearall_desc' => '清空页面上所在DIY数据',
  'header_diy' => '自定义',
  'header_export' => '导出',
  'header_export_desc' => '导出当前页面中所有DIY数据',
  'header_frame' => '框架',
  'header_import' => '导入',
  'header_import_desc' => '将DIY数据导入到当前页面',
  'header_module' => '模块',
  'header_more_actions' => '更多操作',
  'header_perview' => '预览',
  'header_perview_desc' => '预览DIY的效果',
  'header_restore_backup' => '恢复备份',
  'header_restore_backup_desc' => '恢复备份',
  'header_start' => '开始',
  'header_update' => '更新',
  'header_update_desc' => '更新当前页面所有模块的数据',
  'save' => '保存',
  'style' => '风格',

//common/invite.htm
  'invite_all_friend' => '全部好友',
  'invite_friend' => '邀请好友',
  'invite_orderby_friend' => '按好友分组查找',
  'invite_orderby_name' => '按好友用户名查找',
  'invite_send' => '发送邀请',
  'invite_still_choose' => '还能选择',
  'selected' => '已选',
  'unit' => '个',
  'unselected' => '未选',

//common/preview.htm
  'preview_footertext' => '版权及页脚信息',
  'preview_highlightlink' => '高亮链接',
  'preview_lighttext' => '浅色文字',
  'preview_link' => '链接文字',
  'preview_midtext' => '中等文本',
  'preview_msg' => '<a href="javascript:;" class="xi2">Crossday Discuz! Board</a> 社区系统(简称 <stong class="xw1 xi1">Discuz!</stong>)是一个采用 PHP 和 MySQL 等其他多种数据库构建的高效建站解决方案。',
  'preview_smalltext' => 'SMALL FONT',
  'preview_text' => '普通文本',
  'preview_threadtext' => '主题列表字体',
  'table_header' => '表头',

//common/pubsearchform.htm
  'advanced_search' => '高级搜索',
  'album' => '相册',
  'article' => '文章',
  'blog' => '日志',
  'enter_content' => '请输入搜索内容',
  'hot_search' => '热搜',
  'search_friends' => '搜索好友',
  'users' => '用户',
  'baidu' => '百度搜索',

//common/report.htm
  'confirms' => '确定',
  'input_message1' => '还可输入',
  'input_message2' => '个字符',
  'report' => '举报',
  'report_reason' => '举报理由',

//common/seccheck.htm
  'seccode' => '验证码',
  'seccode_change' => '换一个',
  'secqaa' => '验证问答',

//common/seditor.htm
  'e_bold' => '文字加粗',
  'e_code' => '代码',
  'e_forecolor' => '设置文字颜色',
  'e_image' => '图片',
  'e_quote' => '引用',
  'e_url' => '添加链接',

//common/showmessage.htm
  'attach_forward' => '如果 {$refreshsecond} 秒后下载仍未开始，请点击此链接',
  'board_message' => '提示信息',
  'cancel' => '取消',
  'message_forward' => '如果你的浏览器没有自动跳转，请点击此链接',
  'message_go_back' => '[ 点击这里返回上一页 ]',

//common/simplesearchform.htm
  'search_this_forum' => '搜索本版',
  'search_user' => '搜索用户',

//common/stat.htm
  'basic_situation' => '基本概况',
  'forum_stat' => '版块统计',
  'forum_stat_detail' => '子版块统计',
  'manage_team' => '管理团队',
  'member_list' => '会员列表',
  'mod_works' => '管理统计',
  'trend' => '趋势统计',

//common/userabout.htm
  'app_add' => '添加$_G[setting][navs][5][navname]',
  'app_management' => '管理$_G[setting][navs][5][navname]',
  'e_attach_del' => '删除',
  'e_attach_mediacode' => '添加附件媒体播放代码',
  'e_attach_url' => '添加附件地址',
  'readperm' => '阅读权限',
  'update' => '更新',
  'author' => '作者',
  'more' => '更多',
  'publish' => '发布',
  'delete' => '删除',
  'submit' => '提交',
  'anonymous' => '匿名',
  'guest' => '游客',
  'upload' => '上传',
  'e_attach_insert' => '点击文件名添加到帖子内容中',
  'e_attach_inserturl' => '，"attach://" 开头的附件地址支持任何人下载请谨慎添加',
  'e_img_albumlist' => '相册图片',
  'e_img_attach' => '图片列表',
  'e_img_height' => '高(可选)',
  'e_img_insertphoto' => '点击图片添加到帖子内容中',
  'e_img_inserturl' => '请输入图片地址',
  'e_img_width' => '宽(可选)',
  'e_img_www' => '网络图片',
  'thread_activity' => '活动',
  'thread_debate' => '辩论',
  'thread_reward' => '悬赏',
  'thread_trade' => '商品',
  'edit' => '编辑',
  'return' => '返回',
  'discuzcode' => 'Discuz 代码',
  'member' => '会员',
  'panel_login' => '登录管理面板',
  'panel_login_password' => '密码',
  'panel_login_username' => '用户名',
  'panel_notice_login' => '首次进入管理面板或空闲时间过长, 你输入密码才能进入。如果密码输入错误超过 5 次，管理面板将会锁定 15 分钟。',
  'e_attach' => '附件',
  'e_attach_title' => '添加附件',
  'e_audio' => '音乐',
  'e_audio_title' => '添加音乐',
  'e_autotypeset' => '自动排版',
  'e_center' => '居中',
  'e_code_title' => '添加代码文字',
  'e_flash' => 'Flash',
  'e_flash_title' => '添加 Flash',
  'e_font' => '字体',
  'e_fontname' => '设置字体',
  'e_fontsize' => '设置文字大小',
  'e_free' => '添加免费信息',
  'e_hide' => '添加隐藏内容',
  'e_hr_title' => '分隔线',
  'e_image_title' => '添加图片',
  'e_italic' => '文字斜体',
  'e_left' => '居左',
  'e_orderedlist' => '排序的列表',
  'e_paste' => '粘贴',
  'e_quote_title' => '添加引用文字',
  'e_removeformat' => '清除文本格式',
  'e_right' => '居右',
  'e_size' => '大小',
  'e_smilies' => '表情',
  'e_smilies_title' => '添加表情',
  'e_table' => '添加表格',
  'e_underline' => '文字加下划线',
  'e_unlink' => '移除链接',
  'e_unorderedlist' => '未排序列表',
  'e_video' => '视频',
  'e_video_title' => '添加视频',
  'username' => '用户名',
  'smilies' => '表情',
  'stats' => '站点统计',
  'recommend' => '模块推送',
  'trade' => '商品',
  'manage' => '管理',
  'not_enough' => '不足',
  'moderate_need' => '待审核',
  'portal' => '门户',
  'reward_answer' => '我来回答',
  'favorite' => '收藏',
  'choose_please' => '请选择',
  'create' => '创建',
  'option_required' => '必选项',
  'finished' => '完成',
  'thread' => '帖子',
  'ignore_all' => '忽略全部',
  'pass_all' => '全部通过',
  'create_on' => '创建于',
  'add' => '添加',
  'activity' => '活动',
  'blockdata_recommend' => '模块推送',
  'share' => '分享',
  'article_push' => '生成文章',
  'debate' => '辩论',
  'doing' => '记录',
  'online_member' => '在线成员',
  'my_space' => '我的中心',
  'task_applies' => '人气',
  'top_orderby_blogs' => '按日志总数',
  'top_orderby_credits' => '按积分总数',
  'top_orderby_posts' => '按发帖总数',
  'top_orderby_uid' => '按注册时间',
  'magic' => '道具',
  'medals' => '勋章',
  'notice' => '提醒',
  'poll' => '投票',
  'group_expiry_type_ext' => '扩展用户组',
  'reward' => '悬赏',
  'task_admins' => '管理人员',
  'task_applicants' => '已有 $task[applicants] 位会员参与此任务',
  'task_applies_full' => '人数已满',
  'task_apply_condition' => '申请此任务所需条件',
  'task_applyagain' => ' 后可以再次申请',
  'task_applyagain_now' => '现在可以再次申请',
  'task_complete' => '已完成',
  'task_complete_condition' => '完成此任务所需条件',
  'task_complete_on' => '完成于',
  'task_detail' => '任务详情',
  'task_endtime' => '当前任务下线时间为 $task[endtime]，过期后你将不能申请此任务',
  'task_general_users' => '普通会员',
  'task_group_nopermission' => '你所在的用户组无法申请此任务',
  'task_lose_on' => '失败于',
  'task_numlimit' => '人次上限',
  'task_period_day' => '每 $task[period] 天允许申请一次',
  'task_period_hour' => '每隔 $task[period] 小时允许申请一次',
  'task_period_month' => '每月 $task[period] 日允许申请一次',
  'task_period_week' => '每周 $periodweek 允许申请一次',
  'task_reapply' => '后可以重新申请',
  'task_reapply_now' => '现在可以重新申请',
  'task_relatedtask' => '必须完成指定任务',
  'task_reward' => '奖励',
  'task_done' => '已完成的任务',
  'task_failed' => '失败的任务',
  'task_new' => '新任务',
  'task_nodoing' => '暂无进行中的任务，请到新任务中申请！',
  'task_nonew' => '暂无新任务，周期性任务完成后可以再次申请，敬请关注！',
  'select_all' => '全选',
  'increase' => '增加',
  'friend_top' => '排行榜',
  'connect_config_newpassword_comment' => '你目前使用的是QQ账号绑定本站，你可以在这里设置独立密码，只有设置了独立密码后本站需要填写密码的相应功能才可使用',
  'email' => 'Email',
  'security_question' => '安全提问(未设置请忽略)',
  'security_question_0' => '安全提问',
  'security_question_1' => '母亲的名字',
  'security_question_2' => '爷爷的名字',
  'security_question_3' => '父亲出生的城市',
  'security_question_4' => '你其中一位老师的名字',
  'security_question_5' => '你个人计算机的型号',
  'security_question_6' => '你最喜欢的餐馆名称',
  'security_question_7' => '驾驶执照最后四位数字',
  'group_expiry_time' => '到期时间',
  'login_id' => '账号',
  'login_password' => '密码',
  'login_permanent' => '自动登录',
  'group_expiry_type' => '类型',
  'group_expiry_type_main' => '主用户组',
  'login_permanent' => '记住我的登录状态',
  'user_center' => '个人中心',
  'security_a' => '答案',
  'security_q' => '安全提问',
  'forgotpw' => '找回密码',
  'image' => '图片',
  'remote' => '远程',
  'find' => '查找',
  'credit_ranking' => '积分排行',
  'auction_ranking' => '竞价排行',
  'close_ranking_note' => '管理员已关闭竞价,暂时无法继续上榜',
  'credit_is_not_number' => '你所填写的{$extcredits[$creditid][title]}不是一个合法数值',
  'credit_title_error' => '你所填写的{$extcredits[$creditid][title]}不能小于1',
  'credit_title_message' => '你的当前{$extcredits[$creditid][title]}为: "+maxCredit+"，请填写一个小于该值的数字',
  'current_ranking' => '当前排名',
  'friend_need_help' => '要帮助的好友',
  'friend_num_ranking' => '好友数排行',
  'friend_top_note' => '排行榜公告',
  'handsel' => '赠送',
  'handsel_bid' => '赠送竞价',
  'help_friend_in_top' => '帮助好友来上榜',
  'i_ranking' => '我也要上榜',
  'increase_bid' => '增加竞价',
  'make_persistent_efforts' => '再接再厉',
  'max_char_ranking' => '最多50个汉字，会显示在榜单中',
  'my_ranking_declaration' => '我的上榜宣言',
  'not_exceed' => '不要超过自己的',
  'ranking_message_0' => '你现在还没有上榜。让自己上榜吧，这会大大提升你的主页曝光率。',
  'ranking_message_1' => '竞价{$extcredits[$creditid][title]}越多，竞价排名越靠前，你的主页曝光率也会越高；',
  'ranking_message_2' => '上榜用户的主页被别人有效浏览一次，其竞价{$extcredits[$creditid][title]}将减少1个(恶意刷新访问不扣减)。',
  'self_current_credit' => '你当前的积分',
  'self_current_friend_num' => '你当前的好友数',
  'top_100_update' => '下面列出的为排行前100名，数据每 $cache_time 分钟更新一次。',
  'your_current_bid' => '自己当前的竞价',
  'contactus' => '联系我们',

	'connect_header_login' => 'QQ登录',
	'connect_header_login_tip' => '只需一步，快速开始',
	'connect_register_by' => '你也可以通过',
	'connect_or' => '或者',
	'connect_publish_feed' => '将此主题发送到QQ空间动态',
	'connect_publish_t' => '将此主题转播到腾讯微博',
	'connect_register_complete' => '完善一点信息，马上开始浏览',
	'connect_register_bind' => '绑定我的账号',
	'connect_register_has_id' => '已有账号？',
	'connect_register_new_username' => '输入新用户名',
	'connect_register_used_usernames' => '常用名',
	'connect_register_available_usernames_count' => '$available_username_count 个可用',
	'connect_register_email' => '邮箱',
	'connect_register_birthday' => '生日',
	'connect_register_sex' => '性别',
	'connect_register_noid' => '没有账号？',
	'connect_register_new' => '注册新账号',
	'connect_register_avatar' => '头像',
	'connect_register_sex_male' => '男',
	'connect_register_sex_female' => '女',
	'connect_register_sex_secret' => '保密',
	'connect_register_public_email' => '公开邮箱地址',
	'connect_register_use_qzone_avatar' => '使用QQ空间头像',
	'connect_register_agree' => '同意服务条款',
	'connect_register_must_agree' => '你必须同意服务条款后才能注册',
	'connect_register_finish' => '完成，继续访问',
	'connect_register_bind_window_button' => '绑定账号',
	'connect_register_bind_title' => '输入已有的本站账号，将与你的QQ账号进行绑定',
	'connect_register_bind_success' => '你的账号与QQ账号绑定成功',
	'connect_register_bind_need_inactive' => '你的账号还未激活，请先激活后再绑定QQ账号',
	'connect_config' => 'QQ绑定',
	'connect_config_bind' => '点击按钮，立刻绑定QQ账号',
	'connect_config_bindinfo' => '你已将本站账号 <strong>{$_G[member][username]}</strong> 与QQ号码绑定',
	'connect_config_bind_setting' => '绑定设置',
	'connect_config_title' => '使用QQ账号绑定本站，你可以...',
	'connect_config_desc_title1' => '用QQ账号轻松登录',
	'connect_config_desc_content1' => '无需记住本站的账号和密码，随时使用QQ账号密码轻松登录',
	'connect_config_newpassword_comment' => '你目前使用的是QQ账号绑定本站，你可以在这里设置独立密码，只有设置了独立密码后本站需要填写密码的相应功能才可使用',
	'connect_config_desc_title2' => '发表主题同时推送到QQ空间',
	'connect_config_desc_content2' => '将你在论坛发表的主题推送到QQ空间动态，让好友全方面了解自己',
	'connect_config_desc_title3' => '把本站精彩内容分享到QQ空间',
	'connect_config_desc_content3' => '每一个精彩内容绝不放过，简单快捷的将论坛主题分享到QQ空间',
	'connect_config_profile' => '个人资料',
	'connect_config_link_name' => 'QQ绑定设置',
	'connect_config_default_pulish_feed_title' => '发表主题时默认发送动态到QQ空间',
	'connect_config_default_pulish_t_title' => '发表主题时默认转播到腾讯微博',
	'connect_config_save' => '保存设置',
	'connect_config_newpassword' => '为单独登录站点创建登录密码',
	'connect_config_unbind' => '解除已绑定账号？',
	'connect_config_set_password' => '设置本站密码？',
	'connect_config_unbind_desc1' => '你当前是使用QQ账号绑定到 {$_G[setting][bbname]}',
	'connect_config_unbind_desc2' => '为了你可以继续使用本站账号 <strong>$_G[member][username]</strong>，请设置本站登录密码',
	'connect_config_unbind_desc3' => '你确认要解除与本站账号的绑定关系？',
	'connect_config_unbind_desc4' => '你需要使用QQ账号登录才能解除账号绑定关系，请点击<a href="member.php?mod=logging&action=logout&formhash={FORMHASH}" class="xi2">这里退出</a>后',
	'connect_config_unbind_desc5' => '重新使用QQ账号登录',
	'connect_config_bind_confirm' => '确认解除',
	'connect_config_password_comment' => '登录本站使用',
	'connect_config_set_password_desc' => '因为安全因素考虑，在操作兑换积分、版主管理等行为时需要设置本站密码',
	'connect_post_by_readperm' => '具有特殊阅读权限帖子无法发送QQ空间动态',
	'connect_post_by_price' => '具有售价的帖子无法发送QQ空间动态',
	'connect_post_by_invisible_forum' => '该板块的帖子无法发送QQ空间动态',
	'connect_post_unbind' => '绑定QQ账号，轻松分享到QQ空间与腾讯微博',
	'connect_viewthread_share_to_qzone' => '分享到QQ空间',
	'connect_viewthread_share_to_t' => '转播到腾讯微博',
	'connect_createpassword' => '<b>创建密码</b>',
	'connect_uin_limit' => '你注册的本站账号数量已超过最大限制<br />请<a href="javascript:;" onclick="connect_switch(1)" class="xi2">点击这里</a>绑定已有本站账号',

//mobile(IN_MOBILE)
  'your_mobile_browser' => '您现在使用手机浏览器 - ',
  'go_to_mobile' => '进入手机版',
  'to_be_continue' => '继续访问不再提示',

  'contact_us_with_qq' => '1919501975',
  'contact_us_with_id' => 'say哈喽'
);

?>

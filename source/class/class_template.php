 ...
 *
 *      $Id: class_template.php 22986 2011-06-13 02:38:25Z monkey $
 */
...
class template {
	...
	function parse_template($tplfile, $templateid, $tpldir, $file, $cachefile) {
		...
		@unlink(DISCUZ_ROOT.$cachefile);

		$diycontent = DB::result_first("SELECT diycontent FROM ".DB::table('common_diy')." WHERE diyname = 'data/diy/{$file}.htm'");
		if ($diycontent) {
			$template = base64_decode($diycontent);
		}else{
		...
		$headerexists = preg_match("/{(?:sub)?template\s+([\w\/]+?header(?:_.*)?)\}/", $template, $_templatematchs);
		...
		$headeradd = $headerexists ? "hookscriptoutput('$basefile', '$_templatematchs[1]');" : '';
	...
	function languagevar($var) {
		...
		if(!isset($langvar[$var])) {
			...
			/*
			 * 为whither插件添加language
			 * 2011-09-16 13:52 by MengJin
			 */
			if ($this->file == 'pubinfo') {
				@include DISCUZ_ROOT.'./source/language/forum/lang_template.php';

...
EOT;
	...
	function write_error_log($message) {
		...
		$user = '<b>User:</b> uid='.intval($uid).'; IP='.$ip.'; RIP:'.$_SERVER['REMOTE_ADDR'];
		$uri = 'Request: '.htmlspecialchars(discuz_error::clear($_SERVER['REQUEST_URI']));
		$message = "{$time}[--log--]{$message}[--log--]{$hash}[--log--]{$user} {$uri}\n";
		...
		if($myredis->connected){
			...
			if ($myredis->obj->exists($file.'_'.$yesterday)){
				if($myredis->obj->renameNx($file.'_'.$yesterday, $file.'_'.$yesterday.'_doing')){
			...
			$myredis->obj->rPush($file.'_'.$yearmonth, $message);
		}else{
			if($fp = @fopen($file, 'rb')) {
				$lastlen = 10000;
				$maxtime = 60 * 10;

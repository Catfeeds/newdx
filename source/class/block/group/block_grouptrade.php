 ...
 *
 *      $Id: block_grouptrade.php 22550 2011-05-12 05:21:39Z monkey $
 */
...
class block_grouptrade {
	...
	function getdata($style, $parameter) {
		...
		while($data = DB::fetch($query)) {
			...
			if($data['aid']) {
				$pic = DB::fetch_first("SELECT attachment, remote,serverid FROM ".DB::table('forum_attachment')." WHERE aid='$data[aid]'");
				$one['pic'] = 'forum/'.$pic['attachment'];
				$one['picflag'] = $pic['remote'] ? '2' : '1';
				$one['serverid'] = $pic['serverid'];
			}
			$list[] = $one;
		}

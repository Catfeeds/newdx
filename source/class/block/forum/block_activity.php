 ...
 *
 *      $Id: block_activity.php 22550 2011-05-12 05:21:39Z monkey $
 */
...
class block_activity {
	...
	function fields() {
		return array(
					...
					'area' => array('name' => 'µØÇø', 'formtype' => 'text', 'datatype' => 'text'),
				);
	}
	...
	function getdata($style, $parameter) {
		...
		$sqlfrom .= " INNER JOIN `".DB::table('common_district')."` dt ON dt.id=a.area";
		$query = DB::query("SELECT a.*, t.tid, t.subject, t.authorid, t.author, dt.name as areaname
		...
		while($data = DB::fetch($query)) {
			...
			$list[$data['tid']] = array(
				...
				'fields' => array(
					...
					'area' => $data['areaname'] ? "[{$data['areaname']}]" : '',
				),
			);
			if($data['aid']) {
				$aids[] = $data['aid'];
		...
		if(!empty($listtids)) {
			...
			if($aids) {
				$query = DB::query("SELECT tid, attachment, remote,serverid FROM ".DB::table('forum_attachment')." WHERE aid IN (".dimplode($aids).')');
				while($value = DB::fetch($query)) {
					$list[$value['tid']]['pic'] = 'forum/'.$value['attachment'];
					$list[$value['tid']]['picflag'] = $value['remote'] ? '2' : '1';
					$list[$value['tid']]['serverid'] = $value['serverid'];
				}
			}

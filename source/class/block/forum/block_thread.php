 ...
 *
 *      $Id: block_thread.php 22550 2011-05-12 05:21:39Z monkey $
 */
...
class block_thread {
	...
	function getdata($style, $parameter, $beforewhere = '') {
		global $_G;
		...
		$orderby	= isset($parameter['orderby']) ? (in_array($parameter['orderby'],array('lastpost','dateline','replies','views','heats','recommends','tid')) ? $parameter['orderby'] : 'lastpost') : 'lastpost';
		...
		$query = DB::query("SELECT t.*
			$sqlfrom WHERE $beforewhere t.readperm='0'
			$sql
		...
		while($data = DB::fetch($query)) {
			...
			$list[] = array(
				...
				'pic' => $data['attachment'] ? 'forum/'.$data['attachment'] : 'http://static.8264.com/static/image/common/nophoto.gif',
				...
				'fields' => array(
					...
					'icon' => $data['icon'] > 0 ? 'http://static.8264.com/static/image/stamp/'.$_G['cache']['stamps'][$data['icon']]['url'] : '',
	...
	function getpic($tid) {
		...
		$pic = DB::fetch_first("SELECT attachment, remote,serverid FROM ".DB::table('forum_attachment')." WHERE tid='$tid' AND isimage IN (1, -1) ORDER BY dateline DESC LIMIT 0,1");
		return $pic;
	}
}

...
class image {
		...
		if($wmwidth > 10 && $wmheight > 10 && !$this->imginfo['animated']) {
			switch($this->param['watermarkstatus'][$type]) {
				...
				case 8:
					...
					break;
				case 9:
					$x = $this->imginfo['width'] - $logo_w - 15;
					$y = $this->imginfo['height'] - $logo_h - 15;
					break;
			}
	 ...
	 * 8264添加 by mengjin
	 * 等比例获取期望的图片大小
	 * @param integer $org_width 原始图片宽度
	 * @param integer $org_height 原始图片高度
	 * @param integer $width 期待图片宽度
	 * @param integer $height 期待图片高度
	 * @param integer $max_width 最大图片宽度
	 * @param integer $max_height 最大图片高度
	 * @return array 包含宽度和高度的数组
	 */
	public static function getSizeByCount($org_width, $org_height, $width=0, $height=0, $max_width=0, $max_height=0) {
		...
		if (!empty($max_width) && $width>$max_width) {
			$width = $max_width;
			$height = $width / $ratio;
		}
		if (!empty($max_height) && $height>$max_height) {
			$height = $max_height;
			$width = $height * $ratio;
		}

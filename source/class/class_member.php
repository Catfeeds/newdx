...
class logging_ctl
{
	...
	function on_login()
	{
		...
		$secqaacheck = $from_connect ? false : loginIsAppearVerify($_G['clientip'],$_G['gp_username']);
		$invite = getinvite();
		if (! submitcheck('loginsubmit', 1, $seccodestatus, $secqaacheck)) {
			$auth = '';
			...
			if ($result['status'] > 0) {
				...
				if (empty($_G['gp_handlekey']) || ! empty($_G['gp_lssubmit'])) {
					if (defined('IN_MOBILE')) {
						...
						if (! empty($_G['gp_lssubmit'])) {
							...
							$message = lang('message', $loginmessage, $param);
							// 新模板样式 lxp 20131230
							if($_G['newtpl']){
								$message = $result['ucresult']['username']."<em>欢迎您回来，现在将转入登录前页面。</em>";
							}
							showmessage('location_login_succeed', $location, array(), array(
								...
								'extrajs' => '<script type="text/javascript">'
										...
										. '$(\'succeedlocation\').innerHTML = \'' . $message . '\';</script>' . $ucsynlogin,
								'striptags' => false,
							));
						}
					}
                ...
                showmessage($fmsg, '', array('loginperm' => $num));
			}
		}
	}
	...
	function on_logout()
	{
		...
		$_G['newtpl'] && $message = '2014_logout_succeed';

		$ucsynlogout = $this->setting['allowsynlogin'] ? uc_user_synlogout() : '';
		if ($_G['gp_formhash'] != $_G['formhash']) {
			showmessage($message, dreferer(), array('formhash' => FORMHASH, 'ucsynlogout' => $ucsynlogout));
		}
		//start 记录用户退出
...
class register_ctl
{
	...
	function on_register()
	{
		global $_G;
        //第三方授权登录默认密码
        $_G['default_password'] = md5(random(10));
		...
		if ($_G['uid']) {
			...
			showmessage('login_succeed', $url_forward ? $url_forward : './', array(
				'username' => $_G['member']['username'],
				'usergroup' => $_G['group']['grouptitle'],
				'uid' => $_G['uid']), array('extrajs' => $ucsynlogin));
		...
		if (! submitcheck('regsubmit', 0, 0, $secqaacheck)) {
			...
			* $reg1_filename = DISCUZ_ROOT.'./data/reg1.txt';

			* $reg1_fc = @file_get_contents($reg1_filename);

			* $reg1_fc .= dgmdate($_G['timestamp'], 'Y-m-d H:i:s')."\t".$username."\t".$_G['gp_password']."\t".$_G['gp_email']."\t".$_G['clientip']."\t".$_SERVER['HTTP_USER_AGENT']."\t".serialize($_POST)."\r\n";

			* $fp = @fopen($reg1_filename, 'w+');
			...
			*/
			...
			if (! $activation) {
				...
				if (empty($this->setting['ignorepassword'])) {
					...
					$password = $_G['gp_password'] ? $_G['gp_password'] : md5(random(10));
				}
			}
			...
			//end
			//娉ㄥ唽鎴愬姛涔嬪悗娣诲姞绔欏唴鎺ㄨ崘濂藉弸
			$querys = DB::query("SELECT uid,username FROM " . DB::table('home_specialuser') . " WHERE 1=1 ");
			while ($values = DB::fetch($querys)) {
				if ($values) {
					$dateline = TIMESTAMP;
					DB::query("insert into " . DB::table('home_friend') . " (uid, fuid, fusername, gid, num, dateline, note)

						   values ('$uid','$values[uid]','$values[username]','0','0','$dateline','')");
					DB::query("insert into " . DB::table('home_friend') . " (uid, fuid, fusername, gid, num, dateline, note)

						   values ('$values[uid]','$uid','$username','0','0','$dateline','')");
				}
			}
			//缁撴潫
			if ($verifyarr) {
			...
			// 替换为userstats中的newsetuser lxp 20131024
// 			DB::insert('common_setting', array('skey' => 'lastmember', 'svalue' => $username), false, true);
			...
			switch ($this->setting['regverify']) {
				...
				default:
					...
					$url_forward = 'home-task-item-new.html'; //dreferer();
					break;
			}
			$param = array(
				'bbname' => $this->setting['bbname'],
			...
			$extra = array(
				...
				'extrajs' => '<script type="text/javascript">'
					...
					. '$(\'main_message\').style.display = \'none\';'
					. '$(\'main_succeed\').style.display = \'\';'
					. '$(\'succeedlocation\').innerHTML = \'' . lang('message', $locationmessage) . '\';' . '</script>',
				'striptags' => false,
			);
			//$url_forward='home-task-item-new.html';
			/* ////////////////////////////////////////////

			* $reg2_filename = DISCUZ_ROOT.'./data/reg2.txt';

			* $reg2_fc = @file_get_contents($reg2_filename);

			* $reg2_fc .= dgmdate($_G['timestamp'], 'Y-m-d H:i:s')."\t".$username."\t".$password."\t".$email."\t".$_G['clientip']."\t{$_SERVER['HTTP_USER_AGENT']}"."\r\n";

			* $fp = @fopen($reg2_filename, 'w+');

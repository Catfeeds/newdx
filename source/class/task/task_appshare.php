...
class task_appshare {
	...
	var $version = '1.0';
	var $name = 'appshare_name';
	var $description = 'appshare_desc';
	...
	var $conditions = array(
		...
		'descript' => array(
			'title' => 'appshare_descript',
			'type' => 'text',
			'value' => '',
			'sort' => 'complete',
		),
		'subject' => array(
			'title' => 'appshare_subject',
			'type' => 'text',
			'value' => '',
			'sort' => 'complete',
		),
...
/*	function view($task, $taskvars) {
		...
		if($taskvars['complete']['act']['value'] == 'newreply') {
			if($taskvars['complete']['threadid']) {
				$return .= lang('task/appshare', 'task_complete_act_newreply_thread', array('value' => $value, 'num' => $taskvars['complete']['num']['value']));
			} else {
				$return .= lang('task/appshare', 'task_complete_act_newreply_author', array('value' => $value, 'num' => $taskvars['complete']['num']['value']));
			}
		} else {
			if($taskvars['complete']['forumid']) {
				$return .= lang('task/appshare', 'task_complete_forumid', array('value' => $value));
			}
			if($taskvars['complete']['act']['value'] == 'newthread') {
				$return .= lang('task/appshare', 'task_complete_act_newthread', array('num' => $taskvars['complete']['num']['value']));
			} else {
				$return .= lang('task/appshare', 'task_complete_act_newpost', array('num' => $taskvars['complete']['num']['value']));
			}
		}

...
class task_qreply {
	...
	var $version = '1.0';
	var $name = 'qreply_name';
	var $description = 'qreply_desc';
	...
	var $conditions = array(
		...
		'descript' => array(
			'title' => 'qreply_descript',
			'type' => 'text',
			'value' => '',
			'sort' => 'complete',
		),
		'subject' => array(
			'title' => 'qreply_subject',
			'type' => 'text',
			'value' => '',
			'sort' => 'complete',
		),
	...
	function csc($task = array()) {
		global $_G;
       $num = DB::result_first("SELECT COUNT(*) FROM ".DB::table('question_reply')." WHERE ref_topicid = '4' AND authorid='$_G[uid]'");
        if($num && $num >0) {
            return TRUE;
        } else {
...
/*	function view($task, $taskvars) {
		...
		if($taskvars['complete']['act']['value'] == 'newreply') {
			if($taskvars['complete']['threadid']) {
				$return .= lang('task/qreply', 'task_complete_act_newreply_thread', array('value' => $value, 'num' => $taskvars['complete']['num']['value']));
			} else {
				$return .= lang('task/qreply', 'task_complete_act_newreply_author', array('value' => $value, 'num' => $taskvars['complete']['num']['value']));
			}
		} else {
			if($taskvars['complete']['forumid']) {
				$return .= lang('task/qreply', 'task_complete_forumid', array('value' => $value));
			}
			if($taskvars['complete']['act']['value'] == 'newthread') {
				$return .= lang('task/qreply', 'task_complete_act_newthread', array('num' => $taskvars['complete']['num']['value']));
			} else {
				$return .= lang('task/qreply', 'task_complete_act_newpost', array('num' => $taskvars['complete']['num']['value']));
			}
		}

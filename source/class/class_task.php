...
class task {
	...
	function tasklist($item) {
		...
		$updated = FALSE;
		$num = 0;
		$query = DB::query("SELECT t.*, mt.csc, mt.dateline, mt.checked FROM ".DB::table('common_task')." t
		...
		while($task = DB::fetch($query)) {
			...
			if(!$updated && $item == 'doing' && $task['csc'] < 100 ) {
				...
				if($result === TRUE) {
					...
					/*新增，任务完成后记录用户的任务记录*/
					if(!$task['checked']){
						if(method_exists($taskclass , 'record')){
							$record = $taskclass->record($task);
							if($record) self::record_user_task($_G['uid'] , $task['scriptname'] , $task['taskid'] , $record , TIMESTAMP);
						}
					}
					/*结束*/
	...
	function view($id) {
		...
		$this->task = DB::fetch_first("SELECT t.*, mt.status, mt.csc, mt.checked, mt.dateline, mt.dateline AS applytime FROM ".DB::table('common_task')." t LEFT JOIN ".DB::table('common_mytask')." mt ON mt.uid='$_G[uid]' AND mt.taskid=t.taskid WHERE t.taskid='$id' AND t.available='2'");
		if(!$this->task) {
		...
		if($this->task['status'] == '-1') {
			...
			if($this->task['csc'] < 100) {
				...
				if($result === TRUE) {
					$this->task['csc'] = '100';
					/*新增，任务完成后记录用户的任务记录*/
					if(method_exists($taskclass , 'record') && !$this->task['checked']){
						$record = $taskclass->record($this->task);
						if($record) self::record_user_task($_G['uid'] , $this->task['scriptname'] , $id , $record , TIMESTAMP);
					}
					/*结束*/
...
//                                }
	...
	function apply($id) {
		...
		DB::query("REPLACE INTO ".DB::table('common_mytask')." (uid, username, taskid, csc, dateline, checked)
			VALUES ('$_G[uid]', '$_G[username]', '".$this->task['taskid']."', '0\t".$_G[timestamp]."', '$_G[timestamp]', '".( $this->task['checkit'] ? '0' : '1' )."')");
	...
	function draw($id) {
		...
		if(!($this->task = DB::fetch_first("SELECT t.*, mt.dateline AS applytime, mt.status, mt.checked FROM ".DB::table('common_task')." t, ".DB::table('common_mytask')." mt WHERE mt.uid='$_G[uid]' AND mt.taskid=t.taskid AND t.taskid='$id' AND t.available='2'"))) {
			showmessage('task_nonexistence');
		} elseif($this->task['status'] != 0) {
			showmessage('undefined_action');
		} elseif($this->task['tasklimits'] && $this->task['achievers'] >= $this->task['tasklimits']) {
		...
		if($result === TRUE) {
			...
			if($this->task['reward']) {
				...
				notification_add($_G[uid], 'system', 'task_reward_'.$this->task['reward'], array(
			...
			if($this->task['scriptname'] == 'tiezi'){
				//删除 common_share_task_tiezi表记录 
				DB::delete('common_share_task_tiezi', "uid=".$_G['uid']." AND taskid=".$id);
			}
	...
	function parter($id) {
		$query = DB::query("SELECT * FROM ".DB::table('common_mytask')." WHERE taskid='$id' ORDER BY dateline DESC LIMIT 0, 12");
		$parterlist = array();
	...
	/**
	 * @author JiangHong
	 * 用户将用户的任务信息保存到表 pre_plugin_taskrecord 中
	 */
	function record_user_task($uid , $scriptname , $taskid , $message , $time){
		$arr = array(
					'uid' => $uid ,
					'task_scriptname' => $scriptname ,
					'taskid' => $taskid ,
					'message' => $message ,
					'dateline' => $time ,
		);
		DB::insert('plugin_taskrecord',$arr);
	}
}

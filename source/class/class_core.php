...
class discuz_core {
	...
	function init() {
		if(!$this->initated) {
			...
			$this->_init_newtpl();  // 新老模板切换 lxp 20131212
			$this->_init_ipbanspider();	// 防爬虫
		}
		$this->initated = true;
	}
	...
	function _init_env() {
		...
		if(preg_match('/\/\w+\.php\//i', $_SERVER['PHP_SELF'])){
			$url =  preg_replace('/\/\w+\.php\//i', '/', $_SERVER['PHP_SELF']);
			$url = 'http://' . $_SERVER['HTTP_HOST'] . preg_replace("/\/+(api)?\/*$/i", '', $url);
	...
	function _init_input() {
		...
		// 111201 wistar 当没有$_COOKIE且$_POST['*_auth']不为空时则说明手机终端端请求
//		if ($_COOKIE && !$_POST['post_auth_mobileapp']) {
...
//			}
//		} else {
////			$this->var['cookie']['auth'] = $_POST[$this->config['cookie']['cookiepre'].'auth'];
//		}
		foreach($_COOKIE as $key => $val) {
			if(substr($key, 0, $prelength) == $this->config['cookie']['cookiepre']) {
				$this->var['cookie'][substr($key, $prelength)] = $val;
	...
	function _init_config() {
		...
		define('STATICURL', !empty($_config['output']['staticurl']) ? $_config['output']['staticurl'] : 'http://static.8264.com/static/');
	...
	function _init_session() {
		...
		if($this->init_session)
		{
			...
			/*屏蔽掉论坛本身的封IP方式20120801jianghong*/
	...
	function _init_user() {
		...
		if($this->init_user) {
			...
			// 111223 wistar 识别手机终端用户信息
			if(!$auth && $_POST['auth_mobileapp']) {
				$auth = explode("_", base64_decode($_POST['auth_mobileapp']));
			}
		...
		//测试用，当用户为35732735时，使用cdn上传
		
		if($this->var['uid'] == 35732735){
			$this->config['cdns']['opend'] = true;
		}
		
		// /*2014-1-22用于查询是否有新邀请，先查cookie，15秒间隔查询*/
		// $this->var['member']['newinvite'] = $this->var['cookie']['invite_user_' . $this->var['uid']];
		// if(! $this->var['member']['newinvite']){
			// require_once libfile('class/myredis');
			// $redis = & myredis::instance(6378);
			// $this->var['member']['newinvite'] = max(0, intval($redis->zScore("common_member_invite_users_list", 'uid_' . $this->var['uid'])));
			// dsetcookie('invite_user_' . $this->var['uid'], $this->var['member']['newinvite'], 15);
		// }
	}
	...
	function _init_setting() {
		...
		$this->var['template_last_update_time'] = memory('get','update_time');
	}
	 ...
	 * @author lxp 20131212
	 */
	function _init_newtpl(){
		global $_G;
		$_G['newtpl'] = isset($_GET['old']) ? 0 : 1;  // old=1 切换老版
		$_G['newtpl'] && $_G['setting']['disallowfloat'] = "login|".$_G['setting']['disallowfloat'];
	}

	//防爬虫
	function _init_ipbanspider(){
		if(file_exists(DISCUZ_ROOT.'./source/plugin/ipbanspider/ipbanspider.class.php')){
			require DISCUZ_ROOT.'./source/plugin/ipbanspider/ipbanspider.class.php';
			$ipbanspider = new plugin_ipbanspider;
			$ipbanspider->check();
		}
	}
}
...
class db_mysql
{
	...
	function connect($serverid = 1) {
		...
		$this->curlink_master = $this->link[$serverid];
...
class discuz_memory
{
	...
	function init($config) {
		...
		if($this->extension['memcache'] && !empty($config['memcache_clusters'])) {
			require_once libfile('class/memcache');

			$this->server_hash = new server_hash();
			$clusters_enable = 0;	//集群可用服务器数量

			foreach ($config['memcache_clusters'] as $key => $server) {
				$this->memory = &discuz_memcache::instance($server, $config['memcache']['pconnect']);
				if($this->memory->enable) {
					//添加集群
					$this->server_hash->addNode($server);
					$clusters_enable++;
				}
			}
			if($clusters_enable > 0) {
				$this->enable = true;
				$this->type = 'memcache';
			} else {
				$this->memory = null;

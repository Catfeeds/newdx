...
function getranklist_pictures($num = 20, $dateline = 0, $orderby = 'hot DESC') {
	...
	$query = DB::query("SELECT p.picid, p.uid, p.username, p.title, p.filepath, p.thumb, p.remote, p.hot, p.sharetimes, p.click1,
		p.click2, p.click3 , p.click4, p.click5, p.click6, p.click7, p.click8, a.albumid, a.albumname, a.friend,p.serverid
	...
	while($picture = DB::fetch($query)) {
		++$rank;
		$picture['rank'] = $rank;
		$picture['url'] = $picture['friend'] == 0 ? pic_get($picture['filepath'], 'album', $picture['thumb'], $picture['remote'],1,$picture['serverid']) : 'http://static.8264.com/static/image/common/nopublish.gif';;
		$picture['origurl'] = pic_get($picture['filepath'], 'album', 0, $picture['remote'],1,$picture['serverid']);
		$picturelist[] = $picture;
	}
	return $picturelist;
}
...
function getranklist_pictures_index($num = 20, $dateline = 0, $orderby = 'hot DESC') {
	$picturelist = array();
	$query = DB::query("SELECT p.picid, p.uid, p.username, p.title, p.filepath, p.thumb, p.remote,p.serverid, a.albumid, a.albumname, a.friend
		FROM ".DB::table('home_pic')." p
		LEFT JOIN ".DB::table('home_album')." a ON p.albumid=a.albumid
		WHERE p.hot>'3'
	...
	while($picture = DB::fetch($query)) {
		$picture['url'] = $picture['friend'] == 0 ? pic_get($picture['filepath'], 'album', $picture['thumb'], $picture['remote'],1,$picture['serverid']) : 'http://static.8264.com/static/image/common/nopublish.gif';;
		$picture['origurl'] = $picture['friend'] == 0 ? pic_get($picture['filepath'], 'album', 0, $picture['remote'],1,$picture['serverid']) : 'http://static.8264.com/static/image/common/nopublish.gif';
		$picturelist[] = $picture;
	}
	return $picturelist;
}
...
function getranklist_forums($type = 'threads' , $num = 20) {
	...
	if($type == 'posts') {
		$sql = "SELECT fid, name, posts FROM ".DB::table('forum_forum')." WHERE status='1' AND type<>'group' ORDER BY posts DESC LIMIT 0, $num " . getSlaveID();
		...
		GROUP BY p.fid ORDER BY posts DESC LIMIT 0, $num " . getSlaveID();
	} elseif($type == 'today'){
		$sql = "SELECT DISTINCT(p.fid) AS fid, f.name, COUNT(pid) AS posts FROM ".DB::table('forum_post')." p, ".DB::table('forum_forum')." f
		WHERE p.fid=f.fid AND p.dateline>=".($timestamp-86400)." AND p.invisible='0' AND p.authorid>'0' AND f.status='1' AND f.type<>'group'
		GROUP BY p.fid ORDER BY posts DESC LIMIT 0, $num " . getSlaveID();
	} else {
		$sql = "SELECT fid, name, threads AS posts FROM ".DB::table('forum_forum')." WHERE status='1' AND type<>'group' ORDER BY threads DESC LIMIT 0, $num " . getSlaveID();
	}
	$query = DB::query($sql);
	$i = 1;
...
function getranklist_groups($type = 'threads' , $num = 20) {
	...
	if($type == 'posts') {
		$sql = "SELECT fid, name, posts FROM ".DB::table('forum_forum')." WHERE status='3' AND type='sub' ORDER BY posts DESC LIMIT 0, $num " . getSlaveID();
	} elseif($type == 'thismonth') {
		$sql = "SELECT DISTINCT(p.fid) AS fid, f.name, COUNT(pid) AS posts FROM ".DB::table('forum_post')." p, ".DB::table('forum_forum')." f
		...
		WHERE status='3' AND type='sub' ORDER BY commoncredits DESC LIMIT 0, $num " . getSlaveID();
	} elseif($type == 'member'){
		...
		ORDER BY ff.membernum DESC LIMIT 0, $num " . getSlaveID();
	} else {
		$sql = "SELECT fid, name, threads AS posts FROM ".DB::table('forum_forum')." WHERE status='3' AND type='sub' ORDER BY threads DESC LIMIT 0, $num " . getSlaveID();
	}
	$query = DB::query($sql);
	$i = 1;
	while($result = DB::fetch($query)) {
...
function getranklist_member_credits($type = 'all' , $num = 20) {
	...
	if($type == 'all') {
		$sql = "SELECT m.uid,m.username,m.videophotostatus,m.groupid,m.credits,field.spacenote FROM ".DB::table('common_member')." m
			...
			ORDER BY m.credits DESC LIMIT 0, $num " . getSlaveID();
	} else {
		$sql = "SELECT m.uid,m.username,m.videophotostatus,m.groupid, mc.extcredits$type AS extcredits
			...
			ORDER BY extcredits$type DESC LIMIT 0, $num " . getSlaveID();
	}
...
function getranklist_member_friendnum($num = 20) {
	...
	$query = DB::query('SELECT uid, friends FROM '.DB::table('common_member_count').' WHERE friends>0 ORDER BY friends DESC LIMIT '.$num . getSlaveID());
	...
	if($uids) {
		$query = DB::query('SELECT m.uid, m.username, m.videophotostatus, m.groupid, field.spacenote
			...
			WHERE m.uid IN (".dimplode($uids).")" . getSlaveID());
...
function getranklist_member_blogs($num = 20) {
	...
	$num = $num ? $num : 20;
	$data = $users = $oldorder = array();
	$query = DB::query('SELECT uid, blogs FROM '.DB::table('common_member_count').' WHERE blogs>0 ORDER BY blogs DESC LIMIT '.$num . getSlaveID());
	while($user = DB::fetch($query)) {
		$users[$user['uid']] = $user;
		$oldorder[] = $user['uid'];
	}
...
function getranklist_member_gender($gender, $num = 20) {
	...
	$query = DB::query("SELECT c.uid, c.views FROM ".DB::table('common_member_count')." c
			...
			WHERE c.views>0 AND p.gender = '$gender' ORDER BY c.views DESC LIMIT 0, $num " . getSlaveID());
	while($user = DB::fetch($query)) {
		$users[$user['uid']] = $user;
		$oldorder[] = $user['uid'];
	}

...
$op = $_G['gp_op'];
if(!in_array($op, array('basic', 'threadsrank', 'postsrank', 'forumsrank', 'creditsrank', 'trade', 'onlinetime', 'team', 'trend', 'modworks', 'memberlist', 'forumstat', 'forumstatdetail', 'trend'))) {
	$op = 'basic';
}
...
if($op == 'basic') {
	...
	extract($statvars);
	include template('forum/stat_misc');
} elseif($op == 'forumstatdetail') {
	$statvars = getstatvars('forumstatdetail');
	extract($statvars);
	include template('forum/stat_misc');
} elseif($op == 'trend') {
...
function getstatvars($type) {
	...
	switch($type) {
		...
		case 'forumstat':
			...
			break;
		case 'forumstatdetail':
			$statvars = getstatvars_forumstatdetail($_G['gp_typeid']);
			break;
	}
	return $statvars;
}
...
function getstatvars_basic() {
	...
	$bestmember = DB::fetch_first("SELECT author, COUNT(*) AS posts FROM ".DB::table('forum_post')." WHERE dateline>='$_G[timestamp]'-86400 AND invisible='0' AND authorid>'0' GROUP BY author ORDER BY posts DESC LIMIT 1 " . getSlaveID());
	$statvars['bestmem'] = $bestmember['author'];
	$statvars['bestmemposts'] = $bestmember['posts'];
	$postsinfo = DB::fetch_first("SELECT COUNT(*) AS posts, (MAX(dateline)-MIN(dateline))/86400 AS runtime FROM ".DB::table('forum_post') . " " . getSlaveID());
...
function getstatvars_team() {
	...
	$query = DB::query("SELECT m.uid, m.username, m.adminid, ms.lastvisit, m.credits, mc.posts $oltimeadd1
		FROM ".DB::table('common_member')." m
	...
	while($member = DB::fetch($query)) {
		...
		$member['offdays'] = intval((TIMESTAMP - $member['lastvisit']) / 86400);
		$totaloffdays += $member['offdays'];

		if($_G['setting']['oltimespan']) {
			$member['totalol'] = round($member['totalol'] / 60, 2);
			$member['thismonthol'] = gmdate('Yn', $member['lastvisit']) == gmdate('Yn', TIMESTAMP) ? round($member['thismonthol'] / 60, 2) : 0;
	...
	$query = DB::query("SELECT authorid, COUNT(*) AS posts FROM ".DB::table('forum_post')."
		WHERE dateline>=".($_G['timestamp']-86400*30)." AND authorid IN ($uids) AND invisible='0' GROUP BY authorid " . getSlaveID());
	...
	if(is_array($team)) {
		foreach($team['members'] as $uid => $member) {
			...
			@$team['members'][$uid]['lastvisit'] = dgmdate($member['lastvisit'] + $timeoffset * 3600, 'd');
...
function getstatvars_forumstatdetail($typeid) {
	global $_G;
	$xml = "<chart>\n";
	$statvars = array();
	$monthdays = array('31', '28', '31', '30', '31', '30', '31', '31', '30', '31', '30', '31');
	if(!$typeid) {
		$query = DB::query("SELECT fid, name FROM ".DB::table('forum_forum')." WHERE status<>'3' AND type<>'group'");
		$forums = array();
		while($forum = DB::fetch($query)) {
			$forums[$forum['fid']]['name'] = $forum['name'];
			$forums[$forum['fid']]['more'] = array();
		}
		foreach($forums as $key => $value){
			$query1 = DB::query("SELECT typeid,name FROM ".DB::table('forum_threadclass')." WHERE fid='{$key}' ORDER BY displayorder");
			while($row = DB::fetch($query1)) {
				$forums[$key]['more'][$row['typeid']] = $row['name'];
			}
		}
		$statvars['forums'] = $forums;
	} else {
		$foruminfo = DB::fetch_first("SELECT typeid, name,todayposts FROM ".DB::table('forum_threadclass')." WHERE typeid='$typeid'");
		$statvars['foruminfo'] = $foruminfo;
		...
		$statvars['month'] = $end_month;
		$query = DB::query("SELECT logdate, typeid, value
			FROM ".DB::table('plugin_statlog_detail')."
			WHERE logdate>='$end_month_start' AND logdate<='$end_date' AND type='1' AND typeid='$typeid'
			ORDER BY logdate ASC");
		$logs = array();
		$xml .= "<xaxis>\n";
		$xmlvalue = '';
		...
		$mindate = DB::result_first("SELECT MIN(logdate) FROM ".DB::table('plugin_statlog_detail')." WHERE typeid='$typeid'");
		...
		$query = DB::query("SELECT logdate, `value` FROM ".DB::table('plugin_statlog_detail')." WHERE typeid='$typeid' AND type='1'");
		$monthposts = array();
		while($data = DB::fetch($query)) {
			list($year, $month, $day) = explode('-', $data['logdate']);
			if(isset($monthposts[$year.'-'.$month])) {
	...
	$statvars['statuspara'] = "path=&settings_file=data/stat_setting.xml&data_file=".urlencode("misc.php?mod=stat&op=forumstatdetail&typeid=$typeid&month={$_G['gp_month']}&xml=1");

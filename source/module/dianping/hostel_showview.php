...
if ($_GET['ajaxreply'] == 1) {
    ...
    include template('dianping/hostel_comment');
    exit;
}
...
$detail = $detail_obj->getdetail('hostel', 'i.author_id,i.ispublish,i.uids,i.contact,i.weixin,i.qq,i.lon, i.lat, i.addr,i.tel,i.provinceid,i.placetype,i.type,i.cityid,i.serverid');
if($detail['ispublish']==0 && $detail['author_id']!=$_G['uid'] && $_G['adminid']!=1){
    showmessage('您要查看的内容不存在或未审核，请返回'); 
}
//if(empty($detail) || $detail['fid'] != $dp_modules['hostel']['fid'] || ($detail['ispublish'] == 0 && $_G['adminid'] != 1)) { showmessage('您要查看的内容不存在或未审核，请返回'); }

//title, description, keywords
$pageTitle= $detail['subject']."旅舍客栈地址|电话|怎么样 - 旅舍客栈点评 - 户外资料网8264.com";
if($page>1){
    $pageTitle = strtr($pageTitle,'[page]',' 第'.$page.'页 -');
}else{
    $pageTitle = str_replace('[page]','',$pageTitle);
}
$metadescription = $detail['subject'].'旅舍客栈详细介绍包括地址，联系方式，服务水平以及真实用户点评感受，为驴友出行提供权威靠谱的信息参考';
...
$_G['cache']['dp_hostel_list_hot'] ? $sidebar_list_hot = $_G['cache']['dp_hostel_list_hot'] : updatecache('dp_hostel_list_hot');
...
include template('dianping/hostel_showview');
?>

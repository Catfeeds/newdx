...
//    }
    $modlist_my = $list_obj->getlist($mod, 'ispublish,cityid,provinceid,addr,cnum,tel,chenglishijian,lingduinum,gongsitype', $where, $start, $perpage, $orderby, '', 1);
    $modlistall = $list_obj->getlist($mod, 'ispublish,cityid,provinceid,addr,cnum,chenglishijian,lingduinum,gongsitype', 'ispublish>=0', $start, '', $orderby, '', 0);
    ...
    $pageTitle ="我发布的户外俱乐部-第{$page}页-户外资料网8264.com";
    $metakeywords ="户外俱乐部";
    $metadescription = "我发布的俱乐部";
    unset($pro_city);
    unset($pro_city_ctype);
    /*end----测试lang包,进行pagetitle统一处理*/
    //精彩点评
    $_G['cache']['dp_club_comment_rate'] ? $comment_rate = $_G['cache']['dp_club_comment_rate'] : updatecache('dp_club_comment_rate');
    include template('dianping/club_myshowlist');
}else{

...
if(empty($detail) || $detail['fid'] != $dp_modules['climb']['fid'] || ($detail['ispublish'] == 0 && $_G['adminid'] != 1)) { showmessage('您要查看的内容不存在或未审核，请返回'); }

//title, description, keywords
$pageTitle= $detail['subject']."官网|地址|电话|怎么样-攀岩场地点评 - [page]{$_G['setting']['bbname']}";
if($page>1){
    $pageTitle = strtr($pageTitle,'[page]',' 第'.$page.'页 -');
}else{
    $pageTitle = str_replace('[page]','',$pageTitle);
}
$metadescription = '关于'.$detail['subject'].'的详细介绍包括价格,地址,联系方式,服务水平，场地环境以及真实用户点评感受，为前往'.$detail['subject'].'的攀爬爱好者提供权威靠谱的信息参考';

...
if ($type) {
    ...
    $condition .= " and type in ({$child_type})";
...
if($order == "score"){
    ...
    $where .= ' and cnum < 5';
    $modlist_con = $list_obj->getlist($mod, 'type,cityid,provinceid,addr,cnum,isfree', $condition,$start, $perpage,$orderby, '', 1);      
    ...
    if($modlist_con){
        ...
        if ($page <= $con_page ) {
            if ($page == $con_page && $offset > 0) {            
                $modlist_where = $list_obj->getlist($mod, 'type,cityid,provinceid,addr,cnum,isfree', $where,0, $offset_con,$orderby, '', 1);
            ...
            $modlist_where = $list_obj->getlist($mod, 'type,cityid,provinceid,addr,cnum,isfree', $where,$start_where, $perpage,$orderby, '', 1);
            $modlist = $modlist_where;
        }
        $modlist_where_all = $list_obj->getlist($mod, 'type,cityid,provinceid,addr,cnum,isfree', $where,$start, $perpage,$orderby, '', 1);
        $recordnum_where = $list_obj->recordnum;
        $recordnum = $recordnum_con + $recordnum_where;
    }else{
        $modlist = $list_obj->getlist($mod, 'type,cityid,provinceid,addr,cnum,isfree', $where,$start, $perpage,$orderby, '', 1);
    ...
    $modlist = $list_obj->getlist($mod,  'type,cityid,provinceid,addr,cnum,isfree', $condition, $start, $perpage, $orderby, '', 1);
    $recordnum = $list_obj->recordnum;
...
/*start----处理获取的省市和类型,进行重新组合满足title定制需要*/
if ($provinceid == 0 && $type == 0) {
    $status = 0;
} else {
    //省市变量组合临时字符串，省市及钓鱼类型变量组合临时字符串
...
/*end----处理获取的省市和钓鱼类型,进行重新组合满足title定制需要*/
...
//边栏
$_G['cache']['dp_fishing_comment_rate'] ? $comment_rate = $_G['cache']['dp_fishing_comment_rate'] : updatecache('dp_fishing_comment_rate');

//此脚本用于在指定条件下返回json数据,服务于[后台->内容->榜单管理->批量添加榜单]功能，如果[批量添加榜单]功能已经弃用，此文件可以注销引用
require_once libfile('dianping/outputjson', 'include');

include template('dianping/fishing_showlist');
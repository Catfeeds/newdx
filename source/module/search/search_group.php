...
if(!submitcheck('searchsubmit', 1)) {
	...
	if(!empty($searchid)) {
		...
		if($index['ids']['group'] && ($viewgroup || empty($searchstring[2]))) {
			...
			$query = DB::query("SELECT f.*, ff.description, ff.membernum, ff.icon,ff.serverid_icon, ff.gviewperm, ff.jointype, ff.dateline FROM ".DB::table('forum_forum')." f LEFT JOIN ".DB::table('forum_forumfield')." ff ON f.fid=ff.fid WHERE f.fid IN ({$index[ids][group]}) AND f.status='3' AND `type`='sub'".($viewgroup ? " LIMIT $start_limit, $_G[tpp]" : ''));

...
if($view == 'groupthread' || $view == 'mythread') {
	...
	if($view == 'groupthread' && empty($typeid) && !empty($usergroups['grouptype'])) {
		...
		$attentiongroupfid = '';
		$query = DB::query("SELECT fid, icon,serverid_icon FROM ".DB::table('forum_forumfield')." WHERE fid IN (".dimplode($attentiongroups).")");
		while($row = DB::fetch($query)) {
			$attentiongroup_icon[$row[fid]] = get_groupimg($row['icon'], 'icon',$row['serverid_icon']);
		}
	...
	$mygrouplist = mygrouplist($_G['uid'], 'lastupdate', array('f.name', 'ff.icon','ff.serverid_icon'), 50);
	if($mygrouplist) {
	...
	$num = mygrouplist($_G['uid'], 'lastupdate', array('f.name', 'ff.icon','ff.serverid_icon'), 0, 0, $ismanager, 1);
	$multipage = multi($num, $perpage, $page, 'group.php?mod=my&view='.$view);
	$grouplist = mygrouplist($_G['uid'], 'lastupdate', array('f.name', 'ff.icon','ff.serverid_icon'), $perpage, $start, $ismanager);
}
...
if($frienduid && is_array($frienduid)) {
	...
	$query = DB::query("SELECT f.fid, f.name, ff.icon,ff.serverid_icon
	...
	while($group = DB::fetch($query)) {
		$group['icon'] = get_groupimg($group['icon'], 'icon',$group['serverid_icon']);

...
if(empty($_G['gp_nothumb']) && $attach['isimage'] && $attach['thumb']) {
	...
	if($attach['remote']) {
        ...
        if($attach['serverid']>0){
            getserverfile($attach['attachment'],$server_domain);
        }else{
...
if(!$attach['remote'] && !is_readable($filename) && $attach['serverid']==0) {
	showmessage('attachment_nonexistence');
}
if(!$requestmode) {
	...
	if(empty($_G['gp_noupdate'])) {
		if($_G['setting']['delayviewcount'] == 2 || $_G['setting']['delayviewcount'] == 3) {
			...
			if($myredis->connected){
				$myredis->obj->hincrby('attachment_number',$aid,1);
			}else{
				DB::query("UPDATE ".DB::table('forum_attachment')." SET downloads=downloads+'1' WHERE aid='$aid'", 'UNBUFFERED');exit;
			}
			/**结束**/
...
if($attach['serverid']>0 && $isimage){
	if( is_array( $server_domain ) ) {
		dheader('location: http://' . $server_domain['name'] . '/' . $attach['dir'] . '/' . $attach['attachment'] . $attachserver->getshowString($attach['dir'], false, true, true, $server_domain['domain']['cdnname']) );
	}else{
    	dheader('location: http://'.$server_domain.'/forum/'.$attach['attachment']);
    }
}
$filesize = (!$attach['remote'] && $attach['serverid']==0 )? filesize($filename) : $attach['filesize'];
...
$attach['remote'] ? getremotefile($attach['attachment']) :($attach['serverid']>0 ? getserverfile($attach['attachment'],$server_domain) : getlocalfile($filename, $readmod, $range));
...
function getserverfile($file,$domain){
    ...
    if(!@readfile("http://".$domain."/forum/".$file)){
        dheader("location: http://".$domain."/forum/".$file);
    }
}

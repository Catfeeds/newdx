...
//调取发布活动的相应版块
//论坛首页调取猜你喜欢的模块
$recent_str = recent_forum('get');
//论坛首页调取猜你喜欢的模块

//调取帖子精选或者铿锵玫瑰等列表
$kqmg = getlist(878,0,3);
$yzxx = getlist(880,0,3);

$mryt = getlist(842,0,1);
$hwsys = getlist(912,0,2);
$tzjx = getlist(871,0,8);

//论坛首页任务列表的调取    
$tasklist = gettasklist();
//论坛首页调取在外活动分类
$huodong_cate =  getzaiwai_cate_new();
...
if(empty($gid) && empty($_G['member']['accessmasks']) && empty($showoldetails)) {
	...
	if(defined('FORUM_INDEX_PAGE_MEMORY') && FORUM_INDEX_PAGE_MEMORY) {
		categorycollapse();
		if($_G['gp_archiver']) {
			include loadarchiver('forum/bbs_new');
		} else {
			$newindex ? include template('diy:forum/bbs_new') : include template('diy:forum/discuz');
		}
		dexit();
	}
}
...
//echo "<br>";
//print_r($forumlist);
$newindex ? include template('diy:forum/bbs_new:'.$gid) : include template('diy:forum/discuz:'.$gid);

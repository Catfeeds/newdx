...
}elseif(!empty($_G['gp_paging'])){ //每日一图幻灯上一篇下一篇跳转

	if($_G['tid']){

		$post_title=DB::fetch_first("SELECT dateline,url FROM ".DB::table('portal_article_title')." WHERE idtype='tid' and id ='$_G[tid]'");

		$where=" idtype='tid' and catid=842 ";

		if($_G['gp_paging']=='prev'){
			$where.=" and dateline > '$post_title[dateline]' ";
			$order='dateline ASC';
			$message='redirect_nextoldset_nonexistence';
		}elseif ($_G['gp_paging']=='next'){
			$where.=" and dateline < '$post_title[dateline]' ";
			$order='dateline DESC';
			$message='thread_nonexistence';
		}

		$article=DB::query("SELECT url FROM ".DB::table('portal_article_title')." WHERE $where order by $order limit 0,1");
		$article_array=DB::fetch($article);
		if(!empty($article_array)){
			$article_url=str_replace("/thread-", "/picture-", $article_array['url']);   //替换每日一图新地址
			header("Location:".$article_url);
		}else{
			showmessage($message,str_replace("/thread-", "/picture-", $post_title['url']));
		}
		exit;

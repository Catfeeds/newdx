...
if(empty($_G['gp_action'])) {
	...
	$perpage = 8;
    /*增加，引入新模板，调整原先的样式，2012-08-20 jianghong*/
    if(isset($_G['gp_new_image_template']) && $_G['gp_new_image_template']==1){
        $perpage = 6;
    }
	$perpage = (isset($_G['gp_perpage']) && intval($_G['gp_perpage']) > 1) ? intval($_G['gp_perpage']) : $perpage;
	$showpagenumber = (isset($_G['gp_showpage']) && intval($_G['gp_showpage']) > 1) ? intval($_G['gp_showpage']) : 10;
    /*结束*/
    ...
    /*增加，引入新模板，调整原先的样式，2012-08-20 jianghong*/
    if(isset($_G['gp_new_image_template']) && $_G['gp_new_image_template']==1){
        $multi = multi($count, $perpage, $page, "forum.php?mod=post&action=albumphoto&aid=$aid&new_image_template=1");
        include template('forum/ajax_albumlist_newimage');
    }else{
        $multi = multi($count, $perpage, $page, "forum.php?mod=post&action=albumphoto&aid=$aid&perpage=$perpage&showpage=$showpagenumber", 0, $showpagenumber);
...
if($_G['gp_action'] == 'edit' || $_G['gp_action'] == 'reply') {
	...
	if($_G['gp_action'] == 'reply' && ($thread['closed'] == 1) && $_G['adminid']!=1) {
...
if($_G['forum']['status'] == 3) {
	...
	if($dm){
		$returnurl = 'http://' . $dm . '.' . $_G['setting']['domain']['root']['forum'] . '/' . (!empty($_G['gp_extra']) ? '&'.preg_replace("/^(&)*/", '', $_G['gp_extra']) : '');
	}else{
	...
	$navigation = ' <em>&rsaquo;</em> <a href="./">'.$_G['setting']['navs'][2]['navname'].'</a> '.$navigation;
}
...
//新的活动内容
if ($_G['gp_action'] == 'newthread' && $special == 4 && intval($_G['gp_activitycredit']) == 0) {
...
if($_G['gp_action'] == 'newthread') {
	...
	$query = DB::query("SELECT dateline, fid, tid, pid, subject FROM ".DB::table('forum_post')." WHERE authorid='$_G[uid]' AND invisible='-3' AND first='1' ".getSlaveID());

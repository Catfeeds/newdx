...
if($action == 'index') {
	...
	if($_G['gp_op'] == 'group') {
		...
		if(submitcheck('groupmanage')) {
			...
			$return_icon = upload_icon_banner($_G['forum'], $_FILES['iconnew'], 'icon');
            $return_banner = upload_icon_banner($_G['forum'], $_FILES['bannernew'], 'banner');
			$iconnew = $return_icon['attachment'];
            $iconserverid = $return_icon['serverid'];
			$bannernew = $return_banner['attachment'];
            $bannerserverid = $return_banner['serverid'];
			if($iconnew) {
				$iconsql .= ", icon='$iconnew', serverid_icon=$iconserverid";
			...
			if($bannernew && empty($deletebanner)) {
				$iconsql .= ", banner='$bannernew', serverid_banner=$bannerserverid";
			} elseif($deletebanner) {
				$iconsql .= ", banner='', serverid_banner=0";

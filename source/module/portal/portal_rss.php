...
$rsscatid = empty($_GET['catid']) ? 0 : $_GET['catid'];
$catname = '';
...
if(empty($rsscatid)) {
	...
	if (strrpos($rsscatid, '_') != false) {
		$rsscatids = explode('_', $rsscatid);
		$catarray = array();
		$catname = array();
		foreach ($rsscatids as $item) {
			if(isset($_G['cache']['portalcategory'][$item])) {
				$catarray[] = $item;
				$catname[] = dhtmlspecialchars($_G['cache']['portalcategory'][$item]['catname']);
			}
		}

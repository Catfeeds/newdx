...
$attachinfo = $attachserver->get_server_domain($attach['serverid'],'*');
if($operation == 'delete') {
	if(!$_G['group']['allowmanagearticle'] && $_G['uid'] != $attach['uid']) {
		showmessage("portal_attachment_nopermission_delete");
	}
	$isMpic = !empty($_GET['isMpic']) ? $_GET['isMpic'] : '';
	if($isMpic) {
		DB::query("UPDATE ".DB::table('portal_article_title')." SET mpic = '' WHERE aid='$aid'", 'UNBUFFERED');
	} else {
	...
	pic_delete($attach['attachment'], 'portal', $attach['thumb'], $attach['remote'],$attachinfo['domain'],$attachinfo['api'],$attach['serverid']);
	...
	if($attach['serverid']>0 && $attach['isimage']) {
		dheader('location:http://'.$attachinfo['name'].'/portal/'.$attach['attachment']);
	}
	...
	$attach['remote'] ? getremotefile($attach['attachment']) :($attach['serverid']>0 ? getserverfile($attach['attachment'],"http://".$attachinfo['domain']['cdnurl']) : getlocalfile($filename, $readmod, $range));
}
...
function getserverfile($file,$domain){
	@set_time_limit(0);
    if(!@readfile($domain."/portal/".$file)){
        dheader("location: ".$domain."/portal/".$file);
    }
}

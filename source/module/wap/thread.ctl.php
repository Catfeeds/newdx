...
//			print_r($advShow);
		...
		//8264推荐活动开始
		require DISCUZ_ROOT . '/source/plugin/skiing/hd_zw.php';
		if($_G['fid']=='179'){
			//钓鱼活动推荐
			$huodong_zaiwai = ForumOptionHuoDong::getDataBySearch('海钓');
		}elseif ($_G['fid']=='135') {
			//攀岩活动推荐
			$huodong_zaiwai = ForumOptionHuoDong::getData(52);
		}elseif ($_G['fid']=='24') {
			//登山活动推荐
			$huodong_zaiwai = ForumOptionHuoDong::getData(51);
		}elseif ($_G['fid']=='447') {
			//水上运动活动推荐
			$huodong_zaiwai = ForumOptionHuoDong::getData(53);
		}elseif ($_G['fid']=='88') {
			//骑行活动推荐
			$huodong_zaiwai = ForumOptionHuoDong::getData(5);
		}else{
			//商业活动版活动推荐
			$huodong_zaiwai = ForumOptionHuoDong::getDataAll();
		}

		$returnData["huodongzaiwai"] = $huodong_zaiwai;
		//8264推荐活结束
		...
		if($postusers) {
			...
			foreach($postlist as $pid => $post) {
				if(!empty($post['attachment_old'])) {
					...
					//对图片进行处理
					$postlist[$post["pid"]]["message"] = dealThreadPic($post["message"]);
				}
			}
		}
...
//				case 2: require_once libfile('thread/trade', 'include'); break;
	...
	function viewthread_procpost($post, $lastvisit, $ordertype, $special = 0) {
		...
		//对文本中的图片进行降质处理
		$post["message"] = dealTextPic($post["message"]);
		...
		if ($matA[1]) {
			...
			while ($v = DB::fetch($query)) {
				...
				$pics[$v['aid']] = "<img src='{$_G["config"]['web']['portal']}static/images/loadding.gif' class='lazy' data-original='{$imgsrc}'/>";
			}

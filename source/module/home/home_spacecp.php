...
$acs = array('space', 'doing', 'upload', 'comment', 'blog', 'album', 'relatekw', 'common', 'class',
	'swfupload', 'poke', 'friend', 'eccredit', 'favorite', 'newinvite',
	'avatar', 'profile', 'theme', 'feed', 'privacy', 'pm', 'share', 'invite','sendmail',
	'credit', 'usergroup', 'domain', 'click','magic', 'top', 'videophoto', 'index', 'plugin', 'search', 'activity');

$ac = (empty($_GET['ac']) || !in_array($_GET['ac'], $acs))?'profile':$_GET['ac'];
$op = empty($_GET['op'])?'':$_GET['op'];

//对积分及积分规则页可以不登录
$iscredit = $_GET['ac'] == 'credit' && ($_GET['op'] == 'base' || $_GET['op'] == 'rule') && $_GET['uid'] ? true : false;

if(empty($_G['uid']) && !$iscredit) {	
